...
import { getContentRect, getMarginRect, extractDomRectValue } from "../../utils/css-geometry-tools";
...
type Props = {
  ...
  className: string;
  order: number[];
  onChangeOrder: (arg0: number[]) => unknown;
...
class Reorderable extends React.PureComponent<Props, State> {
  ...
  _onMouseDown = (event: {target: EventTarget;} & React.MouseEvent<HTMLElement>) => {
    const container = event.currentTarget;

    if (event.target === container || !(event.target instanceof HTMLElement) || // Only run for left clicks.
    event.button !== 0) {
      return;
    }
  ...
  _startDraggingElement(container: HTMLElement, element: HTMLElement, event: EventWithPageProperties) {
    ...
    const spaceBefore = extractDomRectValue(elementRect, xy.lefttop) - extractDomRectValue(containerRect, xy.lefttop);
    const spaceAfter = extractDomRectValue(containerRect, xy.rightbottom) - extractDomRectValue(elementRect, xy.rightbottom);
    ...
    const offsets = children.map((child, childIndex) => {
      ...
      return isBefore ? extractDomRectValue(childRect, xy.lefttop) - extractDomRectValue(elementRect, xy.lefttop) : extractDomRectValue(childRect, xy.rightbottom) - extractDomRectValue(elementRect, xy.rightbottom);
    });
    ...
    const nextEdgeAfterElement = elementIndex === children.length - 1 ? extractDomRectValue(containerRect, xy.rightbottom) : extractDomRectValue(getMarginRect(children[elementIndex + 1]), xy.lefttop);

    const nextEdgeBeforeElement = elementIndex === 0 ? extractDomRectValue(containerRect, xy.lefttop) : extractDomRectValue(getMarginRect(children[elementIndex - 1]), xy.rightbottom);

    this.setState({
      manipulatingIndex: elementIndex,
      manipulationDelta: 0,
      destinationIndex: elementIndex,
      adjustPrecedingBy: nextEdgeAfterElement - extractDomRectValue(elementRect, xy.lefttop),
      adjustSucceedingBy: nextEdgeBeforeElement - extractDomRectValue(elementRect, xy.rightbottom)
    });
  ...
  render() {
    ...
    if (this.state.phase === 'RESTING') {
      return <TagName className={className} onMouseDown={this._onMouseDown}>
          {orderedChildren}
        </TagName>;
    }
    ...
    return <TagName className={adjustedClassName}>
