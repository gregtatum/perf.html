...
class ArrowPanel extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    return <div className="arrowPanelAnchor">
        <div className={classNames('arrowPanel', { open, hasTitle, hasButtons }, className)} onClick={this._onArrowPanelClick} ref={this._takePanelElementRef}>
          <div className="arrowPanelArrow" />
          {hasTitle ? <h1 className="arrowPanelTitle">{title}</h1> : null}
          {open || isClosing ? <div className="arrowPanelContent">{children}</div> : null}
          {hasButtons ? <div className="arrowPanelButtons">
              <input type="button" className="arrowPanelCancelButton" value={cancelButtonText} onClick={this._onCancelButtonClick} />
              <input type="button" className="arrowPanelOkButton" value={okButtonText} onClick={this._onOkButtonClick} />
            </div> : null}
        </div>
      </div>;
  }
