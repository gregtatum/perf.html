...
import { IndexIntoSamplesTable, IndexIntoCategoryList, Thread } from "../../../types/profile";
import { SelectedState } from "../../../types/profile-derived";
import { Milliseconds, DevicePixels, CssPixels } from "../../../types/units";
...
type RenderedComponentSettings = {
  ...
  readonly treeOrderSampleComparator: (arg0: IndexIntoSamplesTable, arg1: IndexIntoSamplesTable) => number | null | undefined;
...
const SMOOTHING_KERNEL: Float32Array = _getSmoothingKernel(SMOOTHING_RADIUS, BOX_BLUR_RADII);

export function computeActivityGraphFills(renderedComponentSettings: RenderedComponentSettings) {
  const mutablePercentageBuffers = _createSelectedPercentageAtPixelBuffers(renderedComponentSettings);
  const mutableFills = _getCategoryFills(renderedComponentSettings.categoryDrawStyles, mutablePercentageBuffers);
  const activityGraphFills = new ActivityGraphFillComputer(renderedComponentSettings, mutablePercentageBuffers, mutableFills);
  ...
  return {
    fills,
    fillsQuerier: new ActivityFillGraphQuerier(renderedComponentSettings, fills)
  };
}
...
export class ActivityGraphFillComputer {
  ...
  constructor(renderedComponentSettings: RenderedComponentSettings, mutablePercentageBuffers: SelectedPercentageAtPixelBuffers[], mutableFills: CategoryFill[]) {
  ...
  _accumulateUpperEdge(): void {
    ...
    for (const {
      ...
      for (let i = 0; i < perPixelContribution.length; i++) {
        accumulatedUpperEdge[i] = previousUpperEdge[i] + perPixelContribution[i];
      }
  ...
  _accumulateSampleCategories() {
    ...
    for (let i = 0; i < samples.length - 1; i++) {
      ...
      const category = stackIndex === null ? greyCategoryIndex : stackTable.category[stackIndex];

      // Mutate the percentage buffers.
      this._accumulateInCategory(category, i, prevSampleTime, sampleTime, nextSampleTime);
    ...
    const lastSampleCategory = lastSampleStack !== null ? stackTable.category[lastSampleStack] : greyCategoryIndex;

    this._accumulateInCategory(lastSampleCategory, samples.length - 1, prevSampleTime, sampleTime, sampleTime + interval);
  }
   ...
   */
  _accumulateInCategory(category: IndexIntoCategoryList, sampleIndex: IndexIntoSamplesTable, prevSampleTime: Milliseconds, sampleTime: Milliseconds, nextSampleTime: Milliseconds) {
    const {
      rangeEnd,
      rangeStart,
      categoryDrawStyles,
    ...
    // |       |       +-------+///////////////////////|       |       |
    // +-------+-------+///////////////////////////////+-------+-------+
    const percentageBuffer = this._pickPercentageBuffer(percentageBuffers, sampleIndex);
   ...
   */
  _pickPercentageBuffer(percentageBuffers: SelectedPercentageAtPixelBuffers, sampleIndex: IndexIntoSamplesTable): Float32Array {
    const {
...
export class ActivityFillGraphQuerier {
  ...
  constructor(renderedComponentSettings: RenderedComponentSettings, fills: CategoryFill[]) {
    this.renderedComponentSettings = renderedComponentSettings;
    this.fills = fills;
  }
   ...
   */
  getSampleAtClick(cssX: CssPixels, cssY: CssPixels, time: Milliseconds, canvasBoundingRect: ClientRect): IndexIntoSamplesTable | null {
    const {
      canvasPixelWidth
    } = this.renderedComponentSettings;
    const devicePixelRatio = canvasPixelWidth / canvasBoundingRect.width;
    const categoryUnderMouse = this._categoryAtDevicePixel(Math.round(cssX * devicePixelRatio), Math.round(cssY * devicePixelRatio));
    if (categoryUnderMouse === null) {
      return null;
    }

    let {
      offsetToCategoryStart
    } = categoryUnderMouse;
    const candidateSamples = this._getCategoriesSamplesAtTime(time, categoryUnderMouse.category);
   ...
   */
  _categoryAtDevicePixel(deviceX: DevicePixels, deviceY: DevicePixels): null | {
    ...
    if (deviceX < 0 || deviceX >= canvasPixelWidth || deviceY < 0 || deviceY >= canvasPixelHeight) {
      return null;
    }
   ...
   */
  _getCategoriesSamplesAtTime(time: number, category: IndexIntoCategoryList): ReadonlyArray<SampleContributionToPixel> {
    const {
      rangeStart,
      rangeEnd,
    ...
    const [sampleRangeStart, sampleRangeEnd] = this._getSampleRangeContributingToPixelWhenSmoothed(xPixel);

    const sampleContributions = [];
    for (let sample = sampleRangeStart; sample < sampleRangeEnd; sample++) {
      const stackIndex = samples.stack[sample];
      const sampleCategory = stackIndex !== null ? stackTable.category[stackIndex] : greyCategoryIndex;
      if (sampleCategory === category) {
        sampleContributions.push({
          sample,
          contribution: this._getSmoothedContributionFromSampleToPixel(xPixel, xPixelsPerMs, sample)
        });
      }
    }
   ...
   */
  _getSampleRangeContributingToPixelWhenSmoothed(xPixel: number): [IndexIntoSamplesTable, IndexIntoSamplesTable] {
    const {
      fullThread: {
        samples
      },
    ...
    const contributionTimeRangeStart = rangeStart + (xPixel - SMOOTHING_RADIUS) / xPixelsPerMs;
    const contributionTimeRangeEnd = rangeStart + (xPixel + SMOOTHING_RADIUS) / xPixelsPerMs;
    ...
    const firstSampleAfterContributionTimeRangeStart = bisection.right(samples.time, contributionTimeRangeStart);
    const firstSampleAfterContributionTimeRangeEnd = bisection.right(samples.time, contributionTimeRangeEnd);
    return [Math.max(0, firstSampleAfterContributionTimeRangeStart - 1), Math.min(samples.length - 1, firstSampleAfterContributionTimeRangeEnd) + 1];
  }
   ...
   * been applied.
   */
  _getSmoothedContributionFromSampleToPixel(xPixel: number, xPixelsPerMs: number, sample: IndexIntoSamplesTable): number {
    const {
      fullThread: {
        samples
      },
    ...
    const sampleTimeRangeStart = sample > 0 ? (samples.time[sample - 1] + sampleTime) / 2 : -Infinity;
    const sampleTimeRangeEnd = sample + 1 < samples.length ? (samples.time[sample + 1] + sampleTime) / 2 : Infinity;

    let pixelStart = (sampleTimeRangeStart - rangeStart) * xPixelsPerMs - kernelPos;
 ...
 */
function _getSmoothingKernel(smoothingRadius: number, boxBlurRadii: number[]): Float32Array {
 ...
 */
function _getCategoryFills(categoryDrawStyles: CategoryDrawStyles, percentageBuffers: SelectedPercentageAtPixelBuffers[]): CategoryFill[] {
  // Sort all of the categories by their gravity.
  const categoryIndexesByGravity = categoryDrawStyles.map((_, i) => i).sort((a, b) => categoryDrawStyles[b].gravity - categoryDrawStyles[a].gravity);

  const nestedFills: CategoryFill[][] = categoryIndexesByGravity.map(categoryIndex => {
    ...
    return [{
      ...
      accumulatedUpperEdge: new Float32Array(buffer.beforeSelectedPercentageAtPixel.length)
    }, {
      category: categoryDrawStyle.category,
 ...
 */
function _boxBlur1D(srcArray: Float32Array, destArray: Float32Array, radius: number): void {
 ...
 */
function _applyGaussianBlur1D(srcArray: Float32Array, boxBlurRadii: number[]): void {
  let a = srcArray;
