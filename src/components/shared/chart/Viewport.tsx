...
import { getHasZoomedViaMousewheel, getPanelLayoutGeneration } from "../../../selectors/app";
import { setHasZoomedViaMousewheel } from "../../../actions/app";
import { updatePreviewSelection } from "../../../actions/profile-view";

import { CssPixels, UnitIntervalOfProfileRange, StartEndRange } from "../../../types/units";
import { PreviewSelection } from "../../../types/actions";
import { ConnectedProps } from "../../../utils/connect";
import { getObjectValuesAsUnion, assertExhaustiveCheck } from "../../../utils/flow";
...
const {
  DOM_DELTA_PAGE,
  DOM_DELTA_LINE
} = typeof window === 'object' && window.WheelEvent ? new WheelEvent('mouse') : { DOM_DELTA_LINE: 1, DOM_DELTA_PAGE: 2 };
...
export type Viewport = {
  ...
  readonly moveViewport: (arg0: CssPixels, arg1: CssPixels) => void;
...
type ViewportOwnProps<ChartProps> = {
  readonly viewportProps: {
    ...
    readonly containerRef?: (arg0: HTMLDivElement | null) => void;
    ...
    //     MarkerChartCanvas
    //   )
    readonly viewportNeedsUpdate: (prevProps: ChartProps, nextProps: ChartProps) => boolean;
  };
 ...
 */
export type WithChartViewport<ChartOwnProps extends Object, // The chart component's props are given the viewport object, as well as the original
// ChartOwnProps.
ChartProps extends $ReadOnly<ChartOwnProps & {
  viewport: Viewport;
}>> = ( // Take as input a React component whose props accept the { +viewport: Viewport }.
ChartComponent: React.ComponentType<ChartProps>) => React.ComponentType< // Finally the returned component takes as input the InternalViewportProps, and
...
// generic parameters.
export const withChartViewport: WithChartViewport<any, any> = // ChartOwnProps is the only generic actually used in the implementation. Infer
// the type signature of the arguments as the WithChartViewport will apply them.
<ChartOwnProps>(ChartComponent: React.ComponentType<$Subtype<{readonly viewport: Viewport;}>>): any => {
  type ViewportProps = ConnectedProps<ViewportOwnProps<ChartOwnProps>, ViewportStateProps, ViewportDispatchProps>;

  class ChartViewport extends React.PureComponent<ViewportProps, State> {

    zoomScrollId: number = 0;
    _pendingPreviewSelectionUpdates: Array<(arg0: HorizontalViewport) => PreviewSelection> = [];
    ...
    getHorizontalViewport(previewSelection: PreviewSelection, timeRange: StartEndRange) {
    ...
    getDefaultState(props: ViewportProps) {
      ...
      const horizontalViewport = this.getHorizontalViewport(previewSelection, timeRange);
      const {
        startsAtBottom,
        maxViewportHeight
      } = props.viewportProps;
    ...
    UNSAFE_componentWillReceiveProps(newProps: ViewportProps) {
      if (this.props.viewportProps.viewportNeedsUpdate(this.props.chartProps, newProps.chartProps)) {
        this.setState(this.getDefaultState(newProps));
        this._setSizeNextFrame();
      } else if (this.props.viewportProps.previewSelection !== newProps.viewportProps.previewSelection || this.props.viewportProps.timeRange !== newProps.viewportProps.timeRange) {
        const {
          previewSelection,
          timeRange
      ...
      } else if (this.props.panelLayoutGeneration !== newProps.panelLayoutGeneration) {
        this._setSizeNextFrame();
      }
    }
      ...
      if (this._container) {
        ...
        if (this.state.containerWidth !== rect.width || this.state.containerHeight !== rect.height) {
          this.setState(prevState => ({
            ...
            viewportBottom: startsAtBottom ? prevState.viewportBottom : prevState.viewportTop + rect.height,
            viewportTop: startsAtBottom ? prevState.viewportBottom - rect.height : prevState.viewportTop,
            isSizeSet: true
          }));
        }
      }
    ...
    _mouseWheelListener = (event: React.WheelEvent<>) => {
     ...
     */
    _addBatchedPreviewSelectionUpdate(callback: (arg0: HorizontalViewport) => PreviewSelection) {
    ...
    _flushPendingPreviewSelectionUpdates() {
      if (this._pendingPreviewSelectionUpdates.length !== 0) {
        ...
        for (const callback of pendingUpdates) {
          const horizontalViewport = this.getHorizontalViewport(nextSelection, timeRange);
    ...
    zoomWithMouseWheel(event: React.WheelEvent<>, // Allow different handlers to make the zoom faster or slower.
    zoomModifier: number = 1) {
      ...
      const deltaY = getNormalizedScrollDelta(event, this.state.containerHeight, // Shift is a modifier that will change some mice to scroll horizontally, check
      // for that here.
      ...
      const mouseCenter = (mouseX - containerLeft - marginLeft) / innerContainerWidth;
    ...
    zoomRangeSelection = (center, zoomFactor) => {
      ...
      this._addBatchedPreviewSelectionUpdate(({
        ...
        const newViewportLength = clamp(maximumZoom, 1, viewportLength * zoomFactor);
        const deltaViewportLength = newViewportLength - viewportLength;
        const newViewportLeft = clamp(0, 1 - newViewportLength, viewportLeft - deltaViewportLength * center);
        const newViewportRight = clamp(newViewportLength, 1, viewportRight + deltaViewportLength * (1 - center));
    ...
    _mouseDownListener = (event: React.MouseEvent<>) => {
      event.preventDefault();
      if (this._container) {
    ...
    _keyDownListener = (event: {nativeEvent: KeyboardEvent;} & React.KeyboardEvent<>) => {
      let navigationKey;
    ...
    _keyUpListener = (event: {nativeEvent: KeyboardEvent;} & React.KeyboardEvent<>) => {
      if (!event.ctrlKey) {
    ...
    _keyboardNavigation = timestamp => {
      ...
      const delta = Math.min(KEYBOARD_NAVIGATION_SPEED * (timestamp - this._lastKeyboardNavigationFrame) * 0.001, MAX_KEYBOARD_DELTA // Don't jump like crazy if we experience long janks
      );
    ...
    render() {
      ...
      return <div className={viewportClassName} onWheel={this._mouseWheelListener} onMouseDown={this._mouseDownListener} onKeyDown={this._keyDownListener} onKeyUp={this._keyUpListener} ref={this._takeContainerRef} tabIndex={0}>
  ...
  return explicitConnect<ViewportOwnProps<ChartOwnProps>, ViewportStateProps, ViewportDispatchProps>({
 ...
 */
function getNormalizedScrollDelta(event: React.WheelEvent<>, pageHeight: number, key: "deltaY" | "deltaX"): CssPixels {
