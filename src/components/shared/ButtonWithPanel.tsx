...
type Props = {
  className: string;
  label: string;
  panel: React.ReactElement<Class<Panel & React.Component<$Subtype<PanelProps>, any>>>;
  open?: boolean;
...
class ButtonWithPanel extends React.PureComponent<Props, State> {
  ...
  render() {
    ...
    return <div className={classNames('buttonWithPanel', className, { open })}>
        <div className="buttonWithPanelButtonWrapper">
          <input type="button" className={classNames('buttonWithPanelButton', buttonClassName)} value={label} onClick={this._onButtonClick} />
        </div>
