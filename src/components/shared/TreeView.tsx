...
const TreeViewHeader = ({
  ...
  return <div className="treeViewHeader">
      {fixedColumns.map(col => <span className={`treeViewHeaderColumn treeViewFixedColumn ${col.propName}`} key={col.propName}>
          {col.title}
        </span>)}
...
function reactStringWithHighlightedSubstrings(string: string, regExp: RegExp | null, className: string) {
  ...
  while (result = regExp.exec(string)) {
    ...
    highlighted.push(<span key={typedResult.index} className={className}>
        {typedResult[0]}
      </span>);
  }
...
type TreeViewRowFixedColumnsProps<DisplayData extends Object> = {
  ...
  readonly onClick: (arg0: NodeIndex, arg1: React.MouseEvent<>) => unknown;
  readonly highlightRegExp: RegExp | null;
  readonly rowHeightStyle: {height: CssPixels;lineHeight: string;};
};

class TreeViewRowFixedColumns<DisplayData extends Object> extends React.PureComponent<TreeViewRowFixedColumnsProps<DisplayData>> {
  ...
  render() {
    ...
    return <div className={classNames('treeViewRow', 'treeViewRowFixedColumns', {
      even: index % 2 === 0,
      odd: index % 2 !== 0,
      isSelected,
      isRightClicked
    })} style={rowHeightStyle} onMouseDown={this._onClick}>
        {columns.map(col => {
        const RenderComponent = col.component;
        const text = displayData[col.propName] || '';

        return <span className={`treeViewRowColumn treeViewFixedColumn ${col.propName}`} key={col.propName} title={text}>
              {RenderComponent ? <RenderComponent displayData={displayData} /> : reactStringWithHighlightedSubstrings(text, highlightRegExp, 'treeViewHighlighting')}
            </span>;
      })}
      </div>;
  }
...
type TreeViewRowScrolledColumnsProps<DisplayData extends Object> = {
  readonly displayData: DisplayData;
  readonly nodeId: NodeIndex;
  readonly depth: number;
  readonly mainColumn: Column;
  readonly appendageColumn?: Column;
  readonly index: number;
  readonly canBeExpanded: boolean;
  readonly isExpanded: boolean;
  readonly isSelected: boolean;
  readonly isRightClicked: boolean;
  readonly onToggle: (arg0: NodeIndex, arg1: boolean, arg2: boolean) => unknown;
  readonly onClick: (arg0: NodeIndex, arg1: React.MouseEvent<>) => unknown;
  readonly highlightRegExp: RegExp | null;
...
class TreeViewRowScrolledColumns<DisplayData extends Object> extends React.PureComponent<TreeViewRowScrolledColumnsProps<DisplayData>> {
  /**
   * In this mousedown handler, we use event delegation so we have to use
   * `target` instead of `currentTarget`.
   */
  _onMouseDown = (event: {target: Element;} & React.MouseEvent<Element>) => {
    const {
      nodeId,
      onClick
    } = this.props;
  ...
  _onToggleClick = (event: {target: Element;} & React.MouseEvent<Element>) => {
    const {
      nodeId,
      isExpanded,
      onToggle
  ...
  render() {
    ...
    return <div className={classNames('treeViewRow', 'treeViewRowScrolledColumns', {
      even: index % 2 === 0,
      odd: index % 2 !== 0,
      isSelected,
      isRightClicked,
      dim: displayData.isFrameLabel
    })} style={rowHeightStyle} onMouseDown={this._onMouseDown} // The following attributes are important for accessibility.
    aria-expanded={ariaExpanded} aria-level={depth + 1} aria-selected={isSelected} aria-label={displayData.ariaLabel} // The role and id attributes are used along with aria-activedescendant
    // (set on the parent), to manage the virtual focus of the tree items.
    // The "virtual" focus changes with the arrow keys.
    role="treeitem" id={`treeViewRow-${nodeId}`}>
        <span className="treeRowIndentSpacer" style={{ width: `${depth * indentWidth}px` }} />
        <span className={`treeRowToggleButton ${isExpanded ? 'expanded' : 'collapsed'} ${canBeExpanded ? 'canBeExpanded' : 'leaf'}`} onClick={this._onToggleClick} />
        <span className={`treeViewRowColumn treeViewMainColumn ${mainColumn.propName}`}>
          {displayData.categoryColor && displayData.categoryName ? <span className={`colored-square category-color-${displayData.categoryColor}`} title={displayData.categoryName} /> : null}
          {RenderComponent ? <RenderComponent displayData={displayData} /> : reactStringWithHighlightedSubstrings(displayData[mainColumn.propName], highlightRegExp, 'treeViewHighlighting')}
        </span>
        {appendageColumn ? <span className={`treeViewRowColumn treeViewAppendageColumn ${appendageColumn.propName}`}>
            {reactStringWithHighlightedSubstrings(displayData[appendageColumn.propName], highlightRegExp, 'treeViewHighlighting')}
          </span> : null}
      </div>;
  }
}

interface Tree<DisplayData extends Object> {
  getDepth(arg0: NodeIndex): number;
  getRoots(): NodeIndex[];
  getDisplayData(arg0: NodeIndex): DisplayData;
  getParent(arg0: NodeIndex): NodeIndex;
  getChildren(arg0: NodeIndex): NodeIndex[];
  hasChildren(arg0: NodeIndex): boolean;
  getAllDescendants(arg0: NodeIndex): Set<NodeIndex>;
}
...
type TreeViewProps<DisplayData> = {
  ...
  readonly onExpandedNodesChange: (arg0: Array<NodeIndex | null>) => unknown;
  readonly highlightRegExp?: RegExp | null;
  readonly appendageColumn?: Column;
  ...
  readonly onSelectionChange: (arg0: NodeIndex) => unknown;
  readonly onRightClickSelection?: (arg0: NodeIndex) => unknown;
  readonly onEnterKey?: (arg0: NodeIndex) => unknown;
  readonly rowHeight: CssPixels;
  readonly indentWidth: CssPixels;
};

class TreeView<DisplayData extends Object> extends React.PureComponent<TreeViewProps<DisplayData>> {
  ...
  _updateSpecialItems(props: TreeViewProps<DisplayData>) {
    ...
    if (props.rightClickedNodeId !== undefined && props.rightClickedNodeId !== null) {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: TreeViewProps<DisplayData>) {
    const hasNewSelectedNode = nextProps.selectedNodeId !== this.props.selectedNodeId;
    const hasNewRightClickedNode = nextProps.rightClickedNodeId !== this.props.rightClickedNodeId;
  ...
  _renderRow = (nodeId: NodeIndex, index: number, columnIndex: number) => {
    ...
    if (columnIndex === 0) {
      return <TreeViewRowFixedColumns displayData={displayData} columns={fixedColumns} nodeId={nodeId} index={index} isSelected={nodeId === selectedNodeId} isRightClicked={nodeId === rightClickedNodeId} onClick={this._onRowClicked} highlightRegExp={highlightRegExp || null} rowHeightStyle={rowHeightStyle} />;
    }

    const canBeExpanded = tree.hasChildren(nodeId);
    const isExpanded = canBeExpanded && !this._isCollapsed(nodeId);

    return <TreeViewRowScrolledColumns rowHeightStyle={rowHeightStyle} displayData={displayData} mainColumn={mainColumn} appendageColumn={appendageColumn} depth={tree.getDepth(nodeId)} nodeId={nodeId} index={index} canBeExpanded={canBeExpanded} isExpanded={isExpanded} isSelected={nodeId === selectedNodeId} isRightClicked={nodeId === rightClickedNodeId} onToggle={this._toggle} onClick={this._onRowClicked} highlightRegExp={highlightRegExp || null} indentWidth={indentWidth} />;
  };

  _addVisibleRowsFromNode(props: TreeViewProps<DisplayData>, arr: NodeIndex[], nodeId: NodeIndex, depth: number) {
    arr.push(nodeId);
  ...
  _toggle = (nodeId: NodeIndex, newExpanded: boolean = this._isCollapsed(nodeId), toggleAll: any = false) => {
  ...
  _onCopy = (event: any) => {
    ...
    if (selectedNodeId) {
      ...
      const clipboardData: DataTransfer = (event as Object).clipboardData;
  ...
  _onKeyDown = (event: KeyboardEvent) => {
    ...
    const isNavigationKey = event.key.startsWith('Arrow') || event.key.startsWith('Page') || event.key === 'Home' || event.key === 'End';
  ...
  render() {
    ...
    return <div className="treeView">
        ...
        <ContextMenuTrigger id={contextMenuId} attributes={{ className: 'treeViewContextMenu' }}>
          <VirtualList className="treeViewBody" ariaRole="tree" ariaLabel="Call tree" // This attribute exposes the current active child element,
        // while keeping focus on the parent (call tree).
        ariaActiveDescendant={selectedNodeId ? `treeViewRow-${selectedNodeId}` : null} items={this._visibleRows} renderItem={this._renderRow} itemHeight={rowHeight} columnCount={2} focusable={true} onKeyDown={this._onKeyDown} specialItems={this._specialItems} disableOverscan={!!disableOverscan} onCopy={this._onCopy} // If there is a deep call node depth, expand the width, or else keep it
        // at 3000 wide.
        containerWidth={Math.max(3000, maxNodeDepth * 10 + 2000)} ref={this._takeListRef} />
        </ContextMenuTrigger>
        {contextMenu}
      </div>;
  }
