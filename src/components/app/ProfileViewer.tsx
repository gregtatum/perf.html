...
import { getUploadProgress, getUploadPhase, getIsHidingStaleProfile, getHasSanitizedProfile } from "../../selectors/publish";
...
import { IconWithClassName } from "../../types/state";
...
type StateProps = {
  ...
  readonly icons: IconWithClassName[];
};
  ...
  render() {
    ...
    return <div className={classNames({
      ...
      profileViewerWrapperBackground: hasSanitizedProfile
        ...
        className,
        icon
      }) => <BackgroundImageStyleDef className={className} url={icon} key={className} />)}
        <div className={classNames({
        profileViewer: true,
        profileViewerFadeInSanitized: hasSanitizedProfile && !isHidingStaleProfile,
        profileViewerFadeOut: isHidingStaleProfile
      })} style={timelineHeight === null ? {} : {
        '--profile-viewer-splitter-max-height': `${timelineHeight}px`
      }}>
          <div className="profileViewerTopBar">
            {hasZipFile ? <button type="button" className="profileViewerZipButton" title="View all files in the zip file" onClick={returnToZipFileList} /> : null}
            {profileName ? <div className="profileViewerName">{profileName}</div> : null}
            ...
            <MenuButtons />
            {isUploading ? <div className="menuButtonsPublishUploadBarInner" style={{ width: `${uploadProgress * 100}%` }} /> : null}
          </div>
          <SplitterLayout customClassName="profileViewerSplitter" vertical percentage={false} // The DetailsContainer is primary.
        primaryIndex={1} // The Timeline is secondary.
        secondaryInitialSize={270} onDragEnd={invalidatePanelLayout}>
            <Timeline />
            <DetailsContainer />
          </SplitterLayout>
          <WindowTitle />
