...
import { changeSelectedZipFile, changeExpandedZipFile, viewProfileFromZip, returnToZipFileList, showErrorForNoFileInZip } from "../../actions/zipped-profiles";
import { getZipFileState, getZipFileTree, getZipFileTable, getZipFileMaxDepth, getSelectedZipFileIndex, getExpandedZipFileIndexes, getZipFileErrorMessage } from "../../selectors/zipped-profiles";
import { getPathInZipFileFromUrl } from "../../selectors/url-state";
import TreeView from "../shared/TreeView";
import ProfileViewer from "./ProfileViewer";

import { ConnectedProps } from "../../utils/connect";
import { ZipFileState } from "../../types/state";
import { ZipFileTable, ZipDisplayData, ZipFileTree, IndexIntoZipFileTable } from "../../profile-logic/zip-files";
...
type StateProps = {
  ...
  readonly zipFileTable: ZipFileTable;
...
type DispatchProps = {
  ...
  readonly showErrorForNoFileInZip: typeof showErrorForNoFileInZip;
};
...
type ZipFileRowProps = ConnectedProps<ZipFileRowOwnProps, {}, ZipFileRowDispatchProps>;
...
class ZipFileRowImpl extends React.PureComponent<ZipFileRowProps> {
  ...
  render() {
    ...
    return <a href={url} onClick={this._handleClick}>
        {name}
      </a>;
  }
}

const ZipFileRow = explicitConnect<ZipFileRowOwnProps, {}, ZipFileRowDispatchProps>({
...
class ZipFileViewer extends React.PureComponent<Props> {
  ...
  _treeView: TreeView<ZipDisplayData> | null | undefined;
  ...
  constructor(props) {
    ...
    if (expandedZipFileIndexes.length === 0 && zipFileTree) {
      changeExpandedZipFile(procureInitialInterestingExpandedNodes(zipFileTree));
    }
  }
  ...
  componentDidMount() {
    ...
    if (zipFileState.phase === 'NONE' && pathInZipFile) {
      ...
      if (!zipFileTree) {
        throw new Error('The zipFileTree should exist if this component was mounted');
      }
    }
    this.focus();
  }
  ...
  componentDidUpdate(prevProps: Props) {
    if (prevProps.zipFileState.phase !== 'LIST_FILES_IN_ZIP_FILE' && this.props.zipFileState.phase === 'LIST_FILES_IN_ZIP_FILE') {
  ...
  _renderMessage(message: React.ReactNode) {
    return <section className="zipFileViewer">
  ...
  _renderBackButton() {
    return <button key="backButton" type="button" className="homeSectionInstallButton" onClick={this.props.returnToZipFileList}>
  ...
  render() {
    ...
    switch (phase) {
      case 'NO_ZIP_FILE':
        console.error('Loaded the ZipFileViewer component when there is no zip file.');
      ...
      case 'LIST_FILES_IN_ZIP_FILE':
        return <section className="zipFileViewer">
            <div className="zipFileViewerSection">
              ...
              <TreeView maxNodeDepth={zipFileMaxDepth} tree={zipFileTree} fixedColumns={this._fixedColumns} mainColumn={this._mainColumn} onSelectionChange={changeSelectedZipFile} onExpandedNodesChange={changeExpandedZipFile} selectedNodeId={selectedZipFileIndex} expandedNodeIds={expandedZipFileIndexes} ref={this._takeTreeViewRef} contextMenuId="unused" rowHeight={30} indentWidth={15} onEnterKey={this._onEnterKey} />
              <DragAndDropOverlay />
            </div>
          </section>;
      case 'PROCESS_PROFILE_FROM_ZIP_FILE':
        return this._renderMessage(<span>Loading the profile from the zip file...</span>);
      case 'FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE':
        return this._renderMessage([<p key="message">Unable to load the profile at this path:</p>, <span className="zipFileViewerUntrustedFilePath" key="path">
            /
          ...
          // with it.
          pathInZipFile}
          </span>, <p key="error">{zipFileErrorMessage}</p>, this._renderBackButton()]);
      case 'FILE_NOT_FOUND_IN_ZIP_FILE':
        return this._renderMessage([<span key="message">
            Failed to find a file in the zip at the following path:
          </span>, <span className="zipFileViewerUntrustedFilePath" key="path">
            /
            {// This is text that comes from the URL, make sure it looks visually
          // distinct for guarding against someone doing anything too funky
          // with it.
          pathInZipFile}
          </span>, this._renderBackButton()]);
...
export default explicitConnect<{}, StateProps, DispatchProps>({
  mapStateToProps: state => {
    ...
    if (zipFileTree === null) {
      throw new Error('The zipFileTree should exist if the ZipFileViewer is mounted.');
    }
    const zipFileTable = getZipFileTable(state);
    if (zipFileTable === null) {
      throw new Error('The zipFileTable should exist if the ZipFileViewer is mounted.');
    }
    return {
