...
import { getPreviewSelection, getProfileFilterPageData, getProfileRootRange } from "../../selectors/profile";
...
import { ProfileFilterPageData } from "../../types/profile-derived";
import { StartEndRange } from "../../types/units";

type Props = ElementProps<typeof FilterNavigatorBar> & {
  readonly filterPageData: ProfileFilterPageData | null;
...
class ProfileFilterNavigatorBar extends React.PureComponent<Props> {
  ...
  _getItemsWithFirstElement = memoize((firstItem, items) => [firstItem, ...items], {
    limit: 1
  });
  ...
  render() {
    ...
    const itemsWithFirstElement = this._getItemsWithFirstElement(firstItem, items);
    return <FilterNavigatorBar className={className} items={itemsWithFirstElement} selectedItem={selectedItem} uncommittedItem={uncommittedItem} onPop={onPop} />;
  }
}
...
export default explicitConnect<{}, StateProps, DispatchProps>({
  mapStateToProps: state => {
    ...
    const uncommittedItem = previewSelection.hasSelection ? getFormattedTimeLength(previewSelection.selectionEnd - previewSelection.selectionStart) : undefined;
