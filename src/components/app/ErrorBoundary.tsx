...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: unknown, {
    ...
    console.error('An unhandled error was thrown in a React component.', error, componentStack);
    ...
    reportError({
      exDescription: errorString ? errorString + '\n' + componentStack : componentStack,
      exFatal: true
    });
  }
  ...
  render() {
    if (this.state.hasError) {
      ...
      return <div className="appErrorBoundary">
          <div className="appErrorBoundaryContents">
            <div className="photon-message-bar photon-message-bar-error">
              {this.props.message}
              <button className="photon-button photon-button-micro photon-message-bar-action-button" type="button" onClick={this._toggleErrorDetails} aria-expanded={showDetails ? 'true' : 'false'}>
                {showDetails ? 'Hide error details' : 'View full error details'}
              </button>
            </div>
            <div data-testid="error-technical-details" className={`appErrorBoundaryDetails ${showDetails ? '' : 'hide'}`}>
