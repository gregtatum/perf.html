...
class ProfileViewer extends PureComponent<Props> {
  ...
  render() {
    ...
    const extraButton = hasSidebar && <button className={classNames('sidebar-open-close-button', 'photon-button', 'photon-button-ghost', {
      'sidebar-open-close-button-isopen': isSidebarOpen,
      'sidebar-open-close-button-isclosed': !isSidebarOpen
    })} title={isSidebarOpen ? 'Close the sidebar' : 'Open the sidebar'} type="button" onClick={this._onClickSidebarButton} />;

    return <div className="Details">
        <TabBar selectedTabSlug={selectedTab} visibleTabs={visibleTabs} onSelectTab={this._onSelectTab} extraElements={extraButton} />
        <ErrorBoundary key={selectedTab} message="Uh oh, some unknown error happened in this panel.">
          {{
