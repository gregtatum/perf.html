...
import { retrieveProfileFromFile, triggerLoadingFromUrl } from "../../actions/receive-profile";
import { CSSTransition, TransitionGroup } from "react-transition-group";
import { queryIsMenuButtonEnabled, enableMenuButton } from "../../app-logic/web-channel";
import { assertExhaustiveCheck } from "../../utils/flow";

import { ConnectedProps, WrapFunctionInDispatch } from "../../utils/connect";

require('./Home.css');

const ADDON_URL = 'https://raw.githubusercontent.com/firefox-devtools/Gecko-Profiler-Addon/master/gecko_profiler.xpi';
...
class InstallButton extends React.PureComponent<InstallButtonProps> {
  ...
  onInstallClick = (e: React.SyntheticEvent<HTMLAnchorElement>) => {
  ...
  render() {
    ...
    return <a href={xpiUrl} className={className} onClick={this.onInstallClick}>
        {children}
      </a>;
  }
}
...
type ActionButtonsProps = {
  readonly retrieveProfileFromFile: WrapFunctionInDispatch<typeof retrieveProfileFromFile>;
...
class ActionButtons extends React.PureComponent<ActionButtonsProps, ActionButtonsState> {
  ...
  _loadFromUrlPressed = (event: React.SyntheticEvent<>) => {
  ...
  render() {
    return <div className="homeSectionLoadProfile">
        <div className="homeSectionActionButtons">
          <label className="homeSectionButton">
            <input className="homeSectionUploadFromFileInput" type="file" ref={this._takeInputRef} onChange={this._uploadProfileFromFile} />
            Load a profile from file
          </label>
          <button type="button" className={classNames({
          homeSectionButton: true
        })} onClick={this._loadFromUrlPressed} // when the button is clicked it expands to the URL input
        aria-expanded={this.state.isLoadFromUrlPressed}>
            Load a profile from a URL
          </button>
        </div>
        {this.state.isLoadFromUrlPressed ? <LoadFromUrl {...this.props} /> : <p>You can also drag and drop a profile file here to load it.</p>}
      </div>;
  }
}
...
class LoadFromUrl extends React.PureComponent<LoadFromUrlProps, LoadFromUrlState> {
  ...
  _upload = (event: React.SyntheticEvent<>) => {
    event.preventDefault();
    if (this.state.value) {
  ...
  render() {
    return <form className="homeSectionLoadFromUrl" onSubmit={this._upload}>
        <input className="homeSectionLoadFromUrlInput photon-input" type="url" placeholder="https://" value={this.state.value} onChange={this.handleChange} autoFocus />
        <input type="submit" className="homeSectionButton homeSectionLoadFromUrlSubmitButton" value="Load" />
      </form>;
  }
}

function DocsButton() {
  return <a href="/docs/" className="homeSectionButton">
...
function InstructionTransition(props: {}) {
  return <CSSTransition {...props} classNames="homeTransition" timeout={300} exit={false} />;
}
...
type PopupAddonInstallPhase = // Firefox Beta or Relase
"suggest-install-addon" | "addon-installed" // Firefox Nightly:
| "popup-enabled" | "suggest-enable-popup" // Other browsers:
| "other-browser";
...
class Home extends React.PureComponent<HomeProps, HomeState> {
  ...
  constructor(props: HomeProps) {
    ...
    if (_isFirefox()) {
      ...
      queryIsMenuButtonEnabled().then(isMenuButtonEnabled => {
        this.setState({
          popupAddonInstallPhase: isMenuButtonEnabled ? 'popup-enabled' : 'suggest-enable-popup'
        });
  ...
  addonInstalled() {
    this.setState(({
      ...
      if (popupAddonInstallPhase === 'popup-enabled' || popupAddonInstallPhase === 'suggest-enable-popup') {
  ...
  _renderInstructions() {
    ...
    switch (popupAddonInstallPhase) {
      ...
      default:
        throw assertExhaustiveCheck(popupAddonInstallPhase, 'Unhandled PopupAddonInstallPhase');
  ...
  _renderEnablePopupInstructions() {
    return <InstructionTransition key={0}>
        <div className="homeInstructions" data-testid="home-enable-popup-instructions">
          <div className="homeInstructionsLeft">
            <div style={{ textAlign: 'center' }}>
              <img className="homeSectionScreenshot" src={PerfScreenshot} alt="screenshot of profiler.firefox.com" />
            </div>
          </div>
          <div className="homeInstructionsRight">
            <button type="button" className="homeSectionButton" onClick={this._enableMenuButton}>
            ...
            <ActionButtons // $FlowFixMe Error introduced by upgrading to v0.96.0. See issue #1936.
          retrieveProfileFromFile={this.props.retrieveProfileFromFile} triggerLoadingFromUrl={this.props.triggerLoadingFromUrl} />
          </div>
        </div>
  ...
  _renderInstallAddonInstructions() {
    return <InstructionTransition key={0}>
        <div className="homeInstructions" data-testid="home-install-addon-instructions">
          <div className="homeInstructionsLeft">
            <div style={{ textAlign: 'center' }}>
              <img className="homeSectionScreenshot" src={PerfScreenshot} alt="screenshot of profiler.firefox.com" />
            </div>
          ...
          <div className="homeInstructionsRight">
            <InstallButton name="Gecko Profiler" className="homeSectionButton" xpiUrl={ADDON_URL}>
              <span className="homeSectionPlus">+</span>
              Install add-on
            </InstallButton>
            <DocsButton />
  ...
  _renderRecordInstructions(screenshotSrc: string) {
    return <InstructionTransition key={1}>
        <div className="homeInstructions" data-testid="home-record-instructions">
          <div className="homeInstructionsLeft">
            <p>
              <img className="homeSectionScreenshot" src={screenshotSrc} alt="Screenshot of the profiler settings from the Firefox menu." />
            </p>
          </div>
          <div className="homeInstructionsRight">
            <DocsButton />
  ...
  _renderOtherBrowserInstructions() {
    return <InstructionTransition key={0}>
        <div className="homeInstructions" data-testid="home-other-browser-instructions">
          <div className="homeInstructionsLeft">
            <div style={{ textAlign: 'center' }}>
              <img className="homeSectionScreenshot" src={PerfScreenshot} alt="screenshot of profiler.firefox.com" />
            </div>
        ...
        <section className="homeSection">
          <header>
            <h1 className="homeTitle">
              ...
              <a className="homeTitleGithubIcon" href="https://github.com/firefox-devtools/profiler" target="_blank" rel="noopener noreferrer" title="Go to our git repository (this opens in a new window)">
                <svg width="22" height="22" className="octicon octicon-mark-github" viewBox="0 0 16 16" version="1.1" aria-label="github">
                  <path fillRule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
              </a>
            </h1>
            {specialMessage ? <div className="homeSpecialMessage">{specialMessage}</div> : null}
            <p>
