...
import { formatNanoseconds, formatPercent } from "../../../utils/format-numbers";

import { ProfilerOverhead } from "../../../types/profile";
...
export class MetaOverheadStatistics extends React.PureComponent<Props> {
  ...
  render() {
    ...
    const statKeys = ['Overhead', 'Cleaning', 'Counter', 'Interval', 'Lockings'];
    ...
    return calculatedStats.size > 0 ? <details>
        ...
        <div className="arrowPanelSection">
          <div className="metaInfoGrid">
            ...
            <div>Max</div>
            <div>Min</div>
            {[...calculatedStats].map(([key, val]) => <React.Fragment key={key}>
                <div>{key}</div>
          ...
          {overheadDurations !== 0 ? <div className="metaInfoRow">
