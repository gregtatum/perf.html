...
export class MenuButtonsMetaInfo extends React.PureComponent<Props> {
      ...
      case 'DONE':
        return <>
            <div className="metaInfoRow">
              ...
              {isSymbolicated ? 'Profile is symbolicated' : 'Profile is not symbolicated'}
            </div>
            <div className="metaInfoRow">
              <span className="metaInfoLabel"></span>
              <button onClick={resymbolicateProfile} type="button" className="photon-button photon-button-micro">
                {isSymbolicated ? 'Re-symbolicate profile' : 'Symbolicate profile'}
              </button>
            </div>
          </>;
      case 'SYMBOLICATING':
        return <div className="metaInfoRow">
            <span className="metaInfoLabel">Symbols:</span>
            {isSymbolicated ? 'Attempting to re-symbolicate profile' : 'Currently symbolicating profile'}
          </div>;
      default:
        throw assertExhaustiveCheck(symbolicationStatus, 'Unhandled SymbolicationStatus.');
  ...
  render() {
    ...
    return <ButtonWithPanel className="menuButtonsMetaInfoButton" buttonClassName="menuButtonsMetaInfoButtonButton" label={_formatLabel(meta) || 'Profile information'} panel={<ArrowPanel className="arrowPanelOpenMetaInfo">
            <h2 className="arrowPanelSubTitle">Profile Information</h2>
            <div className="arrowPanelSection">
              {meta.startTime ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Recording started:</span>
                  {_formatDate(meta.startTime)}
                </div> : null}
              {meta.interval ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Interval:</span>
                  {meta.interval}ms
                </div> : null}
              {meta.preprocessedProfileVersion ? <div className="metaInfoRow">
              ...
              {configuration ? <>
                  ...
                  <div className="metaInfoRow">
                    ...
                    {configuration.duration ? `${configuration.duration} seconds` : 'Unlimited'}
                  </div>
                  <div className="arrowPanelSection">
                    <div className="metaInfoRow">
            ...
            <div className="arrowPanelSection">
              {meta.product ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Name:</span>
                  {meta.product}
                </div> : null}
              {meta.misc ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Version:</span>
                  {_formatVersionNumber(meta.misc)}
                </div> : null}
              {meta.updateChannel ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Update Channel:</span>
                  {meta.updateChannel}
                </div> : null}
              {meta.appBuildID ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Build ID:</span>
                  {meta.sourceURL ? <a href={meta.sourceURL} title={meta.sourceURL} target="_blank" rel="noopener noreferrer">
                      {meta.appBuildID}
                    </a> : meta.appBuildID}
                </div> : null}
              {meta.debug !== undefined ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Build Type:</span>
                  {meta.debug ? 'Debug' : 'Opt'}
                </div> : null}
              {meta.extensions ? <div className="metaInfoRow">
            ...
            <div className="arrowPanelSection">
              {meta.platform ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">Platform:</span>
                  {meta.platform}
                </div> : null}
              {meta.oscpu ? <div className="metaInfoRow">
                  <span className="metaInfoLabel">OS:</span>
                  {meta.oscpu}
                </div> : null}
              {meta.abi ? <div className="metaInfoRow">
        ...
        */
            ...
            {profilerOverhead ? <MetaOverheadStatistics profilerOverhead={profilerOverhead} /> : null}
          </ArrowPanel>} />;
  }
}

function _mapInfoListItem(data: string[]): React.DOM {
  return data.map(d => <li className="metaInfoListItem" key={d}>
      {d}
    </li>);
}
