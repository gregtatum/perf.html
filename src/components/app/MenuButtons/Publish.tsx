...
import { toggleCheckedSharingOptions, attemptToPublish, abortUpload, resetUploadState } from "../../../actions/publish";
import { getProfile, getProfileRootRange, getHasPreferenceMarkers } from "../../../selectors/profile";
import { getCheckedSharingOptions, getFilenameString, getDownloadSize, getCompressedProfileBlob, getUploadPhase, getUploadProgress, getUploadProgressString, getUploadError, getShouldSanitizeByDefault } from "../../../selectors/publish";
...
import { UploadPhase } from "../../../types/state";
...
type StateProps = {
  ...
  readonly downloadSizePromise: Promise<string>;
  readonly compressedProfileBlobPromise: Promise<Blob>;
...
type DispatchProps = {
  ...
  readonly abortUpload: typeof abortUpload;
...
class MenuButtonsPublishImpl extends React.PureComponent<PublishProps> {
  ...
  _toggles: {
    ...
    includeHiddenThreads: () => this.props.toggleCheckedSharingOptions('includeHiddenThreads'),
    includeFullTimeRange: () => this.props.toggleCheckedSharingOptions('includeFullTimeRange'),
    ...
    includeExtension: () => this.props.toggleCheckedSharingOptions('includeExtension'),
    includePreferenceValues: () => this.props.toggleCheckedSharingOptions('includePreferenceValues')
  };
  ...
  _renderCheckbox(slug: $Keys<CheckedSharingOptions>, label: string) {
    ...
    return <label className="photon-label menuButtonsPublishDataChoicesLabel">
        <input type="checkbox" className="photon-checkbox photon-checkbox-default" name={slug} onChange={toggle} checked={checkedSharingOptions[slug]} />
        {label}
      </label>;
  }
  ...
  _renderPublishPanel() {
    ...
    return <div data-testid="MenuButtonsPublish-container">
        <form className="menuButtonsPublishContent" onSubmit={attemptToPublish}>
          ...
          <h1 className="menuButtonsPublishTitle">
            {isRepublish ? 'Re-publish Performance Profile' : 'Share Performance Profile'}
          </h1>
          <p className="menuButtonsPublishInfoDescription">
            Upload your profile and make it accessible to anyone with the link.
            {shouldSanitizeByDefault ? ' By default, your personal data is removed.' : ' This profile is from Firefox Nightly, so by default all information is included.'}
          </p>
          <h3>Include additional data that may be identifiable</h3>
          <div className="menuButtonsPublishDataChoices">
            {this._renderCheckbox('includeHiddenThreads', 'Include hidden threads')}
            {this._renderCheckbox('includeFullTimeRange', 'Include hidden time range')}
            {this._renderCheckbox('includeScreenshots', 'Include screenshots')}
            {this._renderCheckbox('includeUrls', 'Include resource URLs')}
            {this._renderCheckbox('includeExtension', 'Include extension information')}
            {shouldShowPreferenceOption ? this._renderCheckbox('includePreferenceValues', 'Include preference values') : null}
          </div>
          <div className="menuButtonsPublishButtons">
            <DownloadButton downloadFileName={downloadFileName} compressedProfileBlobPromise={compressedProfileBlobPromise} downloadSizePromise={downloadSizePromise} />
            <button type="submit" className="photon-button photon-button-primary menuButtonsPublishButton menuButtonsPublishButtonsUpload">
  ...
  _renderUploadPanel() {
    ...
    return <div className="menuButtonsPublishUpload" data-testid="MenuButtonsPublish-container">
        <div className="menuButtonsPublishUploadTop">
          ...
          <div className="menuButtonsPublishUploadBar">
            <div className="menuButtonsPublishUploadBarInner" style={{ width: `${uploadProgress * 100}%` }} />
          </div>
        </div>
        <div className="menuButtonsPublishButtons">
          <DownloadButton downloadFileName={downloadFileName} compressedProfileBlobPromise={compressedProfileBlobPromise} downloadSizePromise={downloadSizePromise} />
          <button type="button" className="photon-button photon-button-default menuButtonsPublishButton menuButtonsPublishButtonsCancelUpload" onClick={abortUpload}>
            Cancel Upload
          </button>
        </div>
      </div>;
  ...
  _renderErrorPanel() {
    ...
    let message: string = 'There was an unknown error when trying to publish the profile.';
    ...
    return <div className="menuButtonsPublishUpload" data-testid="MenuButtonsPublish-container">
        <div className="photon-message-bar photon-message-bar-error">
          ...
          <button className="photon-button photon-button-micro photon-message-bar-action-button" type="button" onClick={resetUploadState}>
            Try again
          </button>
        </div>
  ...
  render() {
    ...
    switch (uploadPhase) {
      ...
      case 'local':case 'uploaded':
        return this._renderPublishPanel();
      case 'uploading':case 'compressing':
...
export const MenuButtonsPublish = explicitConnect<OwnProps, StateProps, DispatchProps>({
 ...
 */
class DownloadSize extends React.PureComponent<DownloadSizeProps, DownloadSizeState> {
...
class DownloadButton extends React.PureComponent<DownloadButtonProps, DownloadButtonState> {
  ...
  static getDerivedStateFromProps(props: DownloadButtonProps, state: DownloadButtonState): $Shape<DownloadButtonState> | null {
  ...
  componentDidUpdate(prevProps: DownloadButtonProps) {
    if (prevProps.compressedProfileBlobPromise !== this.props.compressedProfileBlobPromise) {
      this._unwrapPromise();
    }
  }
  ...
  render() {
    ...
    const className = 'photon-button menuButtonsPublishButton menuButtonsPublishButtonsDownload';

    if (compressedProfileBlob) {
      return <BlobUrlLink blob={compressedProfileBlob} download={`${downloadFileName}.gz`} className={className}>
    ...
    return <button type="button" className={classNames(className, 'menuButtonsPublishButtonDisabled')}>
        Compressing…
      </button>;
  }
}