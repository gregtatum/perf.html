...
import { getProfile, getProfileRootRange, getSymbolicationStatus } from "../../../selectors/profile";
...
import { revertToPrePublishedState, abortUpload } from "../../../actions/publish";
...
import { UploadPhase, SymbolicationStatus } from "../../../types/state";
...
type OwnProps = {
  ...
  // instead.
  injectedUrlShortener?: (arg0: string) => Promise<string>;
};
...
type DispatchProps = {
  ...
  readonly resymbolicateProfile: typeof resymbolicateProfile;
};
...
class MenuButtons extends React.PureComponent<Props> {
  ...
  _renderPublishPanel() {
    ...
    const isUploading = uploadPhase === 'uploading' || uploadPhase === 'compressing';

    if (isUploading) {
      return <button type="button" className="buttonWithPanelButton menuButtonsAbortUploadButton" onClick={abortUpload}>
          Cancel Upload
        </button>;
    }
  ...
  _renderPermalink() {
    ...
    const showPermalink = dataSource === 'public' || dataSource === 'from-url' || dataSource === 'compare';

    return showPermalink ? <MenuButtonsPermalink isNewlyPublished={isNewlyPublished} injectedUrlShortener={injectedUrlShortener} /> : null;
  }

  _renderRevertProfile() {
    const {
      hasPrePublishedState,
      revertToPrePublishedState
    } = this.props;
    if (!hasPrePublishedState) {
      return null;
    }
    return <button type="button" className="buttonWithPanelButton menuButtonsRevertButton" onClick={revertToPrePublishedState}>
  ...
  render() {
    ...
    return <>
        ...
        <MenuButtonsMetaInfo profile={profile} symbolicationStatus={symbolicationStatus} resymbolicateProfile={resymbolicateProfile} />
        <div className="menuButtons">
          {this._renderRevertProfile()}
          {this._renderPublishPanel()}
          {this._renderPermalink()}
          <a href="/docs/" target="_blank" className="menuButtonsLink" title="Open the documentation in a new window">
            Docs
