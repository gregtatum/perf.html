...
class JsAllocationTree {
  _getMarker: MarkerIndex => Marker;
  _markerIndexes: MarkerIndex[];
  _zeroAt: Milliseconds;
  _displayDataByIndex: Map<MarkerIndex, JsAllocationDisplayData>;
  ...
  getDisplayData(markerIndex: MarkerIndex): JsAllocationDisplayData {
...
class MarkerTable extends PureComponent<Props> {
  ...
  _treeView: ?TreeView<JsAllocationDisplayData>;
  _takeTreeViewRef = treeView => (this._treeView = treeView);

  getMarkerTree = memoize((...args) => new JsAllocationTree(...args), {
    limit: 1,
  });
...
export default explicitConnect<{||}, StateProps, DispatchProps>({
  mapStateToProps: state => ({
    ...
    markerIndexes: selectedThreadSelectors.getSearchFilteredJsAllocationMarkerIndexes(
      state
    ),
