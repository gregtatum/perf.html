...
import { getCategories, getCommittedRange, getPreviewSelection, getScrollToSelectionGeneration, getProfileInterval, getPageList } from "../../selectors/profile";
import { selectedThreadSelectors } from "../../selectors/per-thread";
import { getSelectedThreadIndex, getInvertCallstack } from "../../selectors/url-state";
import ContextMenuTrigger from "../shared/ContextMenuTrigger";
import { getCallNodePathFromIndex } from "../../profile-logic/profile-data";
import { changeSelectedCallNode, changeRightClickedCallNode } from "../../actions/profile-view";

import { Thread, CategoryList, PageList } from "../../types/profile";
import { Milliseconds, StartEndRange } from "../../types/units";
import { FlameGraphTiming } from "../../profile-logic/flame-graph";
import { PreviewSelection, CallTreeSummaryStrategy } from "../../types/actions";
...
class FlameGraph extends React.PureComponent<Props> {
   ...
   */
  _nextSelectableInRow = (startingCallNodeIndex: IndexIntoCallNodeTable, direction: 1 | -1): IndexIntoCallNodeTable | void => {
    const {
      flameGraphTiming,
      callNodeInfo: {
        callNodeTable
  ...
  _handleKeyDown = (event: React.KeyboardEvent<HTMLElement>) => {
    ...
    if (selectedCallNodeIndex === null) {
      if (['ArrowDown', 'ArrowUp', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
        ...
        changeSelectedCallNode(threadIndex, getCallNodePathFromIndex(0, callNodeTable));
      }
      return;
    }
      ...
      case 'ArrowDown':
        {
          ...
          if (prefix !== -1) {
            changeSelectedCallNode(threadIndex, getCallNodePathFromIndex(prefix, callNodeTable));
          }
          break;
        }
      case 'ArrowUp':
        {
          ...
          if (callNodeIndex !== undefined && this._wideEnough(callNodeIndex)) {
            changeSelectedCallNode(threadIndex, getCallNodePathFromIndex(callNodeIndex, callNodeTable));
          }
          break;
        }
  ...
  render() {
    ...
    return <div className="flameGraphContent" onKeyDown={this._handleKeyDown}>
        <ContextMenuTrigger id="CallNodeContextMenu" attributes={{
        className: 'treeViewContextMenu'
      }}>
          <FlameGraphCanvas key={threadIndex} // ChartViewport props
        viewportProps={{
          timeRange,
          maxViewportHeight,
          maximumZoom: 1,
...
export default explicitConnect<{}, StateProps, DispatchProps>({
  mapStateToProps: state => ({
    ...
    maxStackDepth: selectedThreadSelectors.getCallNodeMaxDepthForFlameGraph(state),
