...
import { changeSelectedMarker, changeRightClickedMarker } from "../../actions/profile-view";
...
class MarkerTree {
  ...
  _getMarker: (arg0: MarkerIndex) => Marker;
  _markerIndexes: MarkerIndex[];
  _zeroAt: Milliseconds;
  _displayDataByIndex: Map<MarkerIndex, MarkerDisplayData>;

  constructor(getMarker: (arg0: MarkerIndex) => Marker, markerIndexes: MarkerIndex[], zeroAt: Milliseconds) {
...
function _formatDuration(duration: number): string {
  ...
  return duration.toLocaleString(undefined, {
...
class MarkerTable extends PureComponent<Props> {
  ...
  _fixedColumns = [{ propName: 'start', title: 'Start' }, { propName: 'duration', title: 'Duration' }, { propName: 'category', title: 'Category' }];
  _mainColumn = { propName: 'name', title: 'Description' };
  _expandedNodeIds: Array<MarkerIndex | null> = [];
  _onExpandedNodeIdsChange = () => {};
  _treeView: TreeView<MarkerDisplayData> | null | undefined;
  ...
  render() {
    ...
    return <div className="markerTable" id="marker-table-tab" role="tabpanel" aria-labelledby="marker-table-tab-button">
        <MarkerSettings />
        {markerIndexes.length === 0 ? <MarkerTableEmptyReasons /> : <TreeView maxNodeDepth={0} tree={tree} fixedColumns={this._fixedColumns} mainColumn={this._mainColumn} onSelectionChange={this._onSelectionChange} onRightClickSelection={this._onRightClickSelection} onExpandedNodesChange={this._onExpandedNodeIdsChange} selectedNodeId={selectedMarker} rightClickedNodeId={rightClickedMarkerIndex} expandedNodeIds={this._expandedNodeIds} ref={this._takeTreeViewRef} contextMenuId="MarkerContextMenu" rowHeight={16} indentWidth={10} />}
      </div>;
  }
}
...
export default explicitConnect<{}, StateProps, DispatchProps>({
  mapStateToProps: state => ({
    ...
    markerIndexes: selectedThreadSelectors.getPreviewFilteredMarkerIndexes(state),
