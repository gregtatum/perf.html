...
import type { JsTracerTable, ThreadIndex } from '../../types/profile';
...
const _seenChartKeys: Set<string> = new Set();
...
> {
  ...
  constructor(props) {
    super(props);
    if (!_seenChartKeys.has(props.keyString)) {
      ...
      this._doFadeIn = true;
      _seenChartKeys.add(props.keyString);
    }
  }
...
export default class JsTracerChart extends React.PureComponent<ChartProps> {
  render() {
    const { jsTracerTable, showJsTracerSummary, threadIndex } = this.props;
