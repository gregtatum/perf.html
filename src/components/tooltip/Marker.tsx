...
import { formatNumber, formatPercent, formatBytes, formatSI, formatMicroseconds, formatMilliseconds, formatValueTotal } from "../../utils/format-numbers";
...
import { Thread, ThreadIndex, PageList } from "../../types/profile";
import { PhaseTimes } from "../../types/markers";
import { ConnectedProps } from "../../utils/connect";

type PhaseTimeTuple = {name: string;time: Microseconds;};

function _markerDetailPhase(p: PhaseTimeTuple): any {
  return <TooltipDetail key={p.name} label={'Phase ' + p.name}>
      {formatMilliseconds(p.time / 1000)}
    </TooltipDetail>;
}

function _makePhaseTimesArray(phases: PhaseTimes<Microseconds>): Array<PhaseTimeTuple> {
  const array = [];
...
function _treeInsert(tree: Map<string, PhaseTreeNode>, path: Array<string>, phase: PhaseTimeTuple) {
  ...
  if (path.length > 0) {
    ...
    if (node.value) {
      console.error('Duplicate phases in _treeInsert in MarkerTooltipContents.js');
      return;
    }
    node.value = phase;
  }
...
function _treeGetLeaves(tree: Map<string, PhaseTreeNode>): Array<PhaseTimeTuple> {
  const leaves = [];
...
function _filterInterestingPhaseTimes(rawPhases: PhaseTimes<Microseconds>, numSelect: number): Array<PhaseTimeTuple> {
   ...
   * item.
   */
  const sortedPhaseTimes = phaseTimes.map(pt => pt.time) // Descending order
  .sort((a, b) => b - a);
...
function _sumMaybeEntries(entries: PhaseTimes<Microseconds>, selectEntries: Array<string>): Microseconds {
  return selectEntries.map(name => entries[name] ? entries[name] : 0).reduce((a, x) => a + x, 0);
}
...
const MaybeBacktrace = ({
  ...
  if (data && 'cause' in data && data.cause) {
    ...
    return <div className="tooltipDetailsBackTrace" key="backtrace">
        {data.type === 'Styles' || marker.name === 'Reflow' ? <h2 className="tooltipBackTraceTitle">
            First invalidated {formatNumber(causeAge)}ms before the flush, at:
          </h2> : null}
        <Backtrace maxHeight="30em" stackIndex={cause.stack} thread={thread} implementationFilter={implementationFilter} />
      </div>;
  }
  return null;
};

function getMarkerDetails(marker: Marker, thread: Thread, implementationFilter: ImplementationFilter, zeroAt: Milliseconds): React.ReactNode {
  ...
  if (data) {
    switch (data.type) {
              ...
              case 'complete':
                {
                  ...
                  // Milliseconds
                  const evictTimeMS = nursery.phase_times.CollectToFP ? _sumMaybeEntries(nursery.phase_times, ['TraceValues', 'TraceCells', 'TraceSlots', 'TraceWholeCells', 'TraceGenericEntries', 'MarkRuntime', 'MarkDebugger', 'CollectToFP']) : undefined;
                  tooltipDetails = <TooltipDetails>
                  <TooltipDetail label="Reason">{nursery.reason}</TooltipDetail>
                  <TooltipDetail label="Bytes evicted">
                    {formatValueTotal(nursery.bytes_tenured, nursery.bytes_used, formatBytes)}
                  </TooltipDetail>
                  {nursery.cells_tenured && nursery.cells_allocated_nursery ? <TooltipDetail label="Cells evicted">
                      {formatValueTotal(nursery.cells_tenured, nursery.cells_allocated_nursery, formatSI)}
                    </TooltipDetail> : null}
                  {nursery.cur_capacity ? <TooltipDetail label="Bytes used">
                      {formatValueTotal(nursery.bytes_used, nursery.cur_capacity, formatBytes)}
                    </TooltipDetail> : null}
                  {nursery.new_capacity ? <TooltipDetail label="New nursery size">
                      {formatBytes(nursery.new_capacity)}
                    </TooltipDetail> : null}
                  {nursery.lazy_capacity ? <TooltipDetail label="Lazy-allocated size">
                      {formatBytes(nursery.lazy_capacity)}
                    </TooltipDetail> : null}
                  {nursery.cells_allocated_nursery && nursery.cells_allocated_tenured ? <TooltipDetail label="Nursery allocations since last minor GC">
                      {formatValueTotal(nursery.cells_allocated_nursery, nursery.cells_allocated_nursery + nursery.cells_allocated_tenured, formatSI)}
                    </TooltipDetail> : null}
                  {evictTimeMS ? <TooltipDetail label="Tenuring allocation rate">
                      {formatBytes( // evictTimeMS is in milliseconds.
                      nursery.bytes_tenured / (evictTimeMS / 1000000)) + '/s'}
                    </TooltipDetail> : null}
                  {evictTimeMS && nursery.cells_tenured ? <TooltipDetail label="Tenuring allocation rate">
                      {formatSI(nursery.cells_tenured / (evictTimeMS / 10000000)) + '/s'}
                    </TooltipDetail> : null}
                  {nursery.chunk_alloc_us ? <TooltipDetail label="Time spent allocating chunks in mutator">
                      {formatMicroseconds(nursery.chunk_alloc_us)}
                    </TooltipDetail> : null}
                  {nursery.groups_pretenured ? <TooltipDetail label="Number of groups to pretenure">
            ...
            case 'completed':
              {
                ...
                if (timings.nonincremental_reason && timings.nonincremental_reason !== 'None') {
                  nonIncrementalReason = <TooltipDetail label="Non-incremental reason">
                ...
                if (post_heap_size !== undefined) {
                  gcsize = <TooltipDetail label="Heap size (pre - post)">
                  {formatBytes(timings.allocated_bytes) + ' - ' + formatBytes(post_heap_size)}
                </TooltipDetail>;
                } else {
                  gcsize = <TooltipDetail label="Heap size (pre)">
                ...
                <TooltipDetail label="Total slice times">
                  {formatMilliseconds(timings.total_time,
                    /* significantDigits */
                    3,
                    /* maxFractionalDigits */
                    2)}
                ...
                <TooltipDetail label="Max Pause">
                  {formatMilliseconds(timings.max_pause,
                    /* significantDigits */
                    3,
                    /* maxFractionalDigits */
                    2)}
                ...
                <TooltipDetail label="Zones">
                  {formatValueTotal(timings.zones_collected, timings.total_zones, String)}
                </TooltipDetail>
      ...
      case 'GCSlice':
        {
          ...
          if (timings.trigger_amount && timings.trigger_threshold) {
            triggers = <TooltipDetail label="Trigger (amt/trig)">
              {formatValueTotal(timings.trigger_amount, timings.trigger_threshold, formatBytes, false
              /* includePercent */
              )}
            </TooltipDetail>;
          }
      ...
      case 'Network':
        {
          tooltipDetails = <TooltipNetworkMarker payload={data} zeroAt={zeroAt} />;
          break;
        }
      case 'Styles':
        {
      ...
      case 'tracing':
        {
          switch (data.category) {
            case 'DOMEvent':
              {
                const latency = data.timeStamp === undefined ? null : formatMilliseconds(marker.start - data.timeStamp);
                tooltipDetails = <TooltipDetails>
  ...
  return <>
      {tooltipDetails}
      <MaybeBacktrace marker={marker} thread={thread} implementationFilter={implementationFilter} />
    </>;
}
...
class MarkerTooltipContents extends React.PureComponent<Props> {
  ...
  render() {
    ...
    const details = getMarkerDetails(marker, thread, implementationFilter, zeroAt);
    return <div className={classNames('tooltipMarker', className)}>
        <div className={classNames({ tooltipHeader: details })}>
          <div className="tooltipOneLine">
            <div className="tooltipTiming">
              ...
              {!marker.incomplete ? marker.dur ? formatMilliseconds(marker.dur) : '—' : 'unknown duration'}
            </div>
            <div className="tooltipTitle">{marker.title || marker.name}</div>
          </div>
          {threadName || url ? <TooltipDetails>
