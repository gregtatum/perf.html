...
class OverflowEdgeIndicator extends React.PureComponent<Props, State> {
  ...
  componentDidUpdate() {
    this._updateIndicatorStatus();
    if (!this._scrolledToInitialSelected && this.props.initialSelected && this._container) {
  ...
  render() {
    ...
    return <div className={classNames('overflowEdgeIndicator', className, this.state)}>
        ...
        <div className={classNames('overflowEdgeIndicatorScrollbox', `${className}Scrollbox`)} onScroll={this._onScroll} ref={this._takeContainerRef}>
          <div className="overflowEdgeIndicatorContentsWrapper" ref={this._takeContainerWrapperRef}>
            {children}
          </div>
        </div>
      </div>;
