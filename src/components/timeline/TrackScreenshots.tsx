...
import { ThreadIndex, Thread } from "../../types/profile";
...
class Screenshots extends PureComponent<Props, State> {
  ...
  render() {
    ...
    return <div className="timelineTrackScreenshot" style={{ height: TRACK_SCREENSHOT_HEIGHT }} onMouseLeave={this._handleMouseLeave} onMouseMove={this._handleMouseMove}>
        <ScreenshotStrip thread={thread} width={width} rangeStart={rangeStart} rangeEnd={rangeEnd} screenshots={screenshots} />
        <HoverPreview screenshots={screenshots} thread={thread} isMakingPreviewSelection={isMakingPreviewSelection} width={width} pageX={pageX} offsetX={offsetX} containerTop={containerTop} rangeEnd={rangeEnd} rangeStart={rangeStart} />
      </div>;
  }
}
...
export default explicitConnect<OwnProps, StateProps, DispatchProps>({
  mapStateToProps: (state, ownProps) => {
    ...
    return {
      ...
      screenshots: selectors.getRangeFilteredScreenshotsById(state).get(windowId) || EMPTY_SCREENSHOTS_TRACK,
      threadName: selectors.getFriendlyThreadName(state),
      rangeStart: start,
      rangeEnd: end,
      isMakingPreviewSelection: previewSelection.hasSelection && previewSelection.isModifying
    };
  },
...
class HoverPreview extends PureComponent<HoverPreviewProps> {
  ...
  _overlayElement = ensureExists(document.querySelector('#root-overlay'), 'Expected to find a root overlay element.');
  ...
  render() {
    ...
    const payload: ScreenshotPayload = (screenshots[screenshotIndex].data as any);
    const {
      url,
      windowWidth,
      windowHeight
    ...
    const coefficient = windowHeight > windowWidth ? MAXIMUM_HOVER_SIZE / windowHeight : MAXIMUM_HOVER_SIZE / windowWidth;
    let hoverHeight = windowHeight * coefficient;
    let hoverWidth = windowWidth * coefficient;

    const distanceToTopFromTrackCenter = TRACK_SCREENSHOT_HEIGHT / 2 + containerTop;
    ...
    return createPortal(<div className="timelineTrackScreenshotHover" style={{ left, top }}>
        <img className="timelineTrackScreenshotHoverImg" src={thread.stringTable.getString(url)} style={{
        height: hoverHeight,
        width: hoverWidth
      }} />
      </div>, this._overlayElement);
  }
}
...
class ScreenshotStrip extends PureComponent<ScreenshotStripProps> {
  ...
  render() {
    ...
    const timeToPixel = time => outerContainerWidth * (time - rangeStart) / rangeLength;

    const leftmostPixel = Math.max(timeToPixel(screenshots[0].start), 0);
    let screenshotIndex = 0;
    for (let left = leftmostPixel; left < outerContainerWidth; left += imageContainerWidth) {
      ...
      const scaledImageWidth = TRACK_SCREENSHOT_HEIGHT * windowWidth / windowHeight;
      images.push(<div className="timelineTrackScreenshotImgContainer" style={{ left, width: imageContainerWidth }} key={left}>
          {
          /* The following image is centered and cropped by the outer container. */
        }
          <img className="timelineTrackScreenshotImg" src={thread.stringTable.getString(urlStringIndex)} style={{
