...
class EmptyThreadIndicator extends PureComponent<Props> {
  ...
  render() {
    ...
    return <div className="timelineEmptyThreadIndicator">
        {style.startup ? <DivWithTooltip style={style.startup} className="timelineEmptyThreadIndicatorBlock timelineEmptyThreadIndicatorStartup" tooltip="This thread hasn't started yet" /> : null}
        {style.shutdown ? <DivWithTooltip style={style.shutdown} className="timelineEmptyThreadIndicatorBlock timelineEmptyThreadIndicatorShutdown" tooltip="This thread has shut down here" /> : null}
        {style.emptyBufferStart ? <DivWithTooltip style={style.emptyBufferStart} className="timelineEmptyThreadIndicatorBlock timelineEmptyThreadIndicatorEmptyBuffer" tooltip={<span className="timelineEmptyThreadIndicatorLongTooltip">
                {oneLine`
...
export function getIndicatorPositions(props: Props): {
  ...
  const threadEndTime = unregisterTime === null ? processShutdownTime : unregisterTime;
  ...
  if ( // Threads could have no samples, and therefore no range.
  unfilteredSamplesRange !== null && // Was the buffer empty at the beginning of the range, at least one interval length
  // into the profile? This interval length ensures no awkward cut off where it's not
  // really needed.
  unfilteredSamplesRange.start >= rangeStart + interval && // Only show this if it's actually in the current range.
  registerTime < rangeEnd) {
    const startMilliseconds = Math.max(0, registerTime - rangeStart);
    emptyBufferStart = {
      left: startMilliseconds * xPixelsPerMs,
      width: Math.min(width, (unfilteredSamplesRange.start - Math.max(registerTime, rangeStart)) * xPixelsPerMs)
    };
  }
