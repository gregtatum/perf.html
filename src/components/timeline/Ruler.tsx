...
class TimelineRuler extends PureComponent<Props> {
  ...
  _getNotches() {
    ...
    const {
      number: notchTime,
      exponent
    } = this._findNiceNumberGreaterOrEqualTo(minimumNotchWidth / pixelsPerMilliSecond);
  ...
  render() {
    ...
    return <div className="timelineRuler" style={{ '--timeline-ruler-height': `${TIMELINE_RULER_HEIGHT}px` }}>
        <ol className="timelineRulerContainer">
          {notches.map(({
          time,
          pos
        }, i) => <li className="timelineRulerNotch" key={i} style={{ left: `${pos}px` }}>
              <span className="timelineRulerNotchText">{`${time.toFixed(decimalPlaces)}s`}</span>
            </li>)}
        </ol>
      </div>;
  }
