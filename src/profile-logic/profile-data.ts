...
import { resourceTypes, getEmptyUnbalancedNativeAllocationsTable, getEmptyBalancedNativeAllocationsTable } from "./data-structures";

import { Profile, Thread, SamplesTable, StackTable, FrameTable, FuncTable, ResourceTable, CategoryList, IndexIntoCategoryList, IndexIntoSubcategoryListForCategory, IndexIntoFuncTable, IndexIntoSamplesTable, IndexIntoStackTable, IndexIntoResourceTable, ThreadIndex, Category, Counter, CounterSamplesTable, NativeAllocationsTable, InnerWindowID, BalancedNativeAllocationsTable, IndexIntoFrameTable, PageList } from "../types/profile";
import { CallNodeInfo, CallNodeTable, CallNodePath, IndexIntoCallNodeTable, AccumulatedCounterSamples, SelectedState, ProfileFilterPageData } from "../types/profile-derived";
 ...
 */
export function getCallNodeInfo(stackTable: StackTable, frameTable: FrameTable, funcTable: FuncTable, defaultCategory: IndexIntoCategoryList): CallNodeInfo {
  return timeCode('getCallNodeInfo', () => {
    ...
    function addCallNode(prefixIndex: IndexIntoCallNodeTable, funcIndex: IndexIntoFuncTable, categoryIndex: IndexIntoCategoryList, subcategoryIndex: IndexIntoSubcategoryListForCategory, windowID: InnerWindowID) {
      const index = length++;
    ...
    for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
      ...
      let callNodeIndex = prefixCallNodeAndFuncToCallNodeMap.get(prefixCallNodeAndFuncIndex);
 ...
 */
export function getSampleIndexToCallNodeIndex(stacks: Array<IndexIntoStackTable | null>, stackIndexToCallNodeIndex: {
 ...
 */
export function getSamplesSelectedStates(callNodeTable: CallNodeTable, sampleCallNodes: Array<IndexIntoCallNodeTable | null>, activeTabFilteredCallNodes: Array<IndexIntoCallNodeTable | null>, selectedCallNodeIndex: IndexIntoCallNodeTable | null): SelectedState[] {
  ...
  const selectedCallNodeDepth = selectedCallNodeIndex === -1 || selectedCallNodeIndex === null ? 0 : callNodeTable.depth[selectedCallNodeIndex];

  const selectedCallNodeAtDepth: IndexIntoCallNodeTable[] = new Array(selectedCallNodeDepth);

  for (let callNodeIndex = selectedCallNodeIndex, depth = selectedCallNodeDepth; depth >= 0 && callNodeIndex !== null; depth--, callNodeIndex = callNodeTable.prefix[callNodeIndex]) {
   ...
   */
  function getSelectedStateFromCallNode(callNode: IndexIntoCallNodeTable | null, activeTabFilteredCallNode: IndexIntoCallNodeTable | null): SelectedState {
    ...
    while (true) {
      ...
      callNodeIndex = callNodeTable.prefix[callNodeIndex];
      depth--;
      if (callNodeIndex === -1 || callNodeIndex === selectedCallNodeAtDepth[depth]) {
        ...
        return prevCallNodeIndex <= selectedCallNodeAtDepth[depth + 1] ? 'UNSELECTED_ORDERED_BEFORE_SELECTED' : 'UNSELECTED_ORDERED_AFTER_SELECTED';
      }
    }
 ...
 */
export function getJsImplementationForStack(stackIndex: IndexIntoStackTable, {
  stackTable,
  frameTable,
  stringTable
}: Thread): JsImplementation {
  const frameIndex = stackTable.frame[stackIndex];
 ...
 */
export function getTimingsForPath(needlePath: CallNodePath, callNodeInfo: CallNodeInfo, interval: Milliseconds, isInvertedTree: boolean, thread: Thread, unfilteredThread: Thread, sampleIndexOffset: number, categories: CategoryList) {
  return getTimingsForCallNodeIndex(getCallNodeIndexFromPath(needlePath, callNodeInfo.callNodeTable), callNodeInfo, interval, isInvertedTree, thread, unfilteredThread, sampleIndexOffset, categories);
}
 ...
 */
export function getTimingsForCallNodeIndex(needleNodeIndex: IndexIntoCallNodeTable | null, {
  callNodeTable,
  stackIndexToCallNodeIndex
}: CallNodeInfo, interval: Milliseconds, isInvertedTree: boolean, thread: Thread, unfilteredThread: Thread, sampleIndexOffset: number, categories: CategoryList): TimingsForPath {
  const {
    samples,
    stackTable,
    funcTable
   ...
   */
  function accumulateDataToTimings(timings: {
    breakdownByImplementation: BreakdownByImplementation | null;
    breakdownByCategory: BreakdownByCategory | null;
    value: number;
  }, sampleIndex: IndexIntoSamplesTable, stackIndex: IndexIntoStackTable, funcIndex: IndexIntoFuncTable, duration: Milliseconds): void {
    // Step 1: increment the total value
    timings.value += duration;

    // Step 2: find the implementation value for this stack
    const implementation = funcTable.isJS[funcIndex] ? getJsImplementationForStack(stackIndex, thread) : 'native';
    ...
    const unfilteredStackIndex = unfilteredSamples.stack[sampleIndex + sampleIndexOffset];
    if (unfilteredStackIndex !== null) {
      const categoryIndex = unfilteredStackTable.category[unfilteredStackIndex];
      const subcategoryIndex = unfilteredStackTable.subcategory[unfilteredStackIndex];
  ...
  for (let sampleIndex = 0; sampleIndex < samples.length; sampleIndex++) {
    ...
    if (!isInvertedTree) {
      ...
      if (thisNodeIndex === needleNodeIndex) {
        accumulateDataToTimings(pathTimings.selfTime, sampleIndex, thisStackIndex, thisFunc, duration);
      }

      if (thisFunc === needleFuncIndex) {
        accumulateDataToTimings(funcTimings.selfTime, sampleIndex, thisStackIndex, thisFunc, duration);
      }
    }
    ...
    let pathFound = false;
    let nextStackIndex;
    for (let currentStackIndex = thisStackIndex; currentStackIndex !== null; currentStackIndex = nextStackIndex) {
      ...
      if (currentNodeIndex === needleNodeIndex) {
        ...
        if (!isInvertedTree) {
          accumulateDataToTimings(pathTimings.totalTime, sampleIndex, thisStackIndex, thisFunc, duration);
        }
      ...
      if (!funcFound && currentFuncIndex === needleFuncIndex) {
        ...
        if (!isInvertedTree) {
          accumulateDataToTimings(funcTimings.totalTime, sampleIndex, thisStackIndex, thisFunc, duration);
        }
        funcFound = true;
      }
      ...
      if (isInvertedTree && nextStackIndex === null) {
        ...
        if (currentFuncIndex === needleFuncIndex) {
          ...
          accumulateDataToTimings(funcTimings.selfTime, sampleIndex, currentStackIndex, currentFuncIndex, duration);
        }

        if (pathFound) {
          // We contribute the implementation information if the passed path was
          // found in this stack earlier.
          accumulateDataToTimings(pathTimings.totalTime, sampleIndex, currentStackIndex, currentFuncIndex, duration);
        }

        if (funcFound) {
          // We contribute the implementation information if the leaf function
          // of the passed path was found in this stack earlier.
          accumulateDataToTimings(funcTimings.totalTime, sampleIndex, currentStackIndex, currentFuncIndex, duration);
        }
      }
    }
  }
...
function _getTimeRangeForThread({
  ...
  jsAllocations,
  nativeAllocations
}: Thread, interval: Milliseconds): StartEndRange {
  ...
  if (samples.length) {
    ...
    for (let i = 0; i < markers.length; i++) {
      const thisStartTime = markers.data[i] && typeof markers.data[i].startTime === 'number' ? markers.data[i].startTime : markers.time[i];
      ...
      // otherwise.
      const thisEndTime = markers.data[i] && typeof markers.data[i].endTime === 'number' ? markers.data[i].endTime + interval : markers.time[i] + interval;
...
export function getTimeRangeIncludingAllThreads(profile: Profile): StartEndRange {
  ...
  profile.threads.forEach(thread => {
    const threadRange = memoizedGetTimeRangeForThread(thread, profile.meta.interval);
...
export function toValidImplementationFilter(implementation: string): ImplementationFilter {
...
export function toValidCallTreeSummaryStrategy(strategy: unknown): CallTreeSummaryStrategy {
  switch (strategy) {
    case 'timing':case 'js-allocations':case 'native-retained-allocations':case 'native-allocations':case 'native-deallocations-sites':case 'native-deallocations-memory':
      return strategy;
    default:
...
export function filterThreadByImplementation(thread: Thread, implementation: string, defaultCategory: IndexIntoCategoryList): Thread {
  const {
    funcTable,
    stringTable
  } = thread;
...
function _filterThreadByFunc(thread: Thread, filter: (arg0: IndexIntoFuncTable) => boolean, defaultCategory: IndexIntoCallNodeTable): Thread {
      ...
      if (newStack === undefined) {
        ...
        if (filter(funcIndex)) {
          ...
          if (newStack === undefined) {
            ...
            newStackTable.subcategory[newStack] = stackTable.subcategory[stackIndex];
          ...
          } else if (newStackTable.subcategory[stackIndex] !== stackTable.subcategory[stackIndex]) {
            // Conflicting origin stack subcategories -> "Other" subcategory.
...
export function filterThreadToSearchStrings(thread: Thread, searchStrings: string[] | null): Thread {
...
export function filterThreadToSearchString(thread: Thread, searchString: string): Thread {
  ...
  return updateThreadStacks(thread, stackTable, stackIndex => stackMatchesFilter(stackIndex) ? stackIndex : null);
}
 ...
 */
export function filterThreadByTab(thread: Thread, relevantPages: Set<InnerWindowID>): Thread {
  return timeCode('filterThreadByTab', () => {
    ...
    const frameMatchesFilterCache: Map<IndexIntoFrameTable, boolean> = new Map();
    function frameMatchesFilter(frame) {
      ...
      const matches = innerWindowID && innerWindowID > 0 ? relevantPages.has(innerWindowID) : false;
      frameMatchesFilterCache.set(frame, matches);
      return matches;
    }

    // Use the stackTable to navigate to frameTable and cache the result of it.
    const stackMatchesFilterCache: Map<IndexIntoStackTable, boolean> = new Map();
    function stackMatchesFilter(stackIndex) {
      if (stackIndex === null) {
        return false;
      }
 ...
 */
export function getSampleIndexRangeForSelection(table: {time: Milliseconds[];length: number;}, rangeStart: number, rangeEnd: number): [IndexIntoSamplesTable, IndexIntoSamplesTable] {
  ...
  const afterLastSample = table.time.slice(firstSample).findIndex(t => t >= rangeEnd);
...
export function filterThreadSamplesToRange(thread: Thread, rangeStart: number, rangeEnd: number): Thread {
  const {
    samples,
    jsAllocations,
    nativeAllocations
  } = thread;
  const [beginSampleIndex, endSampleIndex] = getSampleIndexRangeForSelection(samples, rangeStart, rangeEnd);
  const newSamples: SamplesTable = {
    ...
    duration: samples.duration ? samples.duration.slice(beginSampleIndex, endSampleIndex) : undefined,
  ...
  if (jsAllocations) {
    const [startAllocIndex, endAllocIndex] = getSampleIndexRangeForSelection(jsAllocations, rangeStart, rangeEnd);
  ...
  if (nativeAllocations) {
    const [startAllocIndex, endAllocIndex] = getSampleIndexRangeForSelection(nativeAllocations, rangeStart, rangeEnd);
    const time = nativeAllocations.time.slice(startAllocIndex, endAllocIndex);
    const duration = nativeAllocations.duration.slice(startAllocIndex, endAllocIndex);
...
export function filterCounterToRange(counter: Counter, rangeStart: number, rangeEnd: number): Counter {
  const filteredGroups = counter.sampleGroups.map(sampleGroup => {
    const samples = sampleGroup.samples;
    let [sBegin, sEnd] = getSampleIndexRangeForSelection(samples, rangeStart, rangeEnd);
 ...
 */
export function accumulateCounterSamples(samplesArray: Array<CounterSamplesTable>): Array<AccumulatedCounterSamples> {
...
export function getCallNodeIndicesFromPaths(callNodePaths: CallNodePath[], callNodeTable: CallNodeTable): Array<IndexIntoCallNodeTable | null> {
  ...
  const cache = new Map();
  return callNodePaths.map(path => _getCallNodeIndexFromPathWithCache(path, callNodeTable, cache));
}

// Returns a CallNodeIndex from a CallNodePath, using and contributing to the
// cache parameter.
function _getCallNodeIndexFromPathWithCache(callNodePath: CallNodePath, callNodeTable: CallNodeTable, cache: Map<string, IndexIntoCallNodeTable>): IndexIntoCallNodeTable | null {
  ...
  while (i < callNodePath.length) {
    ...
    const nextNodeIndex = _getCallNodeIndexFromParentAndFunc(index, func, callNodeTable);
...
function _getCallNodeIndexFromParentAndFunc(parent: IndexIntoCallNodeTable, func: IndexIntoFuncTable, callNodeTable: CallNodeTable): IndexIntoCallNodeTable | null {
  ...
  for (let callNodeIndex = parent + 1; // the root parent is -1
  callNodeIndex < callNodeTable.length; callNodeIndex++) {
    if (callNodeTable.prefix[callNodeIndex] === parent && callNodeTable.func[callNodeIndex] === func) {
...
export function getCallNodeIndexFromPath(callNodePath: CallNodePath, callNodeTable: CallNodeTable): IndexIntoCallNodeTable | null {
...
export function getCallNodePathFromIndex(callNodeIndex: IndexIntoCallNodeTable | null, callNodeTable: CallNodeTable): CallNodePath {
 ...
 */
export function convertStackToCallNodePath(thread: Thread, stack: IndexIntoStackTable): CallNodePath {
  const {
    stackTable,
    frameTable
  } = thread;
  const path = [];
  for (let stackIndex = stack; stackIndex !== null; stackIndex = stackTable.prefix[stackIndex]) {
 ...
 */
export function computeCallNodeMaxDepth(thread: Thread, callNodeInfo: CallNodeInfo): number {
  let maxDepth = 0;
  const {
    samples
  } = thread;
...
export function invertCallstack(thread: Thread, defaultCategory: IndexIntoCategoryList): Thread {
 ...
 */
export function updateThreadStacks(thread: Thread, newStackTable: StackTable, convertStack: (arg0: IndexIntoStackTable | null) => IndexIntoStackTable | null): Thread {
  const {
    jsAllocations,
    nativeAllocations,
    samples
 ...
 */
export function getMapStackUpdater(oldStackToNewStack: Map<null | IndexIntoStackTable, null | IndexIntoStackTable>): (arg0: IndexIntoStackTable | null) => IndexIntoStackTable | null {
...
export function getSampleIndexClosestToTime(samples: SamplesTable, time: number, interval: Milliseconds): IndexIntoSamplesTable {
  ...
  const distanceToLast = time - (samples.time[previousIndex] + previousDuration / 2);
 ...
 */
export function getOriginAnnotationForFunc(funcIndex: IndexIntoFuncTable, funcTable: FuncTable, resourceTable: ResourceTable, stringTable: UniqueStringArray): string {
 ...
 */
export function getFuncNamesAndOriginsForPath(path: CallNodePath, thread: Thread): Array<{funcName: string;origin: string;}> {
  const {
    funcTable,
    stringTable,
    resourceTable
 ...
 */
export function getTreeOrderComparator(callNodeTable: CallNodeTable, sampleCallNodes: Array<IndexIntoCallNodeTable | null>): (arg0: IndexIntoSamplesTable, arg1: IndexIntoSamplesTable) => number {
  /**
   * Determine the ordering of two non-null call nodes.
   */
  function compareCallNodes(callNodeA: IndexIntoCallNodeTable, callNodeB: IndexIntoCallNodeTable): number {
   ...
   */
  return function treeOrderComparator(sampleA: IndexIntoSamplesTable, sampleB: IndexIntoSamplesTable): number {
 ...
 */
export function findBestAncestorCallNode(callNodeInfo: CallNodeInfo, sampleCallNodes: Array<IndexIntoCallNodeTable | null>, sampleCategories: Array<IndexIntoCategoryList | null>, clickedCallNode: IndexIntoCallNodeTable, clickedCategory: IndexIntoCategoryList): IndexIntoCallNodeTable {
  const {
    callNodeTable
  } = callNodeInfo;
  ...
  function limitSameCategoryPathToCommonAncestor(callNode) {
    ...
    const walkUpToDepth = clickedDepth - (callNodesOnSameCategoryPath.length - 1);
  ...
  for (let sample = 0; sample < sampleCallNodes.length; sample++) {
    if (sampleCategories[sample] !== clickedCategory && sampleCallNodes[sample] !== null) {
 ...
 */
export function getSampleCategories(samples: SamplesTable, stackTable: StackTable): Array<IndexIntoSamplesTable | null> {
  return samples.stack.map(s => s !== null ? stackTable.category[s] : null);
}

export function getFriendlyStackTypeName(implementation: StackImplementation): string {
  switch (implementation) {
    case 'ion':case 'baseline':
...
export function shouldDisplaySubcategoryInfoForCategory(category: Category): boolean {
...
export function getCategoryPairLabel(categories: CategoryList, categoryIndex: number, subcategoryIndex: number): string {
  const category = categories[categoryIndex];
  return subcategoryIndex !== 0 ? `${category.name}: ${category.subcategories[subcategoryIndex]}` : `${category.name}`;
}
 ...
 */
export function filterToAllocations(nativeAllocations: NativeAllocationsTable): NativeAllocationsTable {
 ...
 */
export function filterToDeallocationsSites(nativeAllocations: NativeAllocationsTable): NativeAllocationsTable {
  let newNativeAllocations;
  if (nativeAllocations.memoryAddress) {
    newNativeAllocations = getEmptyBalancedNativeAllocationsTable();
    for (let i = 0; i < nativeAllocations.length; i++) {
 ...
 */
export function filterToDeallocationsMemory(nativeAllocations: BalancedNativeAllocationsTable): NativeAllocationsTable {
  // A-----D------A-------D
  type Address = number;
  type IndexIntoAllocations = number;
  const memoryAddressToAllocation: Map<Address, IndexIntoAllocations> = new Map();
  const stackOfOriginalAllocation: Array<IndexIntoAllocations | null> = [];
  for (let allocationIndex = 0; allocationIndex < nativeAllocations.length; allocationIndex++) {
    ...
    if (bytes >= 0) {
      ...
      const previousAllocationIndex = memoryAddressToAllocation.get(memoryAddress);
 ...
 */
export function filterToRetainedAllocations(nativeAllocations: BalancedNativeAllocationsTable): NativeAllocationsTable {
  // A-----D------A-------D
  type Address = number;
  type IndexIntoAllocations = number;
  const memoryAddressToAllocation: Map<Address, IndexIntoAllocations> = new Map();
 ...
 */
export function extractProfileFilterPageData(pages: PageList | null, relevantPages: Set<InnerWindowID>): ProfileFilterPageData | null {
  ...
  const filteredPages = pages.filter(page => page.innerWindowID === innerWindowID);
...
export function getOrCreateURIResource(scriptURI: string, resourceTable: ResourceTable, stringTable: UniqueStringArray, originToResourceIndex: Map<string, IndexIntoResourceTable>): IndexIntoResourceTable {
  // Figure out the origin and host.
  let origin;
  let host;
  try {
    const url = new URL(scriptURI);
    if (!(url.protocol === 'http:' || url.protocol === 'https:' || url.protocol === 'moz-extension:')) {
