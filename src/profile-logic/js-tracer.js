...
export function getJsTracerTiming(
  ...
  for (
    ...
    const stringIndex = jsTracer.events[currentEventIndex];
    ...
    for (let i = 0; i <= jsTracerTiming.length; i++) {
      ...
      const start = jsTracer.timestamps[currentEventIndex] / 1000;
      ...
      if (otherEnd === undefined || otherEnd <= start) {
        ...
        timingRow.index.push(currentEventIndex);
        timingRow.length++;
        break;
      }
    }
...
export function convertJsTracerToThread(
  ...
  const prefixesEventIndexes = [null];
  const prefixesEnds = [0];
  ...
  for (
    ...
    const start = jsTracer.timestamps[currentEventIndex];
    const durationRaw = jsTracer.durations[currentEventIndex];
    const duration = durationRaw === null ? 0 : durationRaw;
    const end = start + duration;

    // Try to find the current prefix.
    let prefixIndex = prefixesEventIndexes[prefixesTip];
    while (prefixIndex !== null) {
      const otherEnd = prefixesEnds[prefixesTip];
      if (end <= otherEnd) {
        break;
      }
      // Keep on searching for the next prefix.
      prefixIndex = prefixesEventIndexes[prefixesTip];
      prefixesTip--;
    }
    ...
    prefixesEnds[prefixesTip] = end;
  }

  return thread;
}

export function convertJsTracerToThread2(
  fromThread: Thread,
  jsTracer: JsTracerTable,
  categories: CategoryList
): Thread {
  ...
  const prefixesStarts: Microseconds[] = [-Infinity];
  const prefixesEnds: Microseconds[] = [-Infinity];
  const prefixesEventIndexes: Array<null | IndexIntoJsTracerEvents> = [null];
  const prefixesStackIndexes: Array<null | IndexIntoStackTable> = [null];
  ...
  for (
    ...
    console.log(
      `!!! currentEventIndex ${currentEventIndex} of ${jsTracer.length - 1}`
    );
    ...
    let end;
    {
      ...
      // Build out the frame table.
      const frameIndex = frameTable.length++;
      frameTable.address.push(blankStringIndex);
      frameTable.category.push(otherCategory);
      frameTable.func.push(funcIndex);
      // TODO - Implementations could feasibly be added here.
      frameTable.implementation.push(null);
      frameTable.line.push(null);
      frameTable.column.push(null);
      frameTable.optimizations.push(null);
      ...
      // Try to find the current prefix.
      let prefixIndex = prefixesEventIndexes[prefixesTip];
      console.log('!!! considering prefixIndex', prefixIndex);
      while (prefixIndex !== null) {
        if (i++ > 10) throw new Error('i > 10');
        const otherEnd = prefixesEnds[prefixesTip];
        if (end <= otherEnd) {
    ...
    while (prefixesTip !== 0) {
      ...
      if (prefixEnd <= currentStart) {
        ...
        addSelfTimeAsASample(stackIndex, prefixStart, prefixEnd);
        ...
        if (prefixesTip === 0) {
          ...
          prefixesStackIndexes[prefixesTip] = stackIndex;
          break;
        }
        ...
        addSelfTimeAsASample(stackIndex, prefixStart, currentStart);
        ...
        console.log(
          `!!! jsTracer.events[prefixEventIndex] prefixEventIndex`,
          prefixEventIndex
        );
        const prefixName =
          prefixEventIndex === null
            ? 'no prefix'
            : stringTable.getString(jsTracer.events[prefixEventIndex]);
  ...
  for (; prefixesTip > 0; prefixesTip--) {
    const stackIndex = prefixesStackIndexes[prefixesTip];
    if (stackIndex === null) {
      throw new Error(
        'A null stack index was unexpectedly received when walking up the prefix stacks.'
      );
    }
