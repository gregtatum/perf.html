...
import { toValidImplementationFilter, getCallNodeIndexFromPath, updateThreadStacks, getMapStackUpdater } from "./profile-data";
import { timeCode } from "../utils/time-code";
import { assertExhaustiveCheck, convertToTransformType } from "../utils/flow";
import { CallTree } from "../profile-logic/call-tree";
import { shallowCloneFrameTable, shallowCloneFuncTable, getEmptyStackTable } from "./data-structures";
import { getFunctionName } from "./function-info";

import { Thread, FuncTable, IndexIntoCategoryList, IndexIntoFuncTable, IndexIntoStackTable, IndexIntoResourceTable } from "../types/profile";
import { CallNodePath, CallNodeTable, StackType } from "../types/profile-derived";
...
['focus-subtree', 'focus-function', 'merge-call-node', 'merge-function', 'drop-function', 'collapse-resource', 'collapse-direct-recursion', 'collapse-function-subtree'].forEach((transform: TransformType) => {
...
export function parseTransforms(stringValue: string = ''): TransformStack {
  ...
  stringValue.split('~').forEach(s => {
    ...
    switch (type) {
      ...
      case 'merge-function':case 'focus-function':case 'drop-function':case 'collapse-function-subtree':
        {
          // e.g. "mf-325"
      ...
      case 'focus-subtree':case 'merge-call-node':
        {
          // e.g. "f-js-xFFpUMl-i" or "f-cpp-0KV4KV5KV61KV7KV8K"
...
export function stringifyTransforms(transforms: TransformStack = []): string {
  return transforms.map(transform => {
    ...
    switch (transform.type) {
      case 'merge-function':case 'drop-function':case 'collapse-function-subtree':case 'focus-function':
      ...
      case 'focus-subtree':case 'merge-call-node':
        {
          let string = [shortKey, transform.implementation, uintArrayToString(transform.callNodePath)].join('-');
...
export function getTransformLabels(thread: Thread, threadName: string, transforms: Transform[]) {
  const {
    funcTable,
    libs,
    stringTable,
  ...
  const labels: string[] = transforms.map(transform => {
    ...
    switch (transform.type) {
      case 'focus-subtree':case 'merge-call-node':
        ...
        break;
      case 'focus-function':case 'merge-function':case 'drop-function':case 'collapse-direct-recursion':case 'collapse-function-subtree':
...
export function applyTransformToCallNodePath(callNodePath: CallNodePath, transform: Transform, transformedThread: Thread): CallNodePath {
  switch (transform.type) {
    case 'focus-subtree':
...
function _removePrefixPathFromCallNodePath(prefixPath: CallNodePath, callNodePath: CallNodePath): CallNodePath {
  return _callNodePathHasPrefixPath(prefixPath, callNodePath) ? callNodePath.slice(prefixPath.length - 1) : [];
}

function _startCallNodePathWithFunction(funcIndex: IndexIntoFuncTable, callNodePath: CallNodePath): CallNodePath {
  const startIndex = callNodePath.indexOf(funcIndex);
  return startIndex === -1 ? [] : callNodePath.slice(startIndex);
}

function _mergeNodeInCallNodePath(prefixPath: CallNodePath, callNodePath: CallNodePath): CallNodePath {
  return _callNodePathHasPrefixPath(prefixPath, callNodePath) ? callNodePath.filter((_, i) => i !== prefixPath.length - 1) : callNodePath;
}

function _mergeFunctionInCallNodePath(funcIndex: IndexIntoFuncTable, callNodePath: CallNodePath): CallNodePath {
  return callNodePath.filter(nodeFunc => nodeFunc !== funcIndex);
}

function _dropFunctionInCallNodePath(funcIndex: IndexIntoFuncTable, callNodePath: CallNodePath): CallNodePath {
  // If the CallNodePath contains the function, return an empty path.
  return callNodePath.includes(funcIndex) ? [] : callNodePath;
}

function _collapseResourceInCallNodePath(resourceIndex: IndexIntoResourceTable, collapsedFuncIndex: IndexIntoFuncTable, funcTable: FuncTable, callNodePath: CallNodePath) {
  ...
  .filter((pathFuncIndex, pathIndex, path) => // This function doesn't match the previous one, so keep it.
...
function _collapseDirectRecursionInCallNodePath(funcIndex: IndexIntoFuncTable, callNodePath: CallNodePath) {
  const newPath = [];
  let previousFunc;
...
function _collapseFunctionSubtreeInCallNodePath(funcIndex: IndexIntoFuncTable, callNodePath: CallNodePath) {
  const index = callNodePath.indexOf(funcIndex);
  return index === -1 ? callNodePath : callNodePath.slice(0, index + 1);
}

function _callNodePathHasPrefixPath(prefixPath: CallNodePath, callNodePath: CallNodePath): boolean {
  return (prefixPath.length <= callNodePath.length && prefixPath.every((prefixFunc, i) => prefixFunc === callNodePath[i]));
}
 ...
 */
export function mergeCallNode(thread: Thread, callNodePath: CallNodePath, implementation: ImplementationFilter): Thread {
  return timeCode('mergeCallNode', () => {
    ...
    for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
      ...
      if (doMerge) {
        ...
        const newStackPrefix = oldStackToNewStack.get(prefix);
        newStackTable.prefix[newStackIndex] = newStackPrefix === undefined ? null : newStackPrefix;
    ...
    return updateThreadStacks(thread, newStackTable, getMapStackUpdater(oldStackToNewStack));
  });
}
 ...
 */
export function mergeFunction(thread: Thread, funcIndexToMerge: IndexIntoFuncTable): Thread {
  const {
    stackTable,
    frameTable
  } = thread;
 ...
 */
export function dropFunction(thread: Thread, funcIndexToDrop: IndexIntoFuncTable) {
  const {
    stackTable,
    frameTable
  } = thread;
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    ...
    const frameIndex = stackTable.frame[stackIndex];
    const funcIndex = frameTable.func[frameIndex];
    if ( // This is the function we want to remove.
  ...
  return updateThreadStacks(thread, stackTable, stack => // Drop the stacks that contain that function.
  stack !== null && stackContainsFunc[stack] ? null : stack);
}

export function collapseResource(thread: Thread, resourceIndexToCollapse: IndexIntoResourceTable, implementation: ImplementationFilter, defaultCategory: IndexIntoCategoryList): Thread {
  const {
    stackTable,
    funcTable,
    frameTable,
  ...
  const newStackTable = getEmptyStackTable();
  const oldStackToNewStack: Map<IndexIntoStackTable | null, IndexIntoStackTable | null> = new Map();
  const prefixStackToCollapsedStack: Map<IndexIntoStackTable | null, // prefix stack index
  ...
  return updateThreadStacks(newThread, newStackTable, getMapStackUpdater(oldStackToNewStack));
}

export function collapseDirectRecursion(thread: Thread, funcToCollapse: IndexIntoFuncTable, implementation: ImplementationFilter): Thread {
  const {
    stackTable,
    frameTable
  } = thread;
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    ...
    if ( // The previous stacks were collapsed or matched the funcToCollapse, check to see
    ...
    funcToCollapse === funcIndex || !funcMatchesImplementation(thread, funcIndex))) {
...
export function collapseFunctionSubtree(thread: Thread, funcToCollapse: IndexIntoFuncTable, defaultCategory: IndexIntoCategoryList): Thread {
  const {
    stackTable,
    frameTable
  } = thread;
  ...
  for (let stackIndex = 0; stackIndex < stackTable.length; stackIndex++) {
    const prefix = stackTable.prefix[stackIndex];
    if ( // The previous stack was collapsed, this one is collapsed too.
    collapsedStacks.has(prefix)) {
      ...
      if (newPrefixStackIndex !== null) {
        if (newStackTable.category[newPrefixStackIndex] !== stackTable.category[stackIndex]) {
          newStackTable.category[newPrefixStackIndex] = defaultCategory;
          newStackTable.subcategory[newPrefixStackIndex] = 0;
        } else if (newStackTable.subcategory[newPrefixStackIndex] !== stackTable.subcategory[stackIndex]) {
          newStackTable.subcategory[newPrefixStackIndex] = 0;
        }
      }
    } else {
 ...
 */
export function focusSubtree(thread: Thread, callNodePath: CallNodePath, implementation: ImplementationFilter): Thread {
 ...
 */
export function focusInvertedSubtree(thread: Thread, postfixCallNodePath: CallNodePath, implementation: ImplementationFilter): Thread {
...
export function focusFunction(thread: Thread, funcIndexToFocus: IndexIntoFuncTable): Thread {
 ...
 */
export function restoreAllFunctionsInCallNodePath(thread: Thread, previousImplementationFilter: ImplementationFilter, callNodePath: CallNodePath): CallNodePath {
  const {
    stackTable,
    frameTable
  } = thread;
...
export function getStackType(thread: Thread, funcIndex: IndexIntoFuncTable): StackType {
...
export function filterCallNodePathByImplementation(thread: Thread, implementationFilter: ImplementationFilter, callNodePath: CallNodePath): CallNodePath {
 ...
 * recursion.
 */
export function funcHasRecursiveCall(thread: Thread, implementation: ImplementationFilter, funcToCheck: IndexIntoFuncTable) {
  const {
    stackTable,
    frameTable
  } = thread;
...
export function applyTransform(thread: Thread, transform: Transform, defaultCategory: IndexIntoCategoryList): Thread {
  switch (transform.type) {
    case 'focus-subtree':
      return transform.inverted ? focusInvertedSubtree(thread, transform.callNodePath, transform.implementation) : focusSubtree(thread, transform.callNodePath, transform.implementation);
    case 'merge-call-node':
