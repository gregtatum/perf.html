...
export async function importProfilesFromArrayBuffer(
  ...
  return importProfileGroup(
    MaybeCompressedDataReader.fromArrayBuffer(fileName, buffer)
  );
}
...
async function _importProfileGroup(
  ...
  if (parsed) {
    if (
      parsed.$schema === 'https://www.speedscope.app/file-format-schema.json'
    ) {
      console.log('Importing as speedscope json file');
      return importSpeedscopeProfiles(JSON.parse(contents));
      ...
      'mode' in parsed &&
      'frames' in parsed &&
      'raw_timestamp_deltas' in parsed
    ) {
      console.log('Importing as stackprof profile');
