...
function parseTSV<T>(contents: string): T[] {
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < line.length; i++) {
      (row: any)[indexToField.get(i)] = line[i];
    }
    ret.push(row);
  }
  return ret;
...
async function extractDirectoryTree(
  ...
  for (const child of children) {
    if (child.isDirectory) {
      const subtree = await extractDirectoryTree(
        (child: FileSystemDirectoryEntry)
      );
      node.subdirectories.set(subtree.name, subtree);
    } else {
      const file = await new Promise<File>((resolve, reject) => {
        (child: FileSystemFileEntry).file(resolve, reject);
      });
...
async function readFormTemplate(
  ...
  const archive = readInstrumentsKeyedArchive(
