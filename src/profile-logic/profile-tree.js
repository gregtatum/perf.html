...
function _getInvertedStackSelfTimes(
  ...
  for (
    ...
    if (prefix !== -1) {
      funcStackToRoot[funcStackIndex] = funcStackToRoot[prefix];
    } else {
  ...
  for (let i = 0; i < sampleFuncStacks.length; i++) {
    const funcStackIndex = sampleFuncStacks[i];
    if (funcStackIndex !== null) {
      const rootFuncStackIndex = funcStackToRoot[funcStackIndex];
      funcStackSelfTime[rootFuncStackIndex] += interval;
...
export function computeCallTreeCountsAndTimings(
  ...
  for (
    ...
    const prefixStack = thread.stackTable.prefix[funcStackIndex];
    if (prefixStack === null) {
      rootTotalTime += funcStackTotalTime[funcStackIndex];
      rootCount++;
    } else {
      const prefixFuncStack = stackIndexToFuncStackIndex[prefixStack];
