...
import { getUserTiming, getProfileWithMarkers, getNetworkTrackProfile } from "../fixtures/profiles/processed-profile";
...
describe('selectors/getMarkerChartTimingAndBuckets', function () {
  ...
  describe('markers of the same name', function () {
    it('puts markers of the same time in two rows', function () {
      ...
      // 'Marker Name': *------*
      //              : *------*
      const markerTiming = getMarkerChartTimingAndBuckets([['Marker Name', 0, { startTime: 0, endTime: 10 }], ['Marker Name', 0, { startTime: 0, endTime: 10 }]]);
    ...
    it('puts markers of disjoint times in one row', function () {
      ...
      //              : 'Category'
      // 'Marker Name': *------*  *------*
      const markerTiming = getMarkerChartTimingAndBuckets([['Marker Name', 0, { startTime: 0, endTime: 10 }], ['Marker Name', 0, { startTime: 15, endTime: 25 }]]);
    ...
    it('puts markers of overlapping times in two rows', function () {
      ...
      // 'Marker Name': *------*
      //              :     *------*
      const markerTiming = getMarkerChartTimingAndBuckets([['Marker Name', 0, { startTime: 0, endTime: 10 }], ['Marker Name', 0, { startTime: 5, endTime: 15 }]]);
      expect(markerTiming).toHaveLength(3);
    });
    ...
    it('puts markers of inclusive overlapping times in two rows', function () {
      ...
      // 'Marker Name': *--------*
      //              :   *---*
      const markerTiming = getMarkerChartTimingAndBuckets([['Marker Name', 0, { startTime: 0, endTime: 20 }], ['Marker Name', 0, { startTime: 5, endTime: 15 }]]);
      expect(markerTiming).toHaveLength(3);
    });
  });
  ...
  describe('markers of the different names', function () {
    it('puts them in different rows', function () {
      ...
      // 'Marker Name A': *------*
      // 'Marker Name B':           *------*
      const markerTiming = getMarkerChartTimingAndBuckets([['Marker Name A', 0, { startTime: 0, endTime: 10 }], ['Marker Name B', 0, { startTime: 20, endTime: 30 }]]);
      expect(markerTiming).toHaveLength(3);
      const [category, markerTimingA, markerTimingB] = markerTiming;
      if (typeof markerTimingA === 'string' || typeof markerTimingB === 'string') {
  ...
  describe('markers that are crossing the profile start or end', function () {
    it('renders properly markers starting before profile start', function () {
      const markerTiming = getMarkerChartTimingAndBuckets([['Rasterize', 1, { category: 'Paint', interval: 'end', type: 'tracing' }]]);
      expect(markerTiming).toEqual(['Idle', {
        name: 'Rasterize',
    ...
    it('renders properly markers ending after profile end', function () {
      const markerTiming = getMarkerChartTimingAndBuckets([['Rasterize', 20, { category: 'Paint', interval: 'start', type: 'tracing' }]]);
      expect(markerTiming).toEqual(['Idle', {
        name: 'Rasterize',
        start: [20],
...
describe('getProcessedRawMarkerTable', function () {
  ...
  it('can process Invalidation markers', function () {
    const markers = setup([['Invalidate http://mozilla.com/script.js:1234', 10, null], ['Invalidate self-hosted:2345', 20, null], ['Invalidate resource://foo -> resource://bar:3456', 30, null], ['Invalidate moz-extension://<URL>', 40, null]]);
  ...
  it('can process Bailout markers', function () {
    const markers = setup([['Bailout_ShapeGuard after getelem on line 3666 of resource://foo.js -> resource://bar.js:3662', 10, null], ['Bailout_TypeBarrierV at jumptarget on line 1021 of self-hosted:970', 20, null], // Also handle sanitized profiles where URLs have been redacted
    ['Bailout_ShapeGuard at jumptarget on line 7 of moz-extension://<URL>', 30, null]]);
...
describe('getTimelineVerticalMarkers', function () {
  it('gets the appropriate markers', function () {
    const {
      getState
    } = storeWithProfile(getNetworkTrackProfile());
    const getMarker = selectedThreadSelectors.getMarkerGetter(getState());
    const markerIndexes = selectedThreadSelectors.getTimelineVerticalMarkerIndexes(getState());
    const allMarkers = selectedThreadSelectors.getFullMarkerListIndexes(getState());
...
describe('memory markers', function () {
  function setup() {
    ...
    return storeWithProfile(getProfileWithMarkers([['A', 0, null], ['B', 1, null], ['C', 2, null], ['IdleForgetSkippable', 3, { type: 'tracing', category: 'CC', interval: 'start' }], ['IdleForgetSkippable', 4, { type: 'tracing', category: 'CC', interval: 'end' }], ['GCMinor', 5, { type: 'GCMinor', startTime: 5, endTime: 5, nursery: any }], ['GCMajor', 6, { type: 'GCMajor', startTime: 6, endTime: 6, timings: any }], ['GCSlice', 7, { type: 'GCSlice', startTime: 7, endTime: 7, timings: any }]]));
  }
  ...
  it('can get memory markers using getMemoryMarkers', function () {
    ...
    const getMarker = selectedThreadSelectors.getMarkerGetter(getState());
    const markerIndexes = selectedThreadSelectors.getMemoryMarkerIndexes(getState());
    expect(markerIndexes.map(markerIndex => getMarker(markerIndex).name)).toEqual(['IdleForgetSkippable', 'GCMinor', 'GCMajor', 'GCSlice']);
  });
  ...
  it('ignores memory markers in getCommittedRangeAndTabFilteredMarkerIndexesForHeader', function () {
    ...
    const getMarker = selectedThreadSelectors.getMarkerGetter(getState());
    const markerIndexes = selectedThreadSelectors.getCommittedRangeAndTabFilteredMarkerIndexesForHeader(getState());
    expect(markerIndexes.map(markerIndex => getMarker(markerIndex).name)).toEqual(['A', 'B', 'C']);
  });
});

describe('selectors/getUserTimingMarkerTiming', function () {
  it('simple profile', function () {
    const profile = getProfileWithMarkers([getUserTiming('renderFunction', 0, 10), getUserTiming('componentA', 1, 8), getUserTiming('componentB', 2, 4)]);
    const {
      getState
    } = storeWithProfile(profile);

    expect(selectedThreadSelectors.getUserTimingMarkerTiming(getState())).toEqual([{
      start: [0],
      end: [10],
      index: [0],
...
describe('selectors/getCommittedRangeAndTabFilteredMarkerIndexes', function () {
  ...
  function setup(showTabOnly, markers: Array<any> | null | undefined) {
    const profile = getProfileWithMarkers(markers || [['Dummy 1', 10, {
      type: 'tracing',
      category: 'Navigation',
      interval: 'start',
      innerWindowID
    }], ['Dummy 2', 20, null], ['Dummy 3', 30, {
      type: 'tracing',
      category: 'Navigation',
      interval: 'start',
    ...
    const markerIndexes = selectedThreadSelectors.getCommittedRangeAndTabFilteredMarkerIndexes(getState());
  ...
  it('does not filter markers if we are not in the single tab view', function () {
    ...
    expect(markers).toEqual(['Dummy 1', 'Dummy 2', 'Dummy 3', 'Dummy 4', 'Dummy 5']);
  });
  ...
  it('preserves global markers', function () {
    const markers = setup(true, [['Dummy 1', 20, null], ['Jank', 20, null], ['Dummy 2', 20, null]]);
