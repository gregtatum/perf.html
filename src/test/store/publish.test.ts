...
import { attemptToPublish, resetUploadState, abortUpload, toggleCheckedSharingOptions, revertToPrePublishedState } from "../../actions/publish";
import { changeSelectedTab } from "../../actions/app";
import { viewProfileFromPathInZipFile, returnToZipFileList } from "../../actions/zipped-profiles";
import { getCheckedSharingOptions, getUploadPhase, getUploadError, getUploadProgress, getUploadProgressString, getUploadGeneration } from "../../selectors/publish";
import { getSelectedTab, getDataSource, getProfileName, getHash, getTransformStack } from "../../selectors/url-state";
...
import { addTransformToStack, hideGlobalTrack } from "../../actions/profile-view";
...
describe('attemptToPublish', function () {
  ...
  function setupFakeUploadsWithStore(store: Store): any {
    ...
    promise.catch(() => {// Node complains if we don't handle a promise/catch, and this one rejects
    ...
    (uploadBinaryProfileData as any).mockImplementation(initUploadProcess);

    jest.spyOn(window, 'open').mockImplementation(() => {});

    function getUpdateUploadProgress() {
      return ensureExists(updateUploadProgress, 'Expected to get a reference to the callback to update the upload progress.');
    }
  ...
  it('should preserve the transforms after sanitization', async function () {
    ...
    dispatch(addTransformToStack(1, {
      type: 'focus-function',
      funcIndex: B
    }));
