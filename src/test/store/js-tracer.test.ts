...
import { convertJsTracerToThread, getJsTracerFixed } from "../../profile-logic/js-tracer";
import { getEmptyProfile } from "../../profile-logic/data-structures";
import { formatTree } from "../fixtures/utils";
import { getProfileFromTextSamples, getProfileWithJsTracerEvents, TestDefinedJsTracerEvent } from "../fixtures/profiles/processed-profile";

import { Profile } from "../../types/profile";

describe('jsTracerFixed', function () {
  function fixTiming(events: any) {
  ...
  it('does not modify a valid structure', function () {
    const timing = fixTiming([// [mozilla                  ]
    //  [int   ][ion          ]
    //   [int]    [ion      ]
    ['https://mozilla.org', 0, 20], ['Interpreter', 1, 5], ['Interpreter', 2, 4], ['IonMonkey', 5, 19], ['IonMonkey', 6, 18]]);
...
describe('convertJsTracerToThread', function () {
  it('can generate stacks correctly', function () {
    const existingProfile = getProfileWithJsTracerEvents([// [mozilla                  ]
    //  [int   ][ion          ]
    //   [int]    [ion      ]
    ['https://mozilla.org', 0, 20], ['Interpreter', 1, 5], ['Interpreter', 2, 4], ['IonMonkey', 5, 19], ['IonMonkey', 6, 18]]);
    ...
    profile.threads = [convertJsTracerToThread(existingThread, jsTracer, categories)];
    const {
      getState
    } = storeWithProfile(profile);
    const callTree = selectedThreadSelectors.getCallTree(getState());

    expect(formatTree(callTree)).toEqual([// This is the real timing:
    '- https://mozilla.org (total: 20, self: 2)', '  - Interpreter (total: 18, self: 2)', '    - IonMonkey (total: 14, self: 2)', '      - IonMonkey (total: 12, self: 12)', '    - Interpreter (total: 2, self: 2)']);
  });
  ...
  it('can generate the frameTable implementations correctly', function () {
    ...
    //  [int   ][ion          ]
    //   [A  ]    [B       ]
    ['https://mozilla.org', 0, 20], ['Interpreter', 1, 5], ['FuncA', 2, 4], ['IonMonkey', 5, 19], ['FuncB', 6, 18]]);
    const existingThread = existingProfile.threads[0];
    const categories = existingProfile.meta.categories;
    const jsTracer = ensureExists(existingThread.jsTracer);
    const thread = convertJsTracerToThread(existingThread, jsTracer, categories);
    const implementationNames = thread.frameTable.implementation.map(implementation => implementation === null ? null : thread.stringTable.getString(implementation));
    expect(implementationNames).toEqual([null, // 'https://mozilla.org'
...
describe('selectors/getJsTracerTiming', function () {
  describe('full stack-based view', function () {
    it('has no JS tracer timing if no js tracer info is present', function () {
      ...
      expect(selectedThreadSelectors.getExpensiveJsTracerTiming(getState())).toEqual(null);
    });

    it('has empty JS tracer timing if no events are in the js tracer table', function () {
      expect(getHumanReadableJsTracerTiming({ useSelfTime: false, events: [] })).toEqual([]);
    });

    it('can generate some simple nested timing', function () {
      expect(getHumanReadableJsTracerTiming({
        useSelfTime: false,
        events: [['https://mozilla.org', 0, 20], ['Interpreter', 1, 19], ['IonMonkey', 2, 18]]
      })).toEqual(['https://mozilla.org (0:20)', 'Interpreter (1:19)', 'IonMonkey (2:18)']);
    });

    it('can generate sibling timing', function () {
      expect(getHumanReadableJsTracerTiming({
        useSelfTime: false,
        events: [['https://mozilla.org', 0, 20], ['Interpreter', 1, 5], ['Interpreter', 2, 4], ['IonMonkey', 5, 19], ['IonMonkey', 6, 18]]
      })).toEqual(['https://mozilla.org (0:20)', 'Interpreter (1:5) | IonMonkey (5:19)', 'Interpreter (2:4) | IonMonkey (6:18)']);
    });
  });

  describe('self time', function () {
    it('has empty JS tracer timing if no events are in the js tracer table', function () {
      expect(getHumanReadableJsTracerTiming({ useSelfTime: true, events: [] })).toEqual([]);
    });
    ...
    it('can generate some simple nested timing', function () {
      expect(getHumanReadableJsTracerTiming({
        useSelfTime: true,
        events: [['https://mozilla.org', 0, 20], ['Interpreter', 1, 19], ['IonMonkey', 2, 18]]
      })).toEqual(['Interpreter (1:2) | Interpreter (18:19)', 'IonMonkey (2:18)', 'https://mozilla.org (0:1) | https://mozilla.org (19:20)']);
    });
    ...
    it('can "drain off" prefixes the first if branch of the algorithm', function () {
      ...
      expect(getHumanReadableJsTracerTiming({
        useSelfTime: true,
        events: [// This comment makes the formatting "prettier".
        ['A', 0, 2], ['B', 0, 2], ['C', 0, 1], ['A', 2, 3]]
      })).toEqual([// This comment makes the formatting "prettier".
      'A (2:3)', 'B (1:2)', 'C (0:1)']);
    });
    ...
    it('works on the sampled data', function () {
...

      {
        ...
        const {
          ...
          jsTracer
        } = getProfileWithJsTracerEvents([['Root', 0, 20], ['Node', 1, 19], ['https://mozilla.org', 2, 18], ['https://mozilla.org', 3, 16]]).threads[0];
        ...
        for (let jsTracerIndex = 0; jsTracerIndex < jsTracer.length; jsTracerIndex++) {
          // Map the old string to the new string.
          jsTracer.events[jsTracerIndex] = thread.stringTable.indexForString(tracerStringTable.getString(jsTracer.events[jsTracerIndex]));
        }
        ...
        thread.funcTable.fileName[foo] = thread.stringTable.indexForString('https://mozilla.org');
        ...
        thread.funcTable.fileName[bar] = thread.stringTable.indexForString('https://mozilla.org');
        ...
        thread.funcTable.fileName[baz] = thread.stringTable.indexForString('https://mozilla.org');
      ...
      expect(getHumanReadableJsTracerTiming({
        useSelfTime: false,
        profile
      })).toEqual(['Root (0:20)', 'Node (1:19)', 'ƒ Foo.js  https://mozilla.org (2:18)', '(multiple matching functions) https://mozilla.org (3:16)']);
    });
  });
});
...
function getHumanReadableJsTracerTiming({
  ...
  if (!profile) {
    profile = getProfileWithJsTracerEvents(ensureExists(events, 'Expected to have a list of tracer events when no profile was supplied.'));
  }
  const {
    dispatch,
    getState
  } = storeWithProfile(profile);
  const computeTiming = useSelfTime ? selectedThreadSelectors.getExpensiveJsTracerLeafTiming : selectedThreadSelectors.getExpensiveJsTracerTiming;
  ...
  return ensureExists(computeTiming(getState())).map(({
    ...
    for (let i = 0; i < length; i++) {
      lines.push(`${label[i]} (${parseFloat(start[i].toFixed(2))}:${parseFloat(end[i].toFixed(2))})`);
    }
    return lines.join(' | ');
  });
}