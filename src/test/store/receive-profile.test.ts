...
import { viewProfile, finalizeProfileView, retrieveProfileFromAddon, retrieveProfileFromStore, retrieveProfileOrZipFromUrl, retrieveProfileFromFile, retrieveProfilesToCompare, _fetchProfile, getProfilesFromRawUrl, changeViewAndRecomputeProfileData } from "../../actions/receive-profile";
import { SymbolsNotFoundError } from "../../profile-logic/errors";
import fakeIndexedDB from "fake-indexeddb";

import { createGeckoProfile } from "../fixtures/profiles/gecko-profile";
import JSZip from "jszip";
import { serializeProfile, processProfile } from "../../profile-logic/process-profile";
import { getProfileFromTextSamples, addMarkersToThreadWithCorrespondingSamples } from "../fixtures/profiles/processed-profile";
...
function simulateSymbolStoreHasNoCache() {
  ...
  (SymbolStoreDB as any).mockImplementation(() => ({
    getSymbolTable: jest.fn().mockImplementation((debugName, breakpadId) => Promise.reject(new SymbolsNotFoundError('The requested library does not exist in the database.', { debugName, breakpadId })))
  }));
}
...
describe('actions/receive-profile', function () {
  ...
  describe('viewProfile', function () {
    it('can take a profile and view it', function () {
      ...
      const initialProfile = ProfileViewSelectors.getProfileOrNull(store.getState());
    ...
    function getProfileWithIdleAndWorkThread() {
      const {
        profile
      } = getProfileFromTextSamples(`A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]`, `work  work  work  work  work  work  work`);
      ...
      idleThread.stackTable.category = idleThread.stackTable.category.map(() => idleCategoryIndex);
    ...
    it('will hide threads with idle samples', function () {
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [process]', '  - hide [thread Idle Thread]', '  - show [thread Work Thread] SELECTED']);
    });
    ...
    it('will show the thread with no samples and with paint markers', function () {
      ...
      profile.threads.push( // This thread shouldn't be hidden because it's the main thread in the main process.
      getEmptyThread({ name: 'GeckoMain', processType: 'default', pid: 1 }), // This thread shouldn't be hidden because it will have useful markers even if it has no samples.
      getEmptyThread({ name: 'GeckoMain', processType: 'tab', pid: 2 }), // This thread will be hidden because it has no samples and no markers.
      getEmptyThread({ name: 'GeckoMain', processType: 'tab', pid: 3 }));

      addMarkersToThreadWithCorrespondingSamples(profile.threads[1], [['RefreshDriverTick', 0, { type: 'tracing', category: 'Paint', interval: 'start' }]]);

      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [thread GeckoMain default]', 'show [thread GeckoMain tab] SELECTED', 'hide [thread GeckoMain tab]']);
    });
    ...
    it('will not hide the Windows GPU thread', function () {
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [thread GeckoMain default] SELECTED', 'show [thread GeckoMain default]']);
    });
    ...
    it('will not hide the Compositor thread', function () {
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [thread GeckoMain default] SELECTED', '  - show [thread Compositor]']);
    });
    ...
    it('will not hide the only global track', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples(`A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]`, `work  work  work  work  work`);
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [thread GeckoMain tab] SELECTED', '  - show [thread Other]']);
    });
    ...
    it('will hide idle content threads with no RefreshDriverTick markers', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples(`A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]  A[cat:Idle]`, `work  work  work  work  work  work  work`, `C[cat:Idle]  C[cat:Idle]  C[cat:Idle]  C[cat:Idle]  C[cat:Idle]`);
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['hide [thread GeckoMain tab]', 'show [thread GeckoMain tab] SELECTED', 'hide [thread GeckoMain tab]']);
    });
    ...
    it('will not hide non-idle content threads with no RefreshDriverTick markers', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples(`work  work  work  work  work  work  work`, `work  work  work  work  work  work  work`, `C[cat:Idle]  C[cat:Idle]  C[cat:Idle]  work  work`);
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [thread GeckoMain tab] SELECTED', 'show [thread GeckoMain tab]', 'show [thread GeckoMain tab]']);
    });
    ...
    it('will hide an empty global track when all child tracks are hidden', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples(`work  work  work  work  work`, // pid 1
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [process]', '  - show [thread Work A] SELECTED', '  - show [thread Work B]', 'hide [process]', // <- Ensure this process is hidden.
      '  - hide [thread Idle C]', 'show [process]', '  - show [thread Work E]']);
    });
    ...
    it('will not hide audio tracks if they have at least one sample', function () {
      ...
      const {
        profile
      } = getProfileFromTextSamples(oneWorkSampleThreadString, // AudioIPC
      ...
      store.dispatch(viewProfile(profile));
      expect(getHumanReadableTracks(store.getState())).toEqual(['show [process]', '  - show [thread AudioIPC work] SELECTED', '  - show [thread MediaPDecoder work]', '  - show [thread MediaTimer work]', '  - show [thread MediaPlayback work]', '  - show [thread MediaDecoderStateMachine work]', '  - hide [thread AudioIPC idle]', '  - hide [thread MediaPDecoder idle]', '  - hide [thread MediaTimer idle]', '  - hide [thread MediaPlayback idle]', '  - hide [thread MediaDecoderStateMachine idle]']);
    });
  });
  ...
  describe('changeViewAndRecomputeProfileData', function () {
    ...
    function setup(initializeShowTabOnly: boolean = false) {
      ...
      profile.threads.push(getEmptyThread({ name: 'GeckoMain', processType: 'tab', pid: 1 }));
    ...
    it('should be able to switch to active tab view from the full view', function () {
      ...
      expect(UrlStateSelectors.getShowTabOnly(getState())).toBe(browsingContextID);
    });
  ...
  describe('retrieveProfileFromAddon', function () {
    ...
    function setup(profileAs = 'json') {
      ...
      switch (profileAs) {
        case 'json':
          ...
          break;
        case 'arraybuffer':
          mockGetProfile = jest.fn().mockResolvedValue(toUint8Array(profileJSON).buffer);
          break;
        case 'gzip':
          mockGetProfile = jest.fn().mockReturnValue(compress(toUint8Array(profileJSON)).then(x => x.buffer));
          break;
        default:
      ...
      const geckoProfiler = {
        ...
        getSymbolTable: jest.fn().mockRejectedValue(new Error('No symbol tables available'))
      };
      window.fetch = jest.fn().mockRejectedValue(new Error('No symbolication API in place'));
    ...
    it('tries to symbolicate the received profile', async () => {
      ...
      expect(geckoProfiler.getSymbolTable).toHaveBeenCalledWith('firefox', expect.any(String));

      expect(window.fetch).toHaveBeenCalledWith('https://symbols.mozilla.org/symbolicate/v5', expect.objectContaining({
    ...
    it('displays a warning after 30 seconds', async function () {
      ...
      const errorMessage = 'We were unable to connect to the Gecko profiler add-on within thirty seconds. This might be because the profile is big or your machine is slower than usual. Still waiting...';
  ...
  describe('retrieveProfileFromStore', function () {
    ...
    const fetch403Response = (({ ok: false, status: 403 } as any) as Response);
    const fetch500Response = (({ ok: false, status: 500 } as any) as Response);
    const fetch200Response = (({
      ...
      json: () => Promise.resolve(JSON.parse(serializeProfile(_getSimpleProfile())))
    } as any) as Response);
    ...
    beforeEach(function () {
      ...
      jest.spyOn(window, 'setTimeout').mockImplementation(callback => process.nextTick(callback));
    });
    ...
    it('can retrieve a profile from the web and save it to state', async function () {
      ...
      window.fetch = jest.fn(url => Promise.resolve(url === expectedUrl ? fetch200Response : fetch403Response));
    ...
    it('symbolicates a profile if it is not symbolicated yet', async () => {
      const {
        profile: unsymbolicatedProfile
      } = getProfileFromTextSamples('0xA[lib:libxul]');
      unsymbolicatedProfile.meta.symbolicated = false;

      window.fetch = jest.fn().mockResolvedValue((({
        ...fetch200Response,
        json: () => Promise.resolve(JSON.parse(serializeProfile(unsymbolicatedProfile)))
      } as any) as Response));
      ...
      expect(window.fetch).toHaveBeenLastCalledWith('https://symbols.mozilla.org/symbolicate/v5', expect.objectContaining({
    ...
    it('requests several times in case of 403', async function () {
      const hash = 'c5e53f9ab6aecef926d4be68c84f2de550e2ac2f';
      const expectedUrl = `https://storage.googleapis.com/profile-store/${hash}`;
      window.fetch.mockImplementationOnce(_ => Promise.resolve(fetch403Response)).mockImplementationOnce(url => Promise.resolve(url === expectedUrl ? fetch200Response : fetch403Response));

      const store = blankStore();
      const views = (await observeStoreStateChanges(store, () => store.dispatch(retrieveProfileFromStore(hash)))).map(state => getView(state));

      const errorMessage = 'Profile not found on remote server.';
      expect(views).toEqual([{ phase: 'INITIALIZING' }, {
        phase: 'INITIALIZING',
        additionalData: {
          attempt: { count: 1, total: 11 },
          message: errorMessage
        }
      }, { phase: 'PROFILE_LOADED' }, { phase: 'DATA_LOADED' }]);
    ...
    it('fails in case the profile cannot be found after several tries', async function () {
      ...
      const errorMessage = 'Profile not found on remote server.';
      expect(views).toEqual([{ phase: 'INITIALIZING' }, ...steps.map(step => ({
        phase: 'INITIALIZING',
        additionalData: {
  ...
  describe('retrieveProfileOrZipFromUrl', function () {
    ...
    it('requests several times in case of 403', async function () {
      ...
      window.fetch.mockResolvedValueOnce(fetch403Response).mockImplementationOnce(url => Promise.resolve(url === expectedUrl ? fetch200Response : fetch403Response));

      const store = blankStore();
      const views = (await observeStoreStateChanges(store, () => store.dispatch(retrieveProfileOrZipFromUrl(expectedUrl)))).map(state => getView(state));
  ...
  describe('_fetchProfile', function () {
    ...
    async function configureFetch(obj: {
          ...
          get: (name: string) => {
            switch (name) {
              case 'content-type':
                return contentType;
              default:
                throw new Error("Unhandled stub for fetch's response.headers.get");
      ...
      window.fetch = jest.fn(actualUrl => Promise.resolve(actualUrl === url ? zippedProfileResponse : fetch403Response));
  ...
  describe('retrieveProfileFromFile', function () {
    ...
    it('can load json with a good mime type', async function () {
      ...
      expect(ProfileViewSelectors.getProfile(getState()).meta.product).toEqual('JSON Test');
    });
    ...
    it('will give an error when unable to parse json', async function () {
      ...
      expect( // Coerce into the object to access the error property.
      (view as Object).error).toMatchSnapshot();
    });

    // eslint-disable-next-line jest/no-disabled-tests
    it.skip('can load gzipped json', async function () {// TODO - See issue #1023. The zee-worker is failing to compress/decompress
      // the profile.
    });

    // eslint-disable-next-line jest/no-disabled-tests
    it.skip('will give an error when unable to parse gzipped profiles', async function () {// TODO - See issue #1023. The zee-worker is failing to compress/decompress
      // the profile.
    });

    async function setupZipTestWithProfile(fileName: string, serializedProfile: string) {
      const zip = new JSZip();
    ...
    it('can load a zipped profile', async function () {
      const {
        getState,
        view
      } = await setupZipTestWithProfile('profile.json', serializeProfile(_getSimpleProfile()));
      expect(view.phase).toBe('DATA_LOADED');
    ...
    it('will load and view a simple profile with no errors', async function () {
      ...
      expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('LIST_FILES_IN_ZIP_FILE');
      await dispatch(viewProfileFromPathInZipFile('profile.json'));
      expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('VIEW_PROFILE_IN_ZIP_FILE');
      const errorMessage = ZippedProfilesSelectors.getZipFileErrorMessage(getState());
    ...
    it('will be an error to view a profile with no threads', async function () {
      const {
        getState,
        dispatch
      } = await setupZipTestWithProfile('profile.json', serializeProfile(getEmptyProfile()));

      expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('LIST_FILES_IN_ZIP_FILE');
      expect(ZippedProfilesSelectors.getZipFileErrorMessage(getState())).toEqual(null);
      ...
      expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE');
      expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('FAILED_TO_PROCESS_PROFILE_FROM_ZIP_FILE');
      const errorMessage = ZippedProfilesSelectors.getZipFileErrorMessage(getState());
  ...
  describe('retrieveProfilesToCompare', function () {
    ...
    function getSomeProfiles() {
      const {
        profile: profile1
      } = getProfileFromTextSamples(`A  B  C  D  E`, `G  H  I  J  K`);
      const {
        profile: profile2
      } = getProfileFromTextSamples(`L  M  N  O  P  Ex  Ex  Ex  Ex`, `Q  R  S  T  U  Ex  Ex  Ex  Ex`);
    ...
    function setupWithLongUrl(profiles: SetupProfileParams, {
      urlSearch1,
      urlSearch2
    ...
    async function setupWithShortUrl(profiles: SetupProfileParams, {
      urlSearch1,
      urlSearch2
    }: SetupUrlSearchParams): any {
    ...
    async function setup({
      ...
      profile1.threads.forEach(thread => addMarkersToThreadWithCorrespondingSamples(thread, [['A', 1, { startTime: 1, endTime: 3 }], ['A', 1, null], ['B', 2, null], ['C', 3, null], ['D', 4, null], ['E', 5, null]]));
      profile2.threads.forEach(thread => addMarkersToThreadWithCorrespondingSamples(thread, [['F', 1, { startTime: 1, endTime: 3 }], ['G', 2, null], ['H', 3, null], ['I', 4, null], ['J', 5, null]]));

      window.fetch.mockResolvedValueOnce(fetch200Response(serializeProfile(profile1))).mockResolvedValueOnce(fetch200Response(serializeProfile(profile2)));
    ...
    beforeEach(function () {
      window.fetch = jest.fn();
      window.fetch.mockImplementation(() => Promise.reject(new Error('No more answers have been configured.')));
    });
    ...
    it('retrieves profiles and put them in the same view', async function () {
      ...
      const expectedThreads = [profile1.threads[0], profile2.threads[1]].map((thread, i) => ({
        ...thread,
      ...
      // comparison thread
      expectedThreads.push(expect.objectContaining({
    ...
    it('does not reuse the implementation information if one profile used it', async function () {
      ...
      expect(UrlStateSelectors.getImplementationFilter(getState())).not.toBe('js');
    });
  ...
  describe('getProfilesFromRawUrl', function () {
    ...
    async function setup(location: {[name: string]: any }, requiredProfile: number = 1) {
      const profile = _getSimpleProfile();
      ...
      for (let i = 0; i < requiredProfile; i++) {
        window.fetch.mockResolvedValueOnce(fetch200Response(serializeProfile(profile)));
      }
      ...
      const waitUntilPhase = phase => waitUntilState(store, state => getView(state).phase === phase);

      const waitUntilSymbolication = () => waitUntilState(store, state => ProfileViewSelectors.getSymbolicationStatus(state) === 'DONE');
    ...
    it('keeps the `from-url` value in the URL', async function () {
      ...
      await dispatch(finalizeProfileView());
      const [, fromAddon, urlString] = urlFromState(UrlStateSelectors.getUrlState(getState())).split('/');
    ...
    it('retrieves profile from a `from-addon` data source and loads it', async function () {
      ...
      expect(ProfileViewSelectors.getProfile(getState())).toEqual(processedProfile);
    });
