...
describe('actions/icons', function () {
  const validIcons = ['https://valid.icon1.example.org/favicon.ico', 'https://valid.icon2.example.org/favicon.ico'];
  const expectedClasses = ['https___valid_icon1_example_org_favicon_ico', 'https___valid_icon2_example_org_favicon_ico'];
  ...
  beforeEach(() => {
    ...
    (window as Object).Image = mock.Image;
  });

  afterEach(() => {
    delete (window as Object).Image;
    imageInstances = [];
  });
  ...
  describe('With the initial state', function () {
    ...
    it('getIconClassName returns an empty string for any icon', function () {
      const subject = iconsAccessors.getIconClassName(getInitialState(), _createCallNodeWithIcon(validIcons[0]).icon);
      expect(subject).toBe('');
    });
  ...
  describe('Requesting an existing icon', function () {
    it('will populate the local cache', async function () {
      ...
      const promises = [dispatch(iconsActions.iconStartLoading(validIcons[0])), // Second request for the same icon shouldn't dspatch anything
      dispatch(iconsActions.iconStartLoading(validIcons[0])), // 3rd request for another icon should dispatch the loaded action
      ...
      imageInstances.forEach(instance => (instance as Object).onload());
      ...
      validIcons.forEach((icon, i) => {
        subject = iconsAccessors.getIconClassName(state, _createCallNodeWithIcon(icon).icon);
  ...
  describe('Requesting a non-existing image', function () {
    it('will not populate the local cache', async function () {
      ...
      const actionPromise = dispatch(iconsActions.iconStartLoading(invalidIcon));
      expect(imageInstances.length).toBe(1);
      (imageInstances[0] as Object).onerror();
      ...
      subject = iconsAccessors.getIconClassName(state, _createCallNodeWithIcon(invalidIcon).icon);
      expect(subject).toBe('');
