...
import { formatZipFileTable, storeWithZipFile } from "../fixtures/profiles/zip-file";
...
describe('reducer zipFileState', function () {
  ...
  it('can load a profile from the zip file', async function () {
    const {
      dispatch,
      getState
    } = await storeWithZipFile(['foo/bar/profile1.json', 'foo/profile2.json', 'baz/profile3.json']);
    expect(ProfileViewSelectors.getProfileOrNull(getState())).toEqual(null);

    await dispatch(ZippedProfilesActions.viewProfileFromPathInZipFile('foo/bar/profile1.json'));

    expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toBe('VIEW_PROFILE_IN_ZIP_FILE');
  ...
  describe('profileName handling', function () {
    async function setup() {
      const {
        dispatch,
        getState
      } = await storeWithZipFile(['foo/bar/profile1.json', 'foo/profile2.json', 'baz/profile3.json', 'profile4.json']);
    ...
    it('computes a profile when no folder present', async function () {
      ...
      await dispatch(ZippedProfilesActions.viewProfileFromPathInZipFile('profile4.json'));
    ...
    it('prefers ProfileName if it is given in the URL', async function () {
      ...
      expect(UrlStateSelectors.getProfileName(getState())).toBe(profileNameFromURL);
    });
  });
  ...
  it('will fail when trying to load an invalid profile', async function () {
    ...
    await dispatch(ZippedProfilesActions.viewProfileFromPathInZipFile('not-a-profile.json'));
  ...
  it('will fail when not finding a profile', async function () {
    ...
    dispatch(ZippedProfilesActions.viewProfileFromPathInZipFile('nothing-here.json'));

    expect(ZippedProfilesSelectors.getZipFileState(getState()).phase).toEqual('FILE_NOT_FOUND_IN_ZIP_FILE');
  });

  it('can compute a ZipFileTable', async function () {
    const {
      getState
    } = await storeWithZipFile(['foo/bar/profile1.json', 'foo/profile2.json', 'foo/profile3.json', 'foo/profile4.json', 'baz/profile5.json']);

    const zipFileTable = ZippedProfilesSelectors.getZipFileTable(getState());
    expect(formatZipFileTable(zipFileTable)).toEqual(['foo (dir)', '  bar (dir)', '    profile1.json (file)', '  profile2.json (file)', '  profile3.json (file)', '  profile4.json (file)', 'baz (dir)', '  profile5.json (file)']);
  });
...
describe('selected and expanded zip files', function () {
  it('can expand selections in the zip file', function () {
    ...
    expect(ZippedProfilesSelectors.getExpandedZipFileIndexes(getState())).toEqual([]);
    ...
    expect(ZippedProfilesSelectors.getExpandedZipFileIndexes(getState())).toEqual([123, 456, 789]);
  });
  ...
  it('can procure an interesting selection', async function () {
    const {
      dispatch,
      getState
    } = await storeWithZipFile(['a/profile1.json', 'a/profile2.json', 'b/profile3.json', 'b/profile4.json', 'c/profile5.json', 'c/profile6.json', 'd/profile7.json', 'd/profile8.json']);

    const zipFileTree = ZippedProfilesSelectors.getZipFileTree(getState());
    const zipFileTable = ZippedProfilesSelectors.getZipFileTable(getState());

    dispatch(ZippedProfilesActions.changeExpandedZipFile(procureInitialInterestingExpandedNodes(zipFileTree, 1)));

    const expanded = ZippedProfilesSelectors.getExpandedZipFileIndexes(getState());
    const expandedNames = expanded.map(index => zipFileTable.path[ensureExists(index)]);
  ...
  it('can select a zip file', function () {
    ...
    expect(ZippedProfilesSelectors.getSelectedZipFileIndex(getState())).toEqual(null);

    // The indexes don't check that they are valid when you add them.
    dispatch(ZippedProfilesActions.changeSelectedZipFile(123));

    expect(ZippedProfilesSelectors.getSelectedZipFileIndex(getState())).toEqual(123);
  });
});
...
describe('profile state invalidation when switching between profiles', function () {
  const getStoreViewingProfile = async () => {
    const {
      dispatch,
      getState
    } = await storeWithZipFile(['profile1.json', 'profile2.json']);

    const viewProfile = path => dispatch(ZippedProfilesActions.viewProfileFromPathInZipFile(path));
  ...
  it('invalidates profile view state', async function () {
    ...
    expect(ProfileViewSelectors.getPreviewSelection(getState())).toEqual(getNoSelection());

    // Add a selection.
    dispatch(ProfileViewActions.updatePreviewSelection(getSomeSelection()));
    expect(ProfileViewSelectors.getPreviewSelection(getState())).toEqual(getSomeSelection());
    dispatch(ZippedProfilesActions.returnToZipFileList());
    viewProfile('profile2.json');
