...
import { changeSelectedCallNode, changeExpandedCallNodes } from "../../actions/profile-view";
...
describe('doSymbolicateProfile', function () {
  ...
  function init() {
    ...
    return {
      ...
      funcNamesToFuncIndexes: (names: string[]) => names.map(name => {
      ...
      symbolStore: new SymbolStore(symbolStoreName, {
        requestSymbolsFromServer: requests => requests.map(() => Promise.reject(new Error(''))),
        requestSymbolTableFromAddon: async lib => {
          if (lib.debugName === 'firefox.pdb') {
            return exampleSymbolTable;
          }
          throw new SymbolsNotFoundError('Should only have libs called firefox.pdb', lib);
        }
      })
    };
  }
  ...
  describe('doSymbolicateProfile', function () {
    it('can symbolicate a profile', async () => {
      ...
      expect(formatTree(getCallTree(getState()))).toEqual(['- 0x000a (total: 1, self: —)', '  - 0x2000 (total: 1, self: 1)', '- 0x0000 (total: 1, self: —)', '  - 0x2000 (total: 1, self: 1)', '- 0x1a0f (total: 1, self: 1)', '- 0x0f0f (total: 1, self: 1)']);

      await doSymbolicateProfile(dispatch, profile, symbolStore);
      expect(formatTree(getCallTree(getState()))).toEqual([// 0x0000 and 0x000a get merged together.
      '- first symbol (total: 2, self: —)', '  - last symbol (total: 2, self: 2)', '- third symbol (total: 1, self: 1)', '- second symbol (total: 1, self: 1)']);
    });
    ...
    it('updates the symbolication status', async () => {
      ...
      // Starts out as DONE.
      expect(ProfileViewSelectors.getSymbolicationStatus(getState())).toEqual('DONE');
      const symbolication = doSymbolicateProfile(dispatch, profile, symbolStore);
      expect(ProfileViewSelectors.getSymbolicationStatus(getState())).toEqual('SYMBOLICATING');
      await symbolication;
      expect(ProfileViewSelectors.getSymbolicationStatus(getState())).toEqual('DONE');
    });
  });
  ...
  describe('merging of functions with different memory addresses, but in the same function', () => {
    it('starts with expanded call nodes of multiple memory addresses', async function () {
      ...
      const expandedCallNodePaths = [['0x000a'], ['0x0000']].map(funcNamesToFuncIndexes);
      ...
      assertSetContainsOnly(getExpandedCallNodePaths(getState()), expandedCallNodePaths);
    });
    ...
    it('symbolicates and merges functions in the stored call node paths', async function () {
      ...
      await doSymbolicateProfile(dispatch, profile, symbolStore);
      expect(getSelectedCallNodePath(getState())).toEqual( // The CallNodePath is now symbolicated.
      funcNamesToFuncIndexes(['first symbol', 'last symbol']));

      assertSetContainsOnly(getExpandedCallNodePaths(getState()), [['first symbol']].map(funcNamesToFuncIndexes));
    });
  });
});
...
function _createUnsymbolicatedProfile() {
  const {
    profile
  } = getProfileFromTextSamples( // "0x000a" and "0x0000" are both in the first symbol, and should be merged.
