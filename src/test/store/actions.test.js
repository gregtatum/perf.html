...
describe('selectors/getCallNodeSamplesByDepthForFlameGraph', function() {
  /**
   * Map the callNodeSamplesByDepth data structure into a human readable format where
   * each line takes the form:
   *
   * "FunctionName1 (StartTime:EndTime) | FunctionName2 (StartTime:EndTime)"
   */
  function getHumanReadableCallNodeSamplesByDepth(store, funcNames) {
    ...
    return callNodeSamplesByDepth.map(({ callNode, end, length, start }) => {
      const lines = [];
  ...
  it('computes a basic example', function() {
    ...
    expect(getHumanReadableCallNodeSamplesByDepth(store, funcNames)).toEqual([
      'A (0:3)',
      'B (0:3)',
      'C (0:2) | H (2:3)',
      'D (0:1) | F (1:2) | I (2:3)',
