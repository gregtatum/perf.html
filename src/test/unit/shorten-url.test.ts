...
class Response {
  ...
  constructor(body: string | null, options: {
    status: number;
    statusText?: string;
    headers?: {};
  }) {
...
function mockFetchForBitly({
  ...
  endpointUrl: string;
  responseFromRequestPayload: (arg0: any) => Response;
}) {
...
describe('shortenUrl', () => {
  ...
  it('calls the bitly API properly and returns the value', async () => {
    ...
    const longUrl = 'https://profiler.firefox.com/public/FAKE_HASH/calltree/?thread=1&v=3';
    const shortUrl = await shortenUrl(longUrl);

    expect(shortUrl).toBe(expectedShortUrl);
    expect(window.fetch).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
  ...
  it('changes the requested url if is not the main URL', async () => {
    const bitlyHash = 'BITLYHASH';
    const expectedShortUrl = `https://perfht.ml/${bitlyHash}`;
    const longUrl = 'https://perf-html.io/public/FAKE_HASH/calltree/?thread=1&v=3';
    const expectedLongUrl = longUrl.replace('perf-html.io', 'profiler.firefox.com');
...
describe('expandUrl', () => {
  ...
  it('returns the long url returned by the API', async () => {
    ...
    const returnedLongUrl = 'https://profiler.firefox.com/public/FAKE_HASH/calltree/?thread=1&v=3';
  ...
  it('forwards errors', async () => {
    window.fetch.mockImplementation(async () => new Response(null, {
      status: 503
    }));
