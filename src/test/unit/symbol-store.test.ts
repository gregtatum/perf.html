...
describe('SymbolStore', function () {
  ...
  function deleteDatabase() {
    return new Promise((resolve, reject) => {
      const req = fakeIndexedDB.deleteDatabase('profiler-async-storage-symbol-tables');
  ...
  it('should only request symbols from the symbol provider once per library', async function () {
    ...
    symbolProvider = {
      requestSymbolsFromServer: jest.fn(requests => requests.map(request => {
        expect(request.lib.breakpadId).not.toBe('');
        return Promise.reject(new Error('this example only supports symbol tables'));
      })),
      requestSymbolTableFromAddon: jest.fn(() => Promise.resolve(exampleSymbolTable))
    };
    ...
    await symbolStore.getSymbols([{ lib: lib1, addresses: new Set([0xf01, 0x1a50]) }], (request, results) => {
    ...
    await symbolStore.getSymbols([{ lib: lib2, addresses: new Set([0x33, 0x2000]) }], (request, results) => {
    ...
    await symbolStore.getSymbols([{ lib: libWithEmptyBreakpadId, addresses: new Set([0x33, 0x2000]) }], (_request, _results) => {}, (_request, _error) => {});
    expect(symbolProvider.requestSymbolsFromServer).toHaveBeenCalledTimes(2);
    expect(symbolProvider.requestSymbolTableFromAddon).toHaveBeenCalledTimes(2);
  ...
  it('should persist in DB', async function () {
    jest.spyOn(console, 'log').mockImplementation(() => {});
    symbolProvider = {
      requestSymbolsFromServer: jest.fn(requests => requests.map(() => Promise.reject(new Error('this example only supports symbol tables')))),
      requestSymbolTableFromAddon: jest.fn(() => Promise.resolve(exampleSymbolTable))
    };
    symbolStore = new SymbolStore('profiler-async-storage', symbolProvider);

    const lib = { debugName: 'firefox', breakpadId: 'dont-care' };
    await symbolStore.getSymbols([{ lib: lib, addresses: new Set([0]) }], (_request, _results) => {}, (_request, _error) => {});
    ...
    await symbolStore.getSymbols([{ lib: lib, addresses: new Set([0x1]) }], (_request, _results) => {}, (_request, _error) => {});
  ...
  it('should call requestSymbolsFromServer first', async function () {
    ...
    const fakeSymbolStore = new FakeSymbolStore(new Map([['available-for-addresses', symbolTable]]));
    ...
    symbolProvider = {
      requestSymbolsFromServer: jest.fn(requests => {
        ...
        return requests.map(request => new Promise((resolve, reject) => {
          fakeSymbolStore.getSymbols([request], (_request, results) => resolve(results), (_request, error) => reject(error));
        }));
      }),
      requestSymbolTableFromAddon: jest.fn().mockResolvedValue(exampleSymbolTable)
    };
    symbolStore = new SymbolStore('profiler-async-storage', symbolProvider);
    ...
    await symbolStore.getSymbols([{ lib: lib1, addresses: new Set([0xf01, 0x1a50]) }, { lib: lib2, addresses: new Set([0x33, 0x2000]) }], (request, results) => {
    ...
    // Due to https://github.com/dumbmatter/fakeIndexedDB/issues/22 we need to
    // take care to sequence the DB open requests.
    const symbolStore2 = new SymbolStore('profiler-async-storage', symbolProvider);

    await symbolStore2.getSymbols([{ lib: lib1, addresses: new Set([0xf01, 0x1a50]) }, { lib: lib2, addresses: new Set([0x33, 0x2000]) }], (_request, _results) => {}, (_request, _error) => {});
