...
import { getCallNodeInfo, filterThreadByImplementation, getCallNodePathFromIndex, getSampleIndexClosestToTime, convertStackToCallNodePath, getSampleIndexToCallNodeIndex, getCallNodeIndexFromPath, getTreeOrderComparator, getSamplesSelectedStates } from "../../profile-logic/profile-data";
import { resourceTypes } from "../../profile-logic/data-structures";
import { createGeckoProfile, createGeckoProfileWithJsTimings, createGeckoSubprocessProfile } from ".././fixtures/profiles/gecko-profile";
...
import { getProfileFromTextSamples, getJsTracerTable } from "../fixtures/profiles/processed-profile";
import { funcHasRecursiveCall } from "../../profile-logic/transforms";

import { Thread, IndexIntoStackTable } from "../../types/profile";
...
describe('process-profile', function () {
  describe('processProfile', function () {
    ...
    it('should shift the content process by 1 second', function () {
      ...
      expect(thread2.markers.data[6] && thread2.markers.data[6].type === 'tracing' && thread2.markers.data[6].category === 'DOMEvent' ? thread2.markers.data[6].timeStamp : null).toEqual(1001);
      expect(thread2.markers.data[7] && thread2.markers.data[7].type === 'tracing' && thread2.markers.data[7].category === 'DOMEvent' ? thread2.markers.data[7].timeStamp : null).toEqual(1001);
    ...
    it('should create one function per frame', function () {
      ...
      expect(thread.stringTable.getString(chromeStringIndex)).toEqual('chrome://blargh');
    ...
    it('should create one resource per used library', function () {
      ...
      expect(thread.stringTable.getString(name0)).toEqual('Extension "Form Autofill" (ID: formautofill@mozilla.org)');
  ...
  describe('JS tracer', function () {
    ...
    it('processes JS tracer and offsets the timestamps', function () {
...

      {
        ...
        const jsTracer = getJsTracerTable(stringTable, [['jsTracerA', 0, 10], ['jsTracerB', 1, 9], ['jsTracerC', 2, 8]]);
        ...
        timestampOffsetMs = geckoSubprocess.meta.startTime - geckoProfile.meta.startTime;
      ...
      const childProcessThread = ensureExists(processedProfile.threads.find(thread => thread.jsTracer), 'Could not find the thread with the JS tracer information');
      const processedJsTracer = ensureExists(childProcessThread.jsTracer, 'The JS tracer table was not found on the subprocess');

      // Check that the values are correct from the test defined data.
      expect(processedJsTracer.events.map(index => childProcessThread.stringTable.getString(index))).toEqual(['jsTracerA', 'jsTracerB', 'jsTracerC']);
      expect(processedJsTracer.durations).toEqual([10000, 8000, 6000]);
      expect(processedJsTracer.timestamps).toEqual([0 + timestampOffsetMicro, 1000 + timestampOffsetMicro, 2000 + timestampOffsetMicro]);
    });
  });
  ...
  describe('extensions metadata', function () {
    it('should be processed correctly', function () {
      ...
      geckoProfile.meta.extensions = {
        ...
        data: [['geckoprofiler@mozilla.com', 'Gecko Profiler', 'moz-extension://bf3bb73c-919c-4fef-95c4-070a19fdaf85/'], ['screenshots@mozilla.org', 'Firefox Screenshots', 'moz-extension://fa2edf9c-c45f-4445-b819-c09e3f2d58d5/']]
      };

      const profile = processProfile(geckoProfile);
      expect(profile.meta.extensions).toEqual({
        baseURL: ['moz-extension://bf3bb73c-919c-4fef-95c4-070a19fdaf85/', 'moz-extension://fa2edf9c-c45f-4445-b819-c09e3f2d58d5/'],
...
describe('profile-data', function () {
  describe('createCallNodeTableAndFixupSamples', function () {
    const profile = processProfile(createGeckoProfile());
    const defaultCategory = profile.meta.categories.findIndex(c => c.name === 'Other');
    const thread = profile.threads[0];
    const {
      callNodeTable
    } = getCallNodeInfo(thread.stackTable, thread.frameTable, thread.funcTable, defaultCategory);
  ...
  function _getStackList(thread: Thread, stackIndex: IndexIntoStackTable | null) {
  ...
  describe('getCallNodeInfo', function () {
    ...
    const mergedFuncListA = getCallNodePathFromIndex(stackIndexToCallNodeIndex[stack0], callNodeTable);
    const mergedFuncListB = getCallNodePathFromIndex(stackIndexToCallNodeIndex[stack1], callNodeTable);
...
describe('symbolication', function () {
  describe('getContainingLibrary', function () {
    const libs = [{ start: 0, end: 20, name: 'first' }, { start: 20, end: 40, name: 'second' }, { start: 40, end: 50, name: 'third' }, { start: 60, end: 80, name: 'fourth' }, { start: 80, end: 100, name: 'fifth' }].map(lib => {
  ...
  describe('symbolicateProfile', function () {
    ...
    beforeAll(function () {
      ...
      const symbolStore = new FakeSymbolStore(new Map([['firefox', symbolTable], ['firefox-webcontent', symbolTable]]));
      symbolicatedProfile = Object.assign({}, unsymbolicatedProfile, {
        threads: unsymbolicatedProfile.threads.slice()
      });
      const symbolicationPromise = symbolicateProfile(unsymbolicatedProfile, symbolStore, {
    ...
    it('should assign correct symbols to frames', function () {
      ...
      expect(functionNameForFrameInThread(unsymbolicatedThread, 1)).toEqual('0x100000f84');
      expect(functionNameForFrameInThread(symbolicatedThread, 1)).toEqual('second symbol');
      expect(functionNameForFrameInThread(unsymbolicatedThread, 2)).toEqual('0x100001a45');
      expect(functionNameForFrameInThread(symbolicatedThread, 2)).toEqual('third symbol');
    });
  });
...
describe('filter-by-implementation', function () {
  ...
  it('will return the same thread if filtering to "all"', function () {
    expect(filterThreadByImplementation(thread, 'combined', defaultCategory)).toEqual(thread);
  });

  it('will return only JS samples if filtering to "js"', function () {
    const jsOnlyThread = filterThreadByImplementation(thread, 'js', defaultCategory);
    const nonNullSampleStacks = jsOnlyThread.samples.stack.filter(stack => stack !== null);
    const samplesAreAllJS = nonNullSampleStacks.map(stack => stackIsJS(jsOnlyThread, stack)).reduce((a, b) => a && b);
  ...
  it('will return only C++ samples if filtering to "cpp"', function () {
    const cppOnlyThread = filterThreadByImplementation(thread, 'cpp', defaultCategory);
    const nonNullSampleStacks = cppOnlyThread.samples.stack.filter(stack => stack !== null);
    const samplesAreAllJS = nonNullSampleStacks.map(stack => !stackIsJS(cppOnlyThread, stack)).reduce((a, b) => a && b);
...
describe('get-sample-index-closest-to-time', function () {
  it('returns the correct sample index for a provided time', function () {
    ...
    const {
      samples
    } = filterThreadByImplementation(thread, 'js', defaultCategory);
...
describe('funcHasRecursiveCall', function () {
  ...
  it('correctly identifies recursive functions based taking into account implementation', function () {
    expect([funcHasRecursiveCall(thread, 'combined', funcNames.indexOf('A.js')), funcHasRecursiveCall(thread, 'combined', funcNames.indexOf('B.js')), funcHasRecursiveCall(thread, 'js', funcNames.indexOf('B.js'))]).toEqual([false, false, true]);
  });
});
...
describe('getSamplesSelectedStates', function () {
  ...
  const {
    callNodeTable,
    stackIndexToCallNodeIndex
  } = getCallNodeInfo(thread.stackTable, thread.frameTable, thread.funcTable, 0);
  ...
  it('determines the selection status of all the samples', function () {
    expect(getSamplesSelectedStates(callNodeTable, sampleCallNodes, sampleCallNodes, A_B)).toEqual(['SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED', 'SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED']);
    expect(getSamplesSelectedStates(callNodeTable, sampleCallNodes, sampleCallNodes, A_D)).toEqual(['UNSELECTED_ORDERED_BEFORE_SELECTED', 'SELECTED', 'UNSELECTED_ORDERED_BEFORE_SELECTED', 'SELECTED', 'SELECTED']);
    expect(getSamplesSelectedStates(callNodeTable, sampleCallNodes, sampleCallNodes, A_B_F)).toEqual(['UNSELECTED_ORDERED_BEFORE_SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED', 'SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED']);
    expect(getSamplesSelectedStates(callNodeTable, sampleCallNodes, sampleCallNodes, A_D_E)).toEqual(['UNSELECTED_ORDERED_BEFORE_SELECTED', 'SELECTED', 'UNSELECTED_ORDERED_BEFORE_SELECTED', 'UNSELECTED_ORDERED_AFTER_SELECTED', 'UNSELECTED_ORDERED_BEFORE_SELECTED']);
  });
