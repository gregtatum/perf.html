...
describe('SymbolStoreDB', function () {
  ...
  it('should respect the maximum number of tables limit', async function () {
    ...
    for (let i = 0; i < 5; i++) {
      await expect(symbolStoreDB.getSymbolTable(libs[i].debugName, libs[i].breakpadId)).rejects.toBeInstanceOf(Error);
    ...
    for (let i = 5; i < 10; i++) {
      ...
      await expect(symbolStoreDB.getSymbolTable(libs[i].debugName, libs[i].breakpadId)).resolves.toBeInstanceOf(Array);
    }
