...
import { stripFunctionArguments, removeTemplateInformation, getFunctionName } from "../../profile-logic/function-info";
...
describe('strip-function-arguments', function () {
  ...
  it('should remove also the storage class', function () {
    expect(stripFunctionArguments('static nsThread::ThreadFunc(void*)')).toEqual('nsThread::ThreadFunc');
  });
});

describe('remove-template-information', function () {
  it('should remove template information', function () {
    expect(removeTemplateInformation('ns::Impl<void (ns::foo::*)(), (ns::bar)0>::fn')).toEqual('ns::Impl::fn');
    expect(removeTemplateInformation('fn<ns::foo<ns::bar<a::b> > >')).toEqual('fn');
  });
...
describe('get-function-name', function () {
  it('should get the function name', function () {
    expect(getFunctionName('static ns::Foo<0>::fn(bool (*)(JS::Handle<JSObject*>)) const')).toEqual('ns::Foo::fn');
  });
});