...
import { getProfileFromTextSamples, getMergedProfileFromTextSamples } from "../fixtures/profiles/processed-profile";
import { getCallTree, computeCallTreeCountsAndTimings, CallTree } from "../../profile-logic/call-tree";
import { getRootsAndChildren } from "../../profile-logic/flame-graph";
import { getCallNodeInfo, invertCallstack, getCallNodeIndexFromPath, getOriginAnnotationForFunc, filterThreadSamplesToRange } from "../../profile-logic/profile-data";
...
function callTreeFromProfile(profile: Profile, threadIndex: number = 0): CallTree {
  ...
  const callNodeInfo = getCallNodeInfo(thread.stackTable, thread.frameTable, thread.funcTable, defaultCategory);
  const callTreeCountsAndTimings = computeCallTreeCountsAndTimings(thread.samples, callNodeInfo, interval, false);
  return getCallTree(thread, interval, callNodeInfo, categories, 'combined', callTreeCountsAndTimings, 'timing');
}
...
describe('unfiltered call tree', function () {
  ...
  describe('computed counts and timings', function () {
    ...
    it('yields expected results', function () {
      expect(computeCallTreeCountsAndTimings(thread.samples, callNodeInfo, profile.meta.interval, false)).toEqual({
        rootCount: 1,
        rootTotalTime: 3,
  ...
  describe('roots and children for flame graph', function () {
    ...
    it('returns roots and children', function () {
      expect(getRootsAndChildren(thread, callNodeInfo.callNodeTable, new Uint32Array([1, 2, 2, 1, 0, 1, 0, 1, 0]), new Float32Array([3, 3, 2, 1, 1, 1, 1, 1, 1]))).toEqual({
        roots: [0],
        children: {
  ...
  describe('computed structure', function () {
    ...
    it('computes an unfiltered call tree', function () {
      expect(formatTree(callTree)).toEqual(['- A (total: 3, self: —)', '  - B (total: 3, self: —)', '    - C (total: 2, self: —)', '      - D (total: 1, self: —)', '        - E (total: 1, self: 1)', '      - F (total: 1, self: —)', '        - G (total: 1, self: 1)', '    - H (total: 1, self: —)', '      - I (total: 1, self: 1)']);
    });
  });
  ...
  describe('computed structure with categories', function () {
    ...
    const callTree = callTreeFromProfile(profile);
    it('computes an unfiltered call tree', function () {
      expect(formatTreeIncludeCategories(callTree)).toEqual(['- A [Other] (total: 3, self: —)', '  - B [DOM] (total: 3, self: —)', '    - C [DOM] (total: 2, self: —)', '      - D [DOM] (total: 1, self: —)', '        - E [DOM] (total: 1, self: 1)', '      - F [Graphics] (total: 1, self: —)', '        - G [Graphics] (total: 1, self: 1)', '    - H [DOM] (total: 1, self: —)', '      - I [Other] (total: 1, self: 1)']);
    });
  });
  ...
  describe('CallTree methods', function () {
    ...
    describe('hasSameNodeIds()', function () {
      it('determines if the node IDs are the same between two trees', function () {
        ...
        const otherTree = callTreeFromProfile(getProfileFromTextSamples('A').profile);
    ...
    describe('icons from the call tree', function () {
      it('upgrades http to https', function () {
        ...
        const callTree = callTreeFromProfile(profile);
        const [thread] = profile.threads;
        const hostStringIndex = thread.stringTable.indexForString('examplecom.js');
        ...
        expect(callTree.getDisplayData(A).icon).toEqual('https://example.com/favicon.ico');
      });
    });
  });
  ...
  describe('getCallNodeIndexFromPath', function () {
    ...
    function checkStack(callNodePath, index, name) {
      it(`finds stack that ends in ${name}`, function () {
        expect(getCallNodeIndexFromPath(callNodePath, callNodeTable)).toBe(index);
      });
    }
    ...
    it(`doesn't find a non-existent stack`, function () {
      expect(getCallNodeIndexFromPath([A, B, C, D, E, F, G], callNodeTable)).toBe(null);
    });
  });
});
...
describe('inverted call tree', function () {
  ...
  describe('computed structure', function () {
    ...
    const callNodeInfo = getCallNodeInfo(thread.stackTable, thread.frameTable, thread.funcTable, defaultCategory);
    const callTreeCountsAndTimings = computeCallTreeCountsAndTimings(thread.samples, callNodeInfo, interval, true);
    const callTree = getCallTree(thread, interval, callNodeInfo, categories, 'combined', callTreeCountsAndTimings, 'timing');

    it('computes an non-inverted call tree', function () {
      expect(formatTreeIncludeCategories(callTree)).toEqual(['- A [Other] (total: 3, self: 3)', '  - B [DOM] (total: 3, self: —)', '    - C [Graphics] (total: 2, self: —)', '      - D [Other] (total: 1, self: —)', '        - E [Other] (total: 1, self: —)', '      - X [Graphics] (total: 1, self: —)', '        - Y [Graphics] (total: 1, self: —)', '          - Z [Graphics] (total: 1, self: —)', '    - X [DOM] (total: 1, self: —)', '      - Y [DOM] (total: 1, self: —)', '        - Z [DOM] (total: 1, self: —)']);
    });

    // Now compute the inverted tree and check it.
    const invertedThread = invertCallstack(thread, defaultCategory);
    const invertedCallNodeInfo = getCallNodeInfo(invertedThread.stackTable, invertedThread.frameTable, invertedThread.funcTable, defaultCategory);
    const invertedCallTreeCountsAndTimings = computeCallTreeCountsAndTimings(invertedThread.samples, invertedCallNodeInfo, interval, true);
    const invertedCallTree = getCallTree(invertedThread, interval, invertedCallNodeInfo, categories, 'combined', invertedCallTreeCountsAndTimings, 'timing');
    ...
    it('computes an inverted call tree', function () {
      expect(formatTreeIncludeCategories(invertedCallTree)).toEqual(['- Z [Other] (total: 2, self: 2)', '  - Y [Other] (total: 2, self: —)', '    - X [Other] (total: 2, self: —)', '      - B [DOM] (total: 1, self: —)', '        - A [Other] (total: 1, self: —)', '      - C [Graphics] (total: 1, self: —)', '        - B [DOM] (total: 1, self: —)', '          - A [Other] (total: 1, self: —)', '- E [Other] (total: 1, self: 1)', '  - D [Other] (total: 1, self: —)', '    - C [Graphics] (total: 1, self: —)', '      - B [DOM] (total: 1, self: —)', '        - A [Other] (total: 1, self: —)']);
    });
  });
});
...
describe('diffing trees', function () {
  ...
  it('displays a proper call tree, including nodes with totalTime = 0', () => {
    ...
    expect(formattedTree).toEqual(['- A (total: —, self: —)', '  - B (total: 1, self: —)', '    - D (total: -1, self: -1)', '    - G (total: 1, self: 1)', '    - I (total: 1, self: 1)', '  - C (total: -1, self: —)', '    - F (total: -1, self: -1)']);
  ...
  it('displays a proper call tree, even for range-filtered threads', () => {
    ...
    const rangeStart = 1;
    const rangeEnd = 3;
    profile.threads = profile.threads.map(thread => filterThreadSamplesToRange(thread, rangeStart, rangeEnd));
    const callTree = callTreeFromProfile(profile,
    /* threadIndex */
    2);
    const formattedTree = formatTree(callTree);
    expect(formattedTree).toEqual([// A -> B -> D and A -> B -> G should be filtered out by the range filtering.
    '- A (total: —, self: —)', '  - B (total: 1, self: —)', '    - I (total: 1, self: 1)', '  - C (total: -1, self: —)', '    - F (total: -1, self: -1)']);
  });
...
describe('origin annotation', function () {
  ...
  function addResource(funcName: string, name: string, host: string | null, location: string | null) {
    ...
    thread.funcTable.fileName[funcIndex] = location ? thread.stringTable.indexForString(location) : null;
    thread.resourceTable.lib.push(-1);
    thread.resourceTable.name.push(thread.stringTable.indexForString(name));
    thread.resourceTable.host.push(host ? thread.stringTable.indexForString(host) : undefined);
    thread.resourceTable.length++;
  }

  addResource('A', 'http://foobar.com', 'http://foobar.com', 'http://foobar.com/script.js');

  addResource('B', 'Extension "Gecko Profiler"', 'moz-extension://bf3bb73c-919c-4fef-95c4-070a19fdaf85', 'moz-extension://bf3bb73c-919c-4fef-95c4-070a19fdaf85/script.js');
  ...
  function getOrigin(funcName: string): string {
    return getOriginAnnotationForFunc(funcNames.indexOf(funcName), thread.funcTable, thread.resourceTable, thread.stringTable);
  }
  ...
  it('formats extension origins correctly', function () {
    expect(getOrigin('B')).toEqual('Extension "Gecko Profiler": ' + 'moz-extension://bf3bb73c-919c-4fef-95c4-070a19fdaf85/script.js');
  });

  it('formats library origins correctly', function () {
    expect(getOrigin('C')).toEqual('libxul.so: /home/user/mozilla-central/xul.cpp');
