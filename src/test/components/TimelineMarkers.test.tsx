...
import { TimelineMarkersOverview, MIN_MARKER_WIDTH } from "../../components/timeline/Markers";
...
import { getBoundingBox, getMouseEvent, addRootOverlayElement, removeRootOverlayElement } from "../fixtures/utils";
...
function setupWithMarkers({
  ...
  const renderResult = render(<Provider store={storeWithProfile(profile)}>
      <>
        <TimelineMarkersOverview rangeStart={rangeStart} rangeEnd={rangeEnd} threadIndex={0} onSelect={() => {}} />
        <MarkerContextMenu />
      </>
    </Provider>);
  ...
  function getContextMenu() {
    return ensureExists(renderResult.container.querySelector('.react-contextmenu'), `Couldn't find the context menu.`);
  }
  ...
  function fireMouseEvent(eventName, options) {
    fireEvent(ensureExists(renderResult.container.querySelector('canvas'), `Couldn't find the canvas element`), getMouseEvent(eventName, options));
  }
...
describe('TimelineMarkers', function () {
  it('renders correctly', () => {
    ...
    const {
      container,
      flushDrawLog
    } = setupWithMarkers({ rangeStart: 0, rangeEnd: 15 }, [['Marker A', 0, { startTime: 0, endTime: 10 }], ['Marker B', 0, { startTime: 0, endTime: 10 }], ['Marker C', 5, { startTime: 5, endTime: 15 }], ['BHR-detected hang', 5, { type: 'BHR-detected hang', startTime: 2, endTime: 13 }], ['LongTask', 6, {
  ...
  it('does not render several dot markers in the same position', () => {
    ...
    const {
      flushDrawLog
    } = setupWithMarkers({ rangeStart: 0, rangeEnd: 15000 }, [// 2 very close dot markers. They shouldn't be drawn both together.
    ['Marker A', 5000, null], ['Marker B', 5001, null], // This is a longer marker starting at the same place, it should always be drawn
    ['Marker C', 5001, { startTime: 5001, endTime: 7000 }]]);

    const drawCalls = flushDrawLog();

    // We filter on height to get only 1 relevant fillRect operation for each marker.
    const fillRectOperations = drawCalls.filter(([operation,,,, height]) => operation === 'fillRect' && height > 1);

    // Here 2 markers should be drawn: the first dot, and the long marker.
    expect(fillRectOperations).toHaveLength(2);
    expect(fillRectOperations.every(([,,, width]) => width >= MIN_MARKER_WIDTH / window.devicePixelRatio)).toBe(true);
  ...
  describe('displays context menus', () => {
    ...
    it('when right clicking on a marker', () => {
      const {
        ...
        getContextMenu,
        clickOnMenuItem
      } = setupWithMarkers({ rangeStart: 0, rangeEnd: 10 }, [['Marker A', 0, { startTime: 0, endTime: 10 }]]);
    ...
    it('when right clicking on markers in a sequence', () => {
      const {
        ...
        getContextMenu,
        clickOnMenuItem
      } = setupWithMarkers({ rangeStart: 0, rangeEnd: 10 }, [['Marker A', 0, { startTime: 0, endTime: 3 }], ['Marker B', 0, { startTime: 6, endTime: 10 }]]);
    ...
    it('and still highlights other markers when hovering them', () => {
      ...
      const callsWithHoveredColor = drawCalls.filter(([, argument]) => argument === overlayFills.HOVERED);
