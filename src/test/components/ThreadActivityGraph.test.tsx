...
describe('ThreadActivityGraph', function () {
  ...
  function setup(profile: Profile = getSamplesProfile()) {
    ...
    const activityGraphCanvas = ensureExists(container.querySelector('.threadActivityGraphCanvas'), `Couldn't find the activity graph canvas, with selector .threadActivityGraphCanvas`);
    const thread = profile.threads[0];

    // Perform a click on the activity graph.
    function clickActivityGraph(index: IndexIntoSamplesTable, graphHeightPercentage: number) {
      fireEvent(activityGraphCanvas, getMouseEvent('mouseup', {
    ...
    function getCallNodePath() {
      return selectedThreadSelectors.getSelectedCallNodePath(getState()).map(funcIndex => thread.stringTable.getString(thread.funcTable.name[funcIndex]));
    }
  ...
  describe('ThreadActivityGraph', function () {
    ...
    it('will redraw even when there are no samples in range', function () {
      ...
      expect(drawCalls.map(([fn]) => fn)).toContain('set globalCompositeOperation');
    });
  });
});