...
import { addMarkersToThreadWithCorrespondingSamples, getProfileFromTextSamples, getNetworkMarkers, getProfileWithMarkers } from "../fixtures/profiles/processed-profile";
...
describe('TooltipMarker', function () {
  it('renders tooltips for various markers', () => {
    ...
    addMarkersToThreadWithCorrespondingSamples(profile.threads[0], [['DOMEvent', 10.5, {
      type: 'tracing',
      category: 'DOMEvent',
    ...
    }], ['Bailout_ShapeGuard after getelem on line 3666 of resource://foo.js -> resource://bar.js:3662', 10, null], ['Invalidate http://mozilla.com/script.js:1234', 10, null], ['Styles', 18.5, {
      type: 'tracing',
      category: 'Paint',
      interval: 'start',
      cause: {
    ...
    }], ['ConstructRootFrame', 112.5, {
      type: 'tracing',
      category: 'Frame Construction',
      interval: 'start'
    }], ['ConstructRootFrame', 113.3, {
      type: 'tracing',
      category: 'Frame Construction',
      interval: 'end'
    }], ['FileIO', 114, {
    ...
    const markerIndexes = selectedThreadSelectors.getFullMarkerListIndexes(state);
    ...
    markerIndexes.forEach(markerIndex => {
      ...
      const {
        container
          ...
          <TooltipMarker marker={marker} threadIndex={threadIndex} className="propClass" />
        </Provider>);

      expect(container.firstChild).toMatchSnapshot( // Markers are ordered by start time, but for markers with the same start
  ...
  it('renders properly redirect network markers', () => {
    const {
      ...
      payload: {
        ...
        pri: -20,
        count: 0,
        RedirectURI: 'http://img.buzzfeed.com/buzzfeed-static/static/2018-04/29/11/tmp/buzzfeed-prod-web-02/tmp-name-2-18011-1525016782-0_dblwide.jpg?output-format=auto&output-quality=auto&resize=625'
      }
    }));
