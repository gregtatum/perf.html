...
import { getProfileFromTextSamples, getCounterForThread } from "../fixtures/profiles/processed-profile";
...
describe('TrackMemory', function () {
  function setup() {
    const {
      profile
    } = getProfileFromTextSamples(Array(SAMPLE_COUNT).fill('A').join('  '));
    const threadIndex = 0;
    ...
    const canvas = ensureExists(container.querySelector('.timelineTrackMemoryCanvas'), `Couldn't find the memory canvas, with selector .timelineTrackMemoryCanvas`);
    const getTooltipContents = () => document.querySelector('.timelineTrackMemoryTooltip');
    const getMemoryDot = () => container.querySelector('.timelineTrackMemoryGraphDot');
