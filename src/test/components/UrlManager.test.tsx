...
describe('UrlManager', function () {
  function setup(urlPath: string | null | undefined) {
    (receiveProfile as any).doSymbolicateProfile = jest.fn(() => async () => {});
    ...
    const createUrlManager = () => render(<Provider store={store}>
          <UrlManager>Contents</UrlManager>
        </Provider>);

    const waitUntilUrlSetupPhase = phase => waitUntilState(store, state => getUrlSetupPhase(state) === phase);
