...
import { changeNetworkSearchString, commitRange, updatePreviewSelection } from "../../actions/profile-view";
...
import { getProfileWithMarkers, getNetworkMarkers, TestDefinedMarkers } from "../fixtures/profiles/processed-profile";
import { getBoundingBox, addRootOverlayElement, removeRootOverlayElement, getMouseEvent } from "../fixtures/utils";
...
function setupWithProfile(profile) {
  ...
  jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(() => // We're adding the timeline margin to try to get some round numbers in
  // the tests.
  ...
  function getUrlShorteningParts(): Array<[string, string]> {
    return Array.from(container.querySelectorAll('.networkChartRowItemLabel span')).map(node => [node.className, node.textContent]);
  }

  const getBarElements = () => Array.from(container.querySelectorAll('.networkChartRowItemBar'));

  const getBarElementStyles = () => getBarElements().map(element => element.getAttribute('style'));

  const getPhaseElements = () => Array.from(container.querySelectorAll('.networkChartRowItemBarPhase'));

  const getPhaseElementStyles = () => getPhaseElements().map(element => element.getAttribute('style'));

  function rowItem() {
    return ensureExists(container.querySelector('.networkChartRowItem'), `Couldn't find the row item in the network chart, with selector .networkChartRowItem`);
  }
...
describe('NetworkChart', function () {
  it('renders NetworkChart correctly', () => {
    const {
      flushDrawLog,
      container
    } = setupWithPayload([...NETWORK_MARKERS]);
...
describe('NetworkChartRowBar phase calculations', function () {
  it('divides up the different phases of the request with full set of required information', () => {
    ...
    expect(getBarElementStyles()[0]).toEqual(`width: 198px; left: ${TIMELINE_MARGIN_LEFT}px;`);
    // The sum of widths should equal the width above.
    expect(getPhaseElementStyles()).toEqual(['left: 0px; width: 20px; opacity: 0;', 'left: 20px; width: 20px; opacity: 0.3333333333333333;', 'left: 40px; width: 60px; opacity: 0.6666666666666666;', 'left: 100px; width: 40px; opacity: 1;', 'left: 140px; width: 58px; opacity: 0;']);
  });
  ...
  it('displays properly a network marker even when it crosses the boundary', () => {
    ...
    // The sum of widths should equal the width above.
    expect(getPhaseElementStyles()).toEqual(['left: 0px; width: 50px; opacity: 0;', 'left: 50px; width: 50px; opacity: 0.3333333333333333;', 'left: 100px; width: 150px; opacity: 0.6666666666666666;', // The actual value has a float rounding error, using a regexp accounts for this.
    expect.stringMatching(/^left: 250\.\d*?px; width: 100px; opacity: 1;$/), 'left: 350px; width: 145px; opacity: 0;']);
  });
  ...
  it('renders according to the preview selection', () => {
    ...
    // dedicated line.
    dispatch(updatePreviewSelection({
      hasSelection: true,
      isModifying: false,
      selectionStart: 20,
      selectionEnd: 40
    ...
    expect(firstMarkerLeft + firstMarkerWidth).toBeGreaterThanOrEqual(TIMELINE_MARGIN_LEFT);
  ...
  it('divides up the different phases of the request with subset of required information', () => {
    ...
    expect(getPhaseElementStyles()).toEqual(['left: 0px; width: 20px; opacity: 0;', 'left: 20px; width: 80px; opacity: 0.6666666666666666;', 'left: 100px; width: 40px; opacity: 1;', 'left: 140px; width: 58px; opacity: 0;']);
  });
  ...
  it('takes the full width when there is no details in the payload', () => {
    ...
    expect(getPhaseElementStyles()).toEqual(['left: 0px; width: 198px; opacity: 1;']);
  });

  it('divides the phases when only the start marker is present', () => {
    const markerForProfileRange = ['Some Marker', 0, // With an endTime at 99, the profile's end time is 100 which gives
    ...
    const {
      getPhaseElementStyles
    } = setupWithPayload([markerForProfileRange, startMarker]);

    expect(getPhaseElementStyles()).toEqual([// The marker goes to the end of the profile range.
  ...
  it('renders 2 bars for a network marker with a preconnect part', () => {
    ...
    expect(barStyles).toEqual(['width: 40px; left: -18870px;', 'width: 198px; left: 150px;']);
  });
...
describe('NetworkChartRowBar URL split', function () {
  ...
  it('splits up the url by protocol / domain / path / filename / params / hash', function () {
    const {
      getUrlShorteningParts
    } = setupForUrl('https://test.mozilla.org/img/optimized/test.gif?param1=123&param2=321#hashNode2');
    expect(getUrlShorteningParts()).toEqual([// Then assert that it's broken up as expected
    ['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'test.mozilla.org'], ['networkChartRowItemUriOptional', '/img/optimized'], ['networkChartRowItemUriRequired', '/test.gif'], ['networkChartRowItemUriOptional', '?param1=123&param2=321'], ['networkChartRowItemUriOptional', '#hashNode2']]);
  });
  ...
  it('splits properly a url without a path', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriRequired', '/']]);
  });
  ...
  it('splits properly a url without a directory', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriRequired', '/index.html']]);
  });
  ...
  it('splits properly a url without a filename', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriRequired', '/analytics/']]);
  });
  ...
  it('splits properly a url without a filename and a long directory', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriOptional', '/assets'], ['networkChartRowItemUriRequired', '/analytics/']]);
  });
  ...
  it('splits properly a url with a short directory path', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriOptional', '/img'], ['networkChartRowItemUriRequired', '/image.jpg']]);
  });
  ...
  it('splits properly a url with a long directory path', function () {
    ...
    expect(getUrlShorteningParts()).toEqual([['networkChartRowItemUriOptional', 'https://'], ['networkChartRowItemUriRequired', 'mozilla.org'], ['networkChartRowItemUriOptional', '/assets/img'], ['networkChartRowItemUriRequired', '/image.jpg']]);
  });

  it('returns null with an invalid url', function () {
    const {
      getUrlShorteningParts
    } = setupForUrl('test.mozilla.org/img/optimized/');
...
describe('NetworkChartRowBar MIME-type filter', function () {
  ...
  it('searches for js MIME-Type', function () {
    const {
      rowItem
    } = setupWithPayload(getNetworkMarkers({ uri: 'https://test.mozilla.org/scripts/test.js' }));
  ...
  it('searches for css MIME-Type', function () {
    const {
      rowItem
    } = setupWithPayload(getNetworkMarkers({ uri: 'https://test.mozilla.org/styles/test.css' }));
  ...
  it('uses default when no filter applies', function () {
    const {
      rowItem
    } = setupWithPayload(getNetworkMarkers({ uri: 'https://test.mozilla.org/file.xuul' }));
