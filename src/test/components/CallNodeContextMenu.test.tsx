...
import { changeRightClickedCallNode, changeExpandedCallNodes, setContextMenuVisibility } from "../../actions/profile-view";
...
describe('calltree/CallNodeContextMenu', function () {
  ...
  describe('basic rendering', function () {
    it('does not render the context menu when it is closed', () => {
      ...
      const {
        container
      } = setup(createStore(), isContextMenuOpen);
    ...
    it('renders a full context menu when open, with many nav items', () => {
      ...
      expect(ensureExists(container.querySelector('.react-contextmenu'), `Couldn't find the context menu root component .react-contextmenu`).children.length > 1).toBeTruthy();
  ...
  describe('clicking on call tree transforms', function () {
    ...
    const fixtures = [{ matcher: /Merge node/, type: 'merge-call-node' }, { matcher: /Merge function/, type: 'merge-function' }, { matcher: /Focus.*subtree/, type: 'focus-subtree' }, { matcher: /Focus.*function/, type: 'focus-function' }, { matcher: /Collapse.*subtree/, type: 'collapse-function-subtree' }, { matcher: /Collapse functions/, type: 'collapse-resource' }, { matcher: /Collapse.*recursion/, type: 'collapse-direct-recursion' }, { matcher: /Drop samples/, type: 'drop-function' }];
    ...
    fixtures.forEach(({
      ...
      it(`adds a transform for "${type}"`, function () {
        ...
        expect(selectedThreadSelectors.getTransformStack(getState())[0].type).toBe(type);
      });
    });
  });
  ...
  describe('clicking on the rest of the menu items', function () {
    it('can expand all call nodes in the call tree', function () {
      ...
      expect(selectedThreadSelectors.getExpandedCallNodeIndexes(getState())).toHaveLength(1);

      fireEvent.click(getByText('Expand all'));

      // This test only asserts that a bunch of call nodes were actually expanded.
      expect(selectedThreadSelectors.getExpandedCallNodeIndexes(getState())).toHaveLength(11);
    });
    ...
    it('can look up functions on SearchFox', function () {
      ...
      expect(window.open).toBeCalledWith('https://searchfox.org/mozilla-central/search?q=B', '_blank');
    });
    ...
    it('can copy a script URL', function () {
      ...
      thread.funcTable.fileName[funcIndex] = thread.stringTable.indexForString('https://example.com/script.js');
