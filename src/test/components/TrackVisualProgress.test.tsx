...
import { IndexIntoSamplesTable } from "../../types/profile";
...
function getSamplesPixelPosition(sampleIndex: IndexIntoSamplesTable): CssPixels {
...
describe('TrackVisualProgress', function () {
  function setup() {
    const profile = getVisualProgressTrackProfile(Array(SAMPLE_COUNT).fill('A').join('  '));
    const {
      meta: {
        visualMetrics
      }
    ...
    const renderResult = render(<Provider store={store}>
        <TrackVisualProgress progressGraphData={VisualProgress} graphDotTooltipText=" visual completeness at this time" windowId="0" />
      </Provider>);
    const {
      container
    } = renderResult;
    ...
    const canvas = ensureExists(container.querySelector('.timelineTrackVisualProgressCanvas'), `Couldn't find the memory canvas, with selector .timelineTrackVisualProgressCanvas`);
    const getTooltipContents = () => document.querySelector('.timelineTrackVisualProgressTooltip');
    const getVisualProgressDot = () => container.querySelector('.timelineTrackVisualProgressGraphDot');
    const moveMouseAtCounter = index => fireEvent(canvas, getMouseEvent('mousemove', { pageX: getSamplesPixelPosition(index) }));
