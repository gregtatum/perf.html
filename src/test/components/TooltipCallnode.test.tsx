...
import { getProfileWithUnbalancedNativeAllocations, getProfileFromTextSamples } from "../fixtures/profiles/processed-profile";
import { selectedThreadSelectors } from "../../selectors/per-thread";
import * as ProfileSelectors from "../../selectors/profile";
import { changeSelectedCallNode, changeCallTreeSummaryStrategy } from "../../actions/profile-view";
...
describe('TooltipCallNode', function () {
  function setup(profile) {
    ...
    function renderTooltip(): any {
      // This component is not currently connected.
      return render(<Provider store={store}>
          <TooltipCallNode thread={selectedThreadSelectors.getThread(getState())} pages={ProfileSelectors.getPageList(getState())} callNodeIndex={ensureExists(selectedThreadSelectors.getSelectedCallNodeIndex(getState()), 'Unable to find a selected call node index.')} callNodeInfo={selectedThreadSelectors.getCallNodeInfo(getState())} categories={ProfileSelectors.getCategories(getState())} interval={ProfileSelectors.getProfileInterval(getState())} durationText="Fake Duration Text" callTree={selectedThreadSelectors.getCallTree(getState())} callTreeSummaryStrategy={selectedThreadSelectors.getCallTreeSummaryStrategy(getState())} />
        </Provider>);
    }
  ...
  it('handles native allocations', () => {
    ...
    const threadIndex = 0;
    const callNodePath = ['A', 'B', 'Fjs', 'Gjs'].map(name => funcNamesDict[name]);
  ...
  describe('with page information', function () {
    function setupWithPageInformation(pageUrl: string, iframeUrl?: string) {
      ...
      for (let i = 1; i < frameTable.length; i++) {
        frameTable.innerWindowID[i] = profile.pages[profile.pages.length - 1].innerWindowID;
      }
    ...
    it('displays Page URL for non-iframe pages', () => {
      ...
      const {
        getByText,
        container
      } = setupWithPageInformation(pageUrl);
    ...
    it('displays Page URL for iframe pages', () => {
      ...
      const {
        getByText,
        container
      } = setupWithPageInformation(pageUrl, iframeUrl);
