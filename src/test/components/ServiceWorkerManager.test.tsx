...
import { fatalError, viewProfile, startSymbolicating, doneSymbolicating } from "../../actions/receive-profile";
...
describe('app/ServiceWorkerManager', () => {
  ...
  describe('with the `public` datasource', () => {
    ...
    it('shows a warning notice if the SW was updated before we were ready', async () => {
      ...
      installOptions.onUpdated();
      expect(ensureExists(container.querySelector('.photon-message-bar')).className).toMatch(/\bphoton-message-bar-warning\b/);
      expect(container.firstChild).toMatchSnapshot();
