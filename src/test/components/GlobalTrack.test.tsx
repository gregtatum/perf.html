...
import { changeSelectedThread, hideGlobalTrack } from "../../actions/profile-view";
...
describe('timeline/GlobalTrack', function () {
  ...
  function setup(trackIndex = GECKOMAIN_TAB_TRACK_INDEX) {
    ...
    const renderResult = render(<Provider store={store}>
        <GlobalTrack trackIndex={trackIndex} trackReference={trackReference} setInitialSelected={setInitialSelected} />
      </Provider>);
    const {
      container
    } = renderResult;

    const getGlobalTrackLabel = () => ensureExists(container.querySelector('.timelineTrackLabel'), `Couldn't find the track label with selector .timelineTrackLabel`);
    const getGlobalTrackRow = () => ensureExists(container.querySelector('.timelineTrackGlobalRow'), `Couldn't find the track global row with selector .timelineTrackGlobalRow`);
  ...
  it('matches the snapshot of a global process track', () => {
    const {
      container
    } = setup(GECKOMAIN_TAB_TRACK_INDEX);
