...
import { updatePreviewSelection, changeMarkersSearchString } from "../../actions/profile-view";
...
describe('MarkerTable', function () {
  function setup(markers) {
    ...
    jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(() => getBoundingBox(2000, 1000));

    // These were all taken from real-world values.
    const profile = getProfileWithMarkers(markers !== undefined ? markers : [['UserTiming', 12.5, {
      type: 'UserTiming',
      startTime: 12.5,
      endTime: 12.5,
      name: 'foobar',
    ...
    const fixedRows = () => Array.from(container.querySelectorAll('.treeViewRowFixedColumns'));
    const scrolledRows = () => Array.from(container.querySelectorAll('.treeViewRowScrolledColumns'));

    const getRowElement = functionName => ensureExists(getByText(functionName).closest('.treeViewRow'), `Couldn't find the row for node ${String(functionName)}.`);
