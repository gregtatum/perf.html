...
describe('Viewport', function () {
  ...
  function testScrollingHint(eventOptions) {
    ...
    const isTimerVisible = () => !getByText(/Zoom Chart/).classList.contains('hidden');
  ...
  describe('mousewheel zooming using ctrl + mousewheel', function () {
    ...
    const marginTests = [{ marginLeft: 0, marginRight: 0, description: 'no margins' }, { marginLeft: 33, marginRight: 0, description: 'with a left margin' }, { marginLeft: 0, marginRight: 33, description: 'with a right margin' }, { marginLeft: 33, marginRight: 55, description: 'with both margins' }];
    ...
    for (const {
      ...
      describe(description, function () {
        ...
        const innerComponentWidth = BOUNDING_BOX_WIDTH - marginLeft - marginRight;
        ...
        it('zooms the preview selection in a direction when the mouse is to the right', () => {
          ...
          const viewport = scrollAndGetViewport({
            deltaY: -10,
            ctrlKey: true,
            clientX: BOUNDING_BOX_LEFT + innerComponentWidth * 0.75 + marginLeft
          });
        ...
        it('zooms the preview selection in a direction when the mouse is to the left', () => {
          ...
          const viewport = scrollAndGetViewport({
            deltaY: -10,
            ctrlKey: true,
            clientX: BOUNDING_BOX_LEFT + innerComponentWidth * 0.25 + marginLeft
          });
  ...
  describe('dragging around', function () {
    ...
    it('will not drag beyond the bottom of the window', function () {
      ...
      clickAndDrag(middleX, middleY, middleX, middleY - MAX_VIEWPORT_HEIGHT * 2);
      expect(getChartViewport()).toMatchObject({
        viewportLeft: 0,
        viewportRight: 1,
        viewportTop: MAX_VIEWPORT_HEIGHT - BOUNDING_BOX_HEIGHT,
  ...
  describe('keyboard navigation', function () {
    it('zooms in and out at center', () => {
      ...
      expect(1 - getChartViewport().viewportRight).toBeCloseTo(getChartViewport().viewportLeft, 7);
...
function setup(profileOverrides: Object = {}) {
  ...
  jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(() => getBoundingBoxForViewport());
  ...
  const ConnectedChartWithViewport = explicitConnect({
    ...
    component: (props: Object) => <ChartWithViewport viewportProps={{
      ...
      ...profileOverrides
    }} chartProps={{// None used in dummy component.
    }} />
  });
  ...
  function viewportContainer() {
    return ensureExists(container.querySelector('.chartViewport'), `Couldn't find the viewport container, with the selector .chartViewport`);
  }
  ...
  function setBoundingBoxMock(override: $Shape<BoundingBoxOverride>): void {
    HTMLElement.prototype.getBoundingClientRect.mockImplementation(() => getBoundingBoxForViewport(override));
  }
