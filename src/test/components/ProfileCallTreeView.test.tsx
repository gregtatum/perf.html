...
import { getProfileFromTextSamples, getProfileWithJsAllocations, getProfileWithUnbalancedNativeAllocations, getProfileWithBalancedNativeAllocations } from "../fixtures/profiles/processed-profile";
import { createGeckoProfile } from "../fixtures/profiles/gecko-profile";

import { getEmptyThread, getEmptyProfile } from "../../profile-logic/data-structures";
import { changeCallTreeSearchString, changeImplementationFilter, changeInvertCallstack, commitRange, addTransformToStack } from "../../actions/profile-view";

import { Profile } from "../../types/profile";

beforeEach(() => {
  // Mock out the 2d canvas for the loupe view.
  jest.spyOn(HTMLCanvasElement.prototype, 'getContext').mockImplementation(() => mockCanvasContext());
  // This makes the bounding box large enough so that we don't trigger
  // VirtualList's virtualization. We assert this above.
  jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(() => getBoundingBox(1000, 2000));
});
...
describe('calltree/ProfileCallTreeView', function () {
  function setup(profile?: Profile) {
    ...
    const getRowElement = functionName => ensureExists(getByText(functionName).closest('.treeViewRow'), `Couldn't find the row for node ${functionName}.`);
  ...
  it('computes a width for a call tree of a really deep stack', () => {
    const {
      profile
    } = getProfileFromTextSamples(Array(113).fill('name').join('\n'));

    const {
      container
    } = setup(profile);
    const treeBody = ensureExists(container.querySelector('.treeViewBodyInner1'));
...
describe('calltree/ProfileCallTreeView navigation keys', () => {
  function setup(profileString: string, expectedRowsLength: number) {
    ...
    const renderedRows = container.querySelectorAll('.treeViewRow.treeViewRowScrolledColumns');
    ...
    return {
      ...
      simulateKey: (param: string | {key: string;}) => {
        const treeViewBody = ensureExists(container.querySelector('div.treeViewBody'), `Couldn't find the tree view body with selector div.treeViewBody`);
        fireEvent.keyDown(treeViewBody, // There's a shortcoming in either Flow or the flow type for the
        // `keyDown` method. $FlowExpectError
        param.key ? param : { key: param });
      },
      selectedText: () => ensureExists(container.querySelector('.treeViewRowScrolledColumns.isSelected'), `Couldn't find the selected column with selector .treeViewRowScrolledColumns.isSelected`).textContent
    };
  }
  ...
  it('reacts properly to up/down navigation keys', () => {
    ...
    const profileString = Array.from({ length: 100 }).reduce((result, func, i, array) => {
...
describe('calltree/ProfileCallTreeView TransformNavigator', () => {
  it('renders with multiple transforms applied', () => {
    ...
    store.dispatch(addTransformToStack(0, {
      type: 'focus-subtree',
      callNodePath: [A],
    ...
    expect(container.querySelector('.calltreeTransformNavigator')).toMatchSnapshot();
  });
});
...
describe('ProfileCallTreeView with balanced native allocations', function () {
  ...
  it('can switch to retained memory and back to timing', function () {
    ...
    expect(selectedThreadSelectors.getCallTreeSummaryStrategy(getState())).toEqual('native-retained-allocations');
