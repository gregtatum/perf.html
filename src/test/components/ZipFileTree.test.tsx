...
describe('calltree/ZipFileTree', function () {
  ...
  it('contains a list of all the files', async () => {
    ...
    ['foo', 'bar', 'profile1.json', 'profile2.json', 'baz', 'profile3.json'].forEach(fileName => getByText(fileName));
  });
  ...
  describe('clicking on a profile link', function () {
    const setupClickingTest = async () => {
      ...
      const waitUntilDoneProcessingZip = () => waitUntilState(store, state => ZippedProfileSelectors.getZipFileState(state).phase === 'VIEW_PROFILE_IN_ZIP_FILE');
    ...
    it('kicks off the profile loading process when clicked', async () => {
      ...
      expect(UrlStateSelectors.getPathInZipFileFromUrl(getState())).toBe('foo/bar/profile1.json');
