...
describe('app/ErrorBoundary', function () {
  ...
  function setupComponent(childComponent) {
    ...
    const results = render(<ErrorBoundary message={friendlyErrorMessage}>
        {childComponent}
      </ErrorBoundary>);
    return { spy, ...results };
  }
  ...
  it('shows the component children when there is no error', () => {
    const {
      getByText
    } = setupComponent(childComponentText);
  ...
  it('can click the component to view the full details', () => {
    ...
    expect(getByTestId('error-technical-details').classList.contains('hide')).toBe(true);

    // Click the button to expand the details.
    getByText('View full error details').click();

    // The technical error now exists.
    expect(getByTestId('error-technical-details').classList.contains('hide')).toBe(false);
  });
  ...
  it('reports errors to the analytics', () => {
    withAnalyticsMock(() => {
      ...
      expect(self.ga).toHaveBeenCalledWith('send', 'exception', {
        exDescription: ['Error: This is an error.', '', '    in ThrowingComponent', '    in ErrorBoundary'].join('\n'),
        exFatal: true
      });
    });
  });
