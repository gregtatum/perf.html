...
import { changeSelectedThread, hideLocalTrack } from "../../actions/profile-view";
import TimelineLocalTrack from "../../components/timeline/LocalTrack";
import { getRightClickedTrack, getLocalTrackFromReference, getProfile } from "../../selectors/profile";
import { ensureExists } from "../../utils/flow";
import { getSelectedThreadIndex } from "../../selectors/url-state";
import mockCanvasContext from "../fixtures/mocks/canvas-context";
import { getNetworkTrackProfile, getIPCTrackProfile } from "../fixtures/profiles/processed-profile";
import { getProfileWithNiceTracks, getStoreWithMemoryTrack } from "../fixtures/profiles/tracks";
...
function setup(store: Store, trackReference: TrackReference, localTrack: LocalTrack, threadIndex: ThreadIndex) {
  const {
    getState,
    dispatch
  } = store;
  ...
  jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(() => getBoundingBox(400, 400));

  const renderResult = render(<Provider store={store}>
      <TimelineLocalTrack pid={PID} localTrack={localTrack} trackIndex={trackReference.trackIndex} setIsInitialSelectedPane={setIsInitialSelectedPane} />
    </Provider>);
  ...
  const getLocalTrackLabel = () => ensureExists(container.querySelector('.timelineTrackLabel'), `Couldn't find the track label with selector .timelineTrackLabel`);
  const getLocalTrackRow = () => ensureExists(container.querySelector('.timelineTrackLocalRow'), `Couldn't find the track local row with selector .timelineTrackLocalRow`);
...
function setupThreadTrack() {
  ...
  const localTrack = getLocalTrackFromReference(store.getState(), trackReference);
