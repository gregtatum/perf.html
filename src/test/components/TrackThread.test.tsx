...
import { changeTimelineType, changeInvertCallstack } from "../../actions/profile-view";
...
import { FileIoPayload } from "../../types/markers";
...
describe('timeline/TrackThread', function () {
  ...
  function getMarkersProfile(testMarkers = [['Marker A', 0, { startTime: 0, endTime: 1 }], ['Marker B', 1, { startTime: 1, endTime: 2 }], ['Marker C', 2, { startTime: 2, endTime: 3 }], ['Marker D', 3, { startTime: 3, endTime: 4 }]]) {
  ...
  function setup(profile: Profile) {
    ...
    const stackGraphCanvas = () => ensureExists(container.querySelector('.threadStackGraphCanvas'), `Couldn't find the stack graph canvas, with selector .threadStackGraphCanvas`);
    const markerCanvas = () => ensureExists(container.querySelector(oneLine`
  ...
  it('can click a stack in the stack graph in normal call trees', function () {
    ...
    const getCallNodePath = () => selectedThreadSelectors.getSelectedCallNodePath(getState()).map(funcIndex => thread.stringTable.getString(thread.funcTable.name[funcIndex]));

    fireEvent(stackGraphCanvas(), getMouseEvent('mouseup', { pageX: STACK_1_X_POSITION }));
    expect(getCallNodePath()).toEqual(['a', 'b', 'c']);

    fireEvent(stackGraphCanvas(), getMouseEvent('mouseup', { pageX: STACK_2_X_POSITION }));
    expect(getCallNodePath()).toEqual(['d', 'e', 'f']);

    fireEvent(stackGraphCanvas(), getMouseEvent('mouseup', { pageX: STACK_3_X_POSITION }));
    expect(getCallNodePath()).toEqual(['g', 'h', 'i']);

    fireEvent(stackGraphCanvas(), getMouseEvent('mouseup', { pageX: STACK_4_X_POSITION }));
  ...
  it('adds disk io markers if they are present', function () {
    ...
    const {
      getByTestId
    } = setup(getMarkersProfile(fileIoMarker));
