...
import { changeSelectedThread, changeRightClickedTrack } from "../../actions/profile-view";
...
import { getHiddenGlobalTracks, getHiddenLocalTracks } from "../../selectors/url-state";
import { getProfileWithNiceTracks, getHumanReadableTracks } from "../fixtures/profiles/tracks";
...
describe('timeline/TrackContextMenu', function () {
  ...
  describe('selected global track', function () {
    function setupGlobalTrack(profile, trackIndex = 1) {
      ...
      const threadIndex = track.type === 'process' ? track.mainThreadIndex : null;
      if (threadIndex !== null) {
      ...
      const isolateProcessMainThreadItem = () => getByText(/Only show "Content Process"/);
      const trackItem = () => getByText('Content Process');
      const isolateScreenshotTrack = () => getByText(/Hide other screenshot tracks/);
    ...
    it('matches the snapshot of a global non-process track', () => {
      const {
        container
      } = setupGlobalTrack(getScreenshotTrackProfile());
      expect(container.firstChild).toMatchSnapshot();
    });
    ...
    it('isolates a process track without a main thread', function () {
      ...
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', 'show [process]', '  - show [thread DOM Worker]', '  - show [thread Style]']);
    ...
    it('isolates a screenshot track', () => {
      ...
      expect(getHumanReadableTracks(getState())).toEqual(['show [screenshots]', 'hide [screenshots]', 'show [process]', '  - show [thread Empty] SELECTED']);
    });
    ...
    it.skip('can present a disabled isolate item on non-process tracks', function () {// TODO - We should wait until we have some real tracks without a thread index.
    });

    it('network track will be displayed when a number is not set for showTabOnly', () => {
      const {
        container
      } = setupGlobalTrack(getNetworkTrackProfile(), 0);
  ...
  describe('selected local track', function () {
    ...
    // eslint-disable-next-line jest/no-disabled-tests
    it.skip('can isolate a non-thread track, as long as there process has a thread index', function () {// TODO - We should wait until we have some real non-thread tracks
    });
  });
  ...
  describe('global / local track visibility interplay', function () {
    function setupTracks() {
      ...
      const threadIndex = ensureExists(track.mainThreadIndex, `Couldn't get the mainThreadIndex of global track`);
    ...
    it('will unhide the global track when unhiding one of its local tracks', function () {
      ...
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', // The "GeckoMain tab" process is now hidden.
      ...
      '  - show [thread DOM Worker]', '  - show [thread Style]']);

      // Unhide "DOM Worker" local track.
      fireEvent.click(localTrackItem());
      expect(getHumanReadableTracks(getState())).toEqual(['show [thread GeckoMain process] SELECTED', // The "GeckoMain tab" process is visible again.
      'show [thread GeckoMain tab]', // Only the "DOM Worker" local track is visible.
      '  - show [thread DOM Worker]', '  - hide [thread Style]']);
    });
  });
});