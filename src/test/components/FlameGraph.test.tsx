...
import { changeInvertCallstack, changeSelectedCallNode, commitRange, changeImplementationFilter } from "../../actions/profile-view";
...
describe('FlameGraph', function () {
  ...
  it('can be navigated with the keyboard', () => {
    ...
    function selectedNode() {
      const callNodeIndex = selectedThreadSelectors.getSelectedCallNodeIndex(getState());
      return callNodeIndex && funcNames[callNodeIndex];
    }

    // Start out with callnode B selected
    dispatch(changeSelectedCallNode(0, [0, 1]
    /* B */
    ));
...
function setupFlameGraph() {
  ...
  function fireMouseEvent(eventName, options) {
    fireEvent(ensureExists(container.querySelector('canvas'), 'The container should contain a canvas element.'), getMouseEvent(eventName, options));
  }
  ...
  function getContentDiv() {
    return ensureExists(container.querySelector('.flameGraphContent'), `Couldn't find the content div with selector .flameGraphContent`);
  }
