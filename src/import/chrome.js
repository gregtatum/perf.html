...
export function convertChromeProfile(profile: mixed): Profile {
  if (!Array.isArray(profile)) {
    throw new Error(
...
function processTracingEvents(
  ...
  // Some profiles show sampling from anywhere bretween 100us to 300us.
  // Choose 500us as a somewhat reasonable sampling interval.
  profile.meta.interval = 0.5;
  const profileChunks: ProfileChunkEvent[] =
    (eventsByName.get('ProfileChunk'): any) || [];

  const otherCategoryIndex = ensureExists(
    profile.meta.categories.findIndex(({ name }) => name === 'Other'),
    'Expected to find a category index for the Other category'
  );
  ...
  for (const chunk of profileChunks) {
    ...
    if (nodes) {
      for (const { callFrame, id: nodeIndex, parent } of nodes) {
        ...
        frameTable.category[frameIndex] = null; // TODO
        ...
        stackTable.category.push(otherCategoryIndex);
        stackTable.prefix.push(prefixStackIndex);
        nodeIdToStackId.set(nodeIndex, stackTable.length++);
      }
    }
    // Chrome profiles sample much more frequently than Gecko ones due, and they store
    ...
    // is most likely slightly higher.
  ...
  extractScreenshots(profile, (eventsByName.get('Screenshot'): any));
...
function extractScreenshots(
  ...
  for (const screenshot of screenshots) {
    thread.markers.data.push({
      type: undefined,
      url: thread.stringTable.indexForString(
        'data:image/jpg;base64,' + screenshot.args.snapshot
      ),
      windowID: 'id',
      windowWidth: 100,
      windowHeight: 100,
