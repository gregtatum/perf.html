...
/**
 * This selector gets the hidden tracks for a PID, and ensure that one is selected
 * correctly. This makes it easier to avoid doing null checks everywhere.
 */
export const getHiddenLocalTracks: NonMemoizedSelector1<
  Set<TrackIndex>,
  Pid
> = (state, pid) =>
  ensureExists(
...
/**
 * This selector gets the local tracker order for a PID, and ensure that one is
 * selected correctly. This makes it easier to avoid doing null checks everywhere.
 */
export const getLocalTrackOrder: NonMemoizedSelector1<TrackIndex[], Pid> = (
  state,
  pid
) =>
  ensureExists(
...
/**
 * Search strings filters a thread to only samples that match the strings.
 */
...
export const getTransformStack: NonMemoizedSelector1<
  TransformStack,
  ThreadIndex
> = (state, threadIndex) => {
  return (
