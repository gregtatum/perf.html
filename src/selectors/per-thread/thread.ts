...
import { Thread, ThreadIndex, JsTracerTable, SamplesTable, NativeAllocationsTable } from "../../types/profile";
import { Selector } from "../../types/store";
import { ThreadViewOptions } from "../../types/state";
 ...
 */
export function getThreadSelectorsPerThread(threadIndex: ThreadIndex): any {
  const getThread: Selector<Thread> = state => ProfileSelectors.getProfile(state).threads[threadIndex];
  ...
  const getNativeAllocations: Selector<NativeAllocationsTable | void> = state => getThread(state).nativeAllocations;
  const getThreadRange: Selector<StartEndRange> = state => // This function is already memoized in profile-data.js, so we don't need to
   ...
   */
  const getTabFilteredThread: Selector<Thread> = createSelector(getThread, ProfileSelectors.getRelevantInnerWindowIDsForCurrentTab, (thread, relevantPages) => {
   ...
   */
  const getActiveTabFilteredThread: Selector<Thread> = createSelector(getThread, ProfileSelectors.getRelevantInnerWindowIDsForActiveTab, (thread, relevantPages) => {
    if (relevantPages.size === 0) {
      // If this set doesn't have any relevant page, just return the whole thread.
      return thread;
    }
  ...
  const getRangeFilteredThread: Selector<Thread> = createSelector(getTabFilteredThread, ProfileSelectors.getCommittedRange, (thread, range) => {
    const {
      start,
      end
    } = range;
  ...
  const getTransformStack: Selector<TransformStack> = state => UrlState.getTransformStack(state, threadIndex);

  const getRangeAndTransformFilteredThread: Selector<Thread> = createSelector(getRangeFilteredThread, getTransformStack, ProfileSelectors.getDefaultCategory, (startingThread, transforms, defaultCategory) => transforms.reduce( // Apply the reducer using an arrow function to ensure correct memoization.
  (thread, transform) => _applyTransformMemoized(thread, transform, defaultCategory), startingThread));

  const _getImplementationFilteredThread: Selector<Thread> = createSelector(getRangeAndTransformFilteredThread, UrlState.getImplementationFilter, ProfileSelectors.getDefaultCategory, ProfileData.filterThreadByImplementation);

  const _getImplementationAndSearchFilteredThread: Selector<Thread> = createSelector(_getImplementationFilteredThread, UrlState.getSearchStrings, (thread, searchStrings) => {
    return ProfileData.filterThreadToSearchStrings(thread, searchStrings);
  });

  const getFilteredThread: Selector<Thread> = createSelector(_getImplementationAndSearchFilteredThread, UrlState.getInvertCallstack, ProfileSelectors.getDefaultCategory, (thread, shouldInvertCallstack, defaultCategory) => {
    return shouldInvertCallstack ? ProfileData.invertCallstack(thread, defaultCategory) : thread;
  });

  const getPreviewFilteredThread: Selector<Thread> = createSelector(getFilteredThread, ProfileSelectors.getPreviewSelection, (thread, previewSelection): Thread => {
    ...
    return ProfileData.filterThreadSamplesToRange(thread, selectionStart, selectionEnd);
  });
   ...
   */
  const getSampleIndexOffsetFromCommittedRange: Selector<number> = createSelector(getThread, ProfileSelectors.getCommittedRange, ({
    samples
  }, {
    start,
    end
   ...
   */
  const getSampleIndexOffsetFromPreviewRange: Selector<number> = createSelector(getFilteredThread, ProfileSelectors.getPreviewSelection, getSampleIndexOffsetFromCommittedRange, ({
    samples
  }, previewSelection, sampleIndexFromCommittedRange) => {
    if (!previewSelection.hasSelection) {
      return sampleIndexFromCommittedRange;
    }

    const [beginSampleIndex] = ProfileData.getSampleIndexRangeForSelection(samples, previewSelection.selectionStart, previewSelection.selectionEnd);

    return sampleIndexFromCommittedRange + beginSampleIndex;
  });

  const getFriendlyThreadName: Selector<string> = createSelector(ProfileSelectors.getThreads, getThread, ProfileData.getFriendlyThreadName);

  const getThreadProcessDetails: Selector<string> = createSelector(getThread, ProfileData.getThreadProcessDetails);

  const getTransformLabels: Selector<string[]> = createSelector(getRangeAndTransformFilteredThread, getFriendlyThreadName, getTransformStack, Transforms.getTransformLabels);

  const getViewOptions: Selector<ThreadViewOptions> = state => ProfileSelectors.getProfileViewOptions(state).perThread[threadIndex];
   ...
   * into their own file.
   */
  const getJsTracerTable: Selector<JsTracerTable | null> = state => getThread(state).jsTracer || null;
   ...
   */
  const getExpensiveJsTracerTiming: Selector<JsTracerTiming[] | null> = createSelector(getJsTracerTable, getThread, (jsTracerTable, thread) => jsTracerTable === null ? null : JsTracer.getJsTracerTiming(jsTracerTable, thread));
   ...
   * based timing, and the leaf timing, so that they memoize nicely.
   */
  const getExpensiveJsTracerLeafTiming: Selector<JsTracerTiming[] | null> = createSelector(getJsTracerTable, getStringTable, (jsTracerTable, stringTable) => jsTracerTable === null ? null : JsTracer.getJsTracerLeafTiming(jsTracerTable, stringTable));
