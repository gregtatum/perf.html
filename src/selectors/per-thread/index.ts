...
import { getThreadSelectorsPerThread, ThreadSelectorsPerThread } from "./thread";
import { getMarkerSelectorsPerThread, MarkerSelectorsPerThread } from "./markers";
import { getStackAndSampleSelectorsPerThread, StackAndSampleSelectorsPerThread } from "./stack-sample";
import { getComposedSelectorsPerThread, ComposedSelectorsPerThread } from "./composed";
 ...
 */
export type ThreadSelectors = ThreadSelectorsPerThread & MarkerSelectorsPerThread & StackAndSampleSelectorsPerThread & ComposedSelectorsPerThread;
 ...
 */
export const getThreadSelectors = (threadIndex: ThreadIndex): ThreadSelectors => {
...
export const selectedThreadSelectors: ThreadSelectors = (() => {
  ...
  for (const key in anyThreadSelectors) {
    result[key] = state => getThreadSelectors(UrlState.getSelectedThreadIndex(state))[key](state);
  }
...
export const selectedNodeSelectors: NodeSelectors = (() => {
  const getName: Selector<string> = createSelector(selectedThreadSelectors.getSelectedCallNodePath, selectedThreadSelectors.getFilteredThread, (selectedPath, {
    stringTable,
    funcTable
  }) => {
  ...
  const getIsJS: Selector<boolean> = createSelector(selectedThreadSelectors.getSelectedCallNodePath, selectedThreadSelectors.getFilteredThread, (selectedPath, {
    funcTable
  }) => {
    if (!selectedPath.length) {
      return false;
  ...
  const getLib: Selector<string> = createSelector(selectedThreadSelectors.getSelectedCallNodePath, selectedThreadSelectors.getFilteredThread, (selectedPath, {
    stringTable,
    funcTable,
    resourceTable
  }) => {
    ...
    return ProfileData.getOriginAnnotationForFunc(ProfileData.getLeafFuncIndex(selectedPath), funcTable, resourceTable, stringTable);
  });

  const getTimingsForSidebar: Selector<TimingsForPath> = createSelector(selectedThreadSelectors.getSelectedCallNodePath, selectedThreadSelectors.getCallNodeInfo, ProfileSelectors.getProfileInterval, UrlState.getInvertCallstack, selectedThreadSelectors.getPreviewFilteredThread, selectedThreadSelectors.getThread, selectedThreadSelectors.getSampleIndexOffsetFromPreviewRange, ProfileSelectors.getCategories, ProfileData.getTimingsForPath);
