 ...
 */
export function getStackAndSampleSelectorsPerThread(selectors: {|
...
|}): * {
  ...
  const getRightClickedCallNodePath: Selector<CallNodePath | null> = state =>
    selectors.getViewOptions(state).rightClickedCallNodePath;
  ...
  const getCallTreeTimeCalculator: Selector<CallTree.CallTreeTimeCalculator> = createSelector(
    ProfileSelectors.getProfileInterval,
    CallTree.getTimeCalculatorForDuration
  );

  const getCallTreeJsAllocationCalculator: Selector<CallTree.CallTreeTimeCalculator> = createSelector(
    selectors.getJsAllocationMarkerIndexes,
    selectors.getMarkerGetter,
    (jsAllocationMarkerIndexes, getMarker) => {
      return function addMemorySize(
        thread,
        callNodeIndex,
        indexIntoJsAllocationMarkers
      ) {
