...
import { Thread, JsTracerTable } from "../../types/profile";
import { MarkerTimingRows, CombinedTimingRows, MarkerTiming } from "../../types/profile-derived";
import { StackTiming, StackTimingByDepth } from "../../profile-logic/stack-timing";
 ...
 */
export function getComposedSelectorsPerThread(threadSelectors: NeededThreadSelectors): any {
  /**
   ...
   * when it's absurd.
   */
  const getUsefulTabs: Selector<ReadonlyArray<TabSlug>> = createSelector(threadSelectors.getThread, threadSelectors.getIsNetworkChartEmptyInFullRange, threadSelectors.getJsTracerTable, getShowTabOnly, ({
    processType
  }, isNetworkChartEmpty, jsTracerTable, showTabOnly) => {
   ...
   */
  const getCombinedTimingRows: Selector<CombinedTimingRows> = createSelector(threadSelectors.getUserTimingMarkerTiming, threadSelectors.getStackTimingByDepth, (userTimingMarkerTiming, stackTimingByDepth): Array<MarkerTiming | StackTiming> => [...userTimingMarkerTiming, ...stackTimingByDepth]);
